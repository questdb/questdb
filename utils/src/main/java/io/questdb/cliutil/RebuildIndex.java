...
public class RebuildIndex implements Closeable {
    ...
    public RebuildIndex(String tablePath, CairoConfiguration configuration) {
        this.path = new Path();
                    ...
                    if (metadata.isColumnIndexed(columnIndex)) {
                        ...
                        if (PartitionBy.isPartitioned(partitionBy)) {
                            for (int partitionIndex = txReader.getPartitionCount() - 1; partitionIndex > -1; partitionIndex--) {
                                ...
                                rebuildIndex(indexer, columnName, sink.toString(), metadata.getIndexValueBlockCapacity(columnIndex), partitionSize);
                            }
                        }
                    }
                }
    ...
    private void rebuildIndex(SymbolColumnIndexer indexer, CharSequence columnName, String partitionName, int indexValueBlockCapacity, long partitionSize) {
        ...
        if (ff.exists(path)) {
            ...
            try (final MemoryMR roMem = indexMem) {
                ...
                createIndexFiles(columnName, indexValueBlockCapacity, plen, true);
