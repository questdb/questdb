---
name: review-pr
description: Review GitHub pull requests or local Git ranges against QuestDB coding standards using gh/git, review levels 0-3, and evidence-gated Critical, Moderate, and Minor findings.
---
...
Use Codex shell tools for read-only `gh`, Git, `rg`, and `rg --files` queries,
local file reads for source inspection, and fresh-context Codex subagents. Do not edit
files or push.
...
- **Read the full context of changed files** when the diff alone is ambiguous. Use file reads plus ripgrep (`rg` with shell tools) and `fd` to inspect the surrounding code, callers, and related tests.
...
Before launching review agents, produce a structured change surface map. This step is mandatory and must use `rg` and `rg --files` — do not reason about callsites from memory. The output of this step is required input for every agent in Step 3.
...
Run this step with Codex subagents. Select only roles whose domain is materially touched, obey the level's discovery cap, and launch those roles as fresh-context, read-only `reviewer` tasks. Agent count is never evidence and unused roles are skipped.
...
- It is free to use file reads and ripgrep (`rg` / `rg --files` with shell tools) to explore the repository however it wants.
...
- Work bottom-up from the code, not top-down from a checklist. Trace data flow through each function: what is read, how
  many times, in what order. Look for:
    ...
    - Intermediate collections built and then iterated once (build + iterate = two passes; a single streaming pass may
      suffice)
- Use file reads and ripgrep (`rg` / `rg --files` with shell tools) freely. Read callers to understand actual input sizes and access patterns — an O(n) scan that
