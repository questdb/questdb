...
5. Use fresh subagents as read-only advisers and reviewers when the environment
   exposes multi-agent tools. Give every child a self-contained task, the exact
   finding and paths it needs, and an explicit instruction not to edit
   project/source files. If no subagent facility is available, stop with
   `BLOCKED`; the primary agent's inline self-review cannot replace the
...
Set the item to `FIXING`. The primary Codex agent applies the production and
test changes as the sole writer.
...
Set the item to `REVIEWING`. Launch a fresh subagent as an independent,
...
The primary Codex agent then verifies every reviewer claim against source and
tests:
