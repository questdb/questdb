...
Task 6 adds [experimental planner selection](parallel-hash-join-group-by-planner.md);
the experimental setting defaults to false.
...
The benchmark package builds; no new performance claim is made. The component
measurements and the ordinary-plan baseline report were the available
measurements; the branch does not retain either. RFC task 7's four-worker
end-to-end 2× gate remains pending.
