# IlpV4WebSocketSender Design Document

## Overview

This document describes the design for completing the `IlpV4WebSocketSender` implementation. The key goal is to enable **streaming to hide latency** through a pipeline of in-flight microbatches, achieving higher throughput than request-response patterns while maintaining lower latency than large batch HTTP sends.

## Current State

### What Exists

1. **IlpV4WebSocketSender** (`core/src/main/java/io/questdb/cutlass/line/websocket/IlpV4WebSocketSender.java`)
...
2. **Server-side WebSocket Infrastructure** (in `core/src/main/java/io/questdb/cutlass/http/websocket/`)
   ...
   - `IlpV4WebSocketProcessor` - Adapter for binary message processing
...
The current `IlpV4WebSocketSender` sends each row immediately in `sendRow()`:
...
However, `IlpV4WebSocketSender` is unique among `Sender` implementations because it **internally spawns an I/O thread**. This creates a producer-consumer pattern that must be carefully designed:
...
### Phase 2: Make IlpV4WebSocketSender Work (Synchronous)

Wire `IlpV4WebSocketSender` to use `WebSocketChannel` for basic functionality.

**Changes to IlpV4WebSocketSender:**
- Create `WebSocketChannel` in `ensureConnected()`
- Implement `sendBinary()` calls
- Handle connection errors

**Tests for Phase 2:**
- `IlpV4WebSocketSenderBasicTest` - Basic send operations
- `IlpV4WebSocketSenderIntegrationTest` - Full round-trip with server
...
class MicrobatchBuffer {
    ...
    public boolean addRow(IlpV4TableBuffer table);
...
- `IlpV4WebSocketSenderBatchingTest` - Verify batching behavior
- `IlpV4WebSocketSenderAutoFlushTest` - Row/time-based auto-flush
...
*Integration Tests:*
- `IlpV4WebSocketSenderAsyncTest` - Non-blocking sends work
- `IlpV4WebSocketSenderThroughputTest` - Throughput improvement
...
- `IlpV4WebSocketSenderBackpressureTest` - Verify backpressure works
- `IlpV4WebSocketSenderErrorHandlingTest` - Error recovery
...
**Tests for Phase 6:**
- `IlpV4WebSocketSenderAuthTest` - Token and basic auth
- `IlpV4WebSocketSenderTlsTest` - TLS connection
...
**Tests for Phase 7:**
- `IlpV4WebSocketSenderReconnectTest` - Auto-reconnect scenarios
- `IlpV4WebSocketSenderStressTest` - Long-running stability
- `IlpV4WebSocketSenderConcurrencyTest` - Multi-threaded access
...
```java
IlpV4WebSocketSender sender = Sender.builder(Sender.Transport.WEBSOCKET)
...
| Test Class | Scenario |
|------------|----------|
| `IlpV4WebSocketSenderBasicIntegrationTest` | Simple send/receive |
| `IlpV4WebSocketSenderBatchingIntegrationTest` | Batch accumulation |
| `IlpV4WebSocketSenderBackpressureIntegrationTest` | Window limiting |
| `IlpV4WebSocketSenderReconnectIntegrationTest` | Connection recovery |
| `IlpV4WebSocketSenderTlsIntegrationTest` | TLS connections |
| `IlpV4WebSocketSenderAuthIntegrationTest` | Authentication |
...
2. Phase 2: Basic IlpV4WebSocketSender - Verification
...
- `IlpV4WebSocketSender.java` (exists, needs updates)
