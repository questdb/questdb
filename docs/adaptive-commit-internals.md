...
Two of these are **detection-only sidecars** — `_event.c` (WAL segment events) and `_txnlog.c`
(sequencer log). They carry no durability claim and are fully re-derivable, so their loss must
cost detection, never data or ingestion. `_epoch.enrol` is left in a table dir by a
