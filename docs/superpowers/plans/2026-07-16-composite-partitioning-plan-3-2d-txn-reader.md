...
**Interfaces:**
- `updateAttachedPartitionSizeByTimestamp` and siblings that today resolve a single partition by timestamp gain a `cellKey` (plain wrapper = 0). Raw-index mutators (`updatePartitionSizeByRawIndex:797-804`, `setPartitionSquashCounterByRawIndex:772-780`) are already raw-index-based and need no key change — but every CALLER that resolves a raw index from a timestamp must resolve from `(ts, cellKey)` (via Task 3's `findAttachedPartitionRawIndexBy`).
- `switchPartitions` (`TxWriter.java:510-530`) assumes strictly increasing timestamp at the tail. Under composite, the "current" partition is per-cell; make it correctly handle a multi-cell array rather than assuming the last array slot is the one active partition. `reconcileOptimisticPartitions` (`TxWriter.java:818-837`) walks backward assuming a single active tail partition — generalize to the `(ts, cellKey)` being committed. **Dormancy note:** all real writes in this plan are cellKey 0 at increasing ts, so the common append path stays a tail-append (unchanged) — the test drives multi-cell explicitly via the seam. Only make these methods *correct in the presence of* a multi-cell array; do NOT rewire the hot dormant cellKey-0 append unless correctness requires it.
- **Reuse, don't reimplement (Task-3 carry-forward):** if any mutator/switch path needs to INSERT a partition at a `(ts, cellKey)` position, promote Task 3's `insertPartitionForTest` ordered-insert logic (`TxWriter.java:294-300` — position via `findAttachedPartitionRawIndexBy`, physical insert via `insertPartitionSizeByTimestamp`) into a shared production-callable helper (e.g. `insertPartition(ts,size,nameTxn,cellKey)`) and call it from both the production path and the test seam — do not duplicate the position math. If no production path needs mid-array insert in this dormant plan, leave the primitive as-is and note that Plan 4 will promote it.
- **Do NOT change `getNextPartitionTimestamp`/`getNextExistingPartitionTimestamp` semantics** (Task-3 ⚠️): they advance by one stride and in a multi-cell array would land on the next cell of the same ts-run rather than the next distinct timestamp. Defining "next distinct ts vs next entry" depends on callers and is **Plan 4**'s job — this task must not further entrench a wrong semantics, but also must not attempt the fix. If a Task-4 change would call these on a multi-cell array, flag it in the report.

- [ ] **Step 1: Write the failing tests** (`CompositeTxCellTest`, via the seam):
  - Mutator no-aliasing: set `(day1,cell0)=size 10`, `(day1,cell1)=size 20`; `updateAttachedPartitionSize((day1,cell1), 25)`; assert cell1 size 25 and cell0 still 10. Remove `(day1,cell0)`; assert only `(day1,cell1)` remains at day1. Set a squash counter on `(day1,cell1)` and assert it doesn't touch `(day1,cell0)` (before removal).
  - **Mid-scan finder miss (Task-3 carry-forward):** build `(day1,cell0),(day1,cell1),(day1,cell9)`; assert `findAttachedPartitionRawIndexBy(day1, 2)` returns the negative insertion point positioned strictly BETWEEN cell1 and cell9 (exercises the `thisCellKey > cellKey` mid-scan branch `TxReader.java:216`, currently unverified).

- [ ] **Step 2: Run** — FAIL (size update aliases the first same-ts partition; the mid-scan assertion may already pass — that's fine, it's a coverage lock).

- [ ] **Step 3: Implement** — thread cellKey through the timestamp-resolving mutators/removers (resolve via `findAttachedPartitionRawIndexBy(ts, cellKey)`); make `switchPartitions`/`reconcileOptimisticPartitions` correct for a multi-cell array; promote the ordered-insert helper if a production path needs it (see Interfaces). Keep plain + dormant-cellKey-0 behavior byte-identical.
...
- `getPartitionIndexByTimestamp(ts)` stays timestamp-primary (time pruning); add a `(ts, cellKey)` variant for exact-partition resolution where needed.
- `insertPartition` inserts at the `(ts, cellKey)` position **and MUST populate the new record's slot 6 (`PARTITIONS_SLOT_OFFSET_CELL_KEY`) with the partition's cellKey** (carried from the Task-6 review as **T7-a**). `LongList.insert` arraycopy-shifts the array up without zeroing the opened region, so an inserted partition otherwise inherits a stale/garbage cellKey in slot 6 — the same slots-init bug class Tasks 1–2 fixed on the writer side. Set every slot the fresh-open `initOpenPartitionInfo` sets, slot 6 included; source the cellKey from the reloaded `txFile.getPartitionCellKey(...)` for the partition being inserted.
- The `columnVersionReader.getMaxPartitionVersion(ts)` call INSIDE the reload/refresh path (`reconcileOpenPartitions0`, ~line 1849 — the one call site Task 6 deliberately left 1-arg) MUST become cell-aware (**T7-b**): pass the cellKey the merge-diff resolved for the partition being refreshed, not a bare `(ts)`/cellKey-0. Resolving the right cellKey during reload is exactly what this task's 2-D merge-diff produces. Plain/dormant path (cellKey 0 everywhere) stays byte-identical.

- [ ] **Step 1: Write the failing test** — open a reader over a composite table with `(day1,cell0),(day2,cell0)`; via the write seam add `(day1,cell1)` and bump a txn; `reader.reload()`; assert the reader now sees 3 partitions in `(ts,cellKey)` order, that `(day1,cell1)` was classified INSERTED (not a refresh of `(day1,cell0)`), that **`reader.getPartitionCellKey(idx)` for the inserted `(day1,cell1)` returns exactly `1`** (not stale slot-6 garbage — the T7-a lock), and that `(day1,cell0)`'s open column state was not disturbed. Then a delete case: remove `(day1,cell0)`, reload, assert only `(day1,cell1)` remains at day1 and it wasn't spuriously closed/reopened. Include at least one reload that inserts a NEW cell whose slot-6 lands on a position previously holding a DIFFERENT cellKey (so a non-zeroing insert would surface stale data) — e.g. seed the array so the shift moves a cellKey-bearing record.
