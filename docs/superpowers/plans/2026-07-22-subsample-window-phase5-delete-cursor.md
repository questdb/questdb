...
> **Completed 2026-07-23; reconciled 2026-07-24:** Tasks 1, 2a, 2b, 2c, 3, and 4 are complete. Total routing landed in `79b66d3f56`; cursor/kill-switch/obsolete-A/B-benchmark deletion landed in `6c5482d5ae`. Follow-up review fixes bind SUBSAMPLE to the completed projected schema, map ordered traversal selections back to incoming rows, track all function-owned native memory, restore the safety-only max-row property, align cadence validation/RNG behavior, strengthen cursor-reuse tests, and add `SubsampleWindowBenchmark`. SUBSAMPLE still has one execution implementation: the keep-flag window path.
>
...
**Final state (reconciled 2026-07-24):** Tasks 1/2a/2b/2c/3/4 complete. The legacy cursor, codegen dispatch, kill-switch, cursor-only uniform/cadence helpers, and obsolete cursor-comparison benchmarks are deleted. The safety property `cairo.sql.subsample.max.rows` is intentionally retained for the five legacy count methods on internal SUBSAMPLE keep flags only; it does not restore a second execution path. Value-inspecting algorithm classes remain because the window factories call them. All function-owned native state is query-memory-tracked. Ordered traversal selections map back to incoming row IDs, so selection remains timestamp-ordered while output preserves incoming order. `SubsampleWindowBenchmark` continuously measures the sole path, including descending/reordered input.
