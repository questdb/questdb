...
- **Budget & runtime (REVISED after Task 3 measured reality — READ THIS):** the sweep costs **~4 s per crash point** (Task 3: 200 truncated points ≈ 805 s), so wall-time is `N × ~4 s` and N is the runtime knob. N (durability ops in one `commit()`) is dominated by *schema columns × epoch cadence*, not op type — at `transactionCount=20` + `CAIRO_ADAPTIVE_EPOCH_INTERVAL_MS=0` (epoch every batch, per-column fsync every apply), Task 3 measured **N≈877** → the cap-200 sweep truncated (silently skipping the `full-at-N` bar, which `assertW0Bars` only checks when `!truncated`) and ran ~13 min/seed. **Two nested targets:** (1) HARD ceiling `N ≤ 200` (`DEFAULT_ADAPTIVE_CRASH_POINT_CAP`) so the sweep never truncates and full-at-N is always asserted; (2) RUNTIME target **N ≈ 40–80** so each seed is ≈ 2–4 min and the CI-fast set (2–3 fixed + 1 random seed ⇒ 3–4 sweeps) stays within ~10 min. The runtime target (2) is the binding one — do not size to the 200 ceiling. **Mechanism — force exactly ONE deterministic epoch.** The epoch cadence gate (`ApplyWal2TableJob.fireDurableEpoch`, :692–703) is **wall-clock** (`nowMs = microClock.getTicks()/1000`; `currentMicros` is unpinned in this harness → real time) and fires iff `lastEpochTs == 0 || (nowMs − lastEpochTs) >= intervalMs`. A *moderate* interval (hundreds of ms, comparable to `commit()`'s own wall-time) makes the number of epochs timing-dependent → the sweep's `fired` determinism guard (`AbstractAdaptiveCrashSweepTest:213`) fails intermittently (armed op `base+k` drifts). Instead set `CAIRO_ADAPTIVE_EPOCH_INTERVAL_MS` **far larger than any test `commit()` duration** (e.g. `3_600_000` = 1 h): each sweep iteration recreates the table (fresh `SeqTxnTracker`, `lastEpochTs == 0`), so the **first** applied batch always fires exactly one epoch and no wall-clock delta can reach 1 h to fire a second — **deterministically one epoch per `commit()`**, robust to timing jitter. That single near-empty batch-1 epoch collapses the fsync storm to one materialize-fsync *and* leaves batches 2..M as a maximal lazy gap recovery must roll forward (strongly load-bearing → Task 5). N is then ≈ (one epoch's per-column fsyncs) + (~one msync per applied txn), so **`transactionCount` is the N knob — tune it to land N ≈ 40–80** (the epoch is fixed at one). Do **not** use `interval = −1` (zero epochs → no durable-epoch cut → recovery degenerates to full WAL replay → Task-5 control shows no difference); do **not** use a small interval (flaky). The implementer MUST measure N for the chosen counts, confirm `N ≤ 200` (`!truncated`), and confirm per-seed wall-time ≈ 2–4 min. CI-fast default = a small fixed-seed set (2–3) + one logged random seed; nightly override via `-Dfuzz.adaptive.crash.seeds=<n>`. This trades per-seed op-count for shape breadth-across-seeds — the correct trade for D2 (D1 already did exhaustive crash-point depth on fixed shapes).
...
- [ ] **Step 2: Force exactly one deterministic epoch, then size `transactionCount` to N ≈ 40–80.** Read the **Mechanism** paragraph in the Budget & runtime constraint first — the epoch clock is wall-clock, so a small interval is FLAKY. In `runSeedSweep`, change `CAIRO_ADAPTIVE_EPOCH_INTERVAL_MS` from `0` to a large constant **`3_600_000`** (1 h ≫ any test `commit()` duration): the fresh-table `lastEpochTs == 0` rule fires exactly one epoch on batch 1 and nothing can reach 1 h to fire a second, so N is stable across the count-pass and every sweep run (satisfies the `fired` guard) and batches 2..M form a maximal lazy gap. Keep the full profile (`fuzzOverrideMinimal = false`, the default). N is now ≈ (one epoch's fsyncs) + (~one msync per txn), so `transactionCount` (3rd arg of `setFuzzCounts` in `configureFuzz`, currently `20`) is the N knob — measure in Step 4 and tune it to land N ≈ 40–80 (hard ceiling ≤ 200). Note: `runSeedSweep` is shared, so this interval change also affects the Task-3 self-check and Tasks 5–7 — that is intended (all W=0 sweeps use the one-epoch model); re-run the Task-3 tests in Step 4 to confirm no regression.
...
- [ ] **Step 4: Run, read the logged N, and tune until exhaustive + green.** `mvn -q -pl core test -Dtest=RandomizedAdaptiveCrashFuzzTest#testFullLibraryW0 -DfailIfNoTests=false 2>&1 | tee /tmp/t4.log`. The driver logs `n=<N>`/`sweptPoints`/`truncated` per seed — read them. With the epoch fixed at one, **`transactionCount` is the only N knob**: if any seed reports `truncated=true` (N > 200) the `assertFalse` in `assertW0Bars` fails outright; more generally, if N is well above ~80 (a seed takes ≳ 5 min) lower `transactionCount`, if N < ~30 (sweep trivially shallow) raise it, until every seed is `!truncated` with N ≈ 40–80 and wall-time ≈ 2–4 min. Also re-run the three Task-3 tests (`testFingerprintMembershipPrimitive`, `testTwinFingerprintsDeterministic`, `testSelfCheckW0MinimalProfile`) — the shared `runSeedSweep`/`configureFuzz` changes must not regress them. Record the final `transactionCount`, the epoch interval, and per-seed N + wall-time in your report. Then confirm all three full-library seeds are green. Interpreting a real failure: (a) a fingerprint-canonicalization artifact (a truncate/drop-partition rendering ambiguously under `order by ts`) — dump the mismatch; if genuinely an ordering artifact of legitimately-equal rows, add a deterministic tiebreaker to the dump's ORDER BY or lower that op's probability and note it; (b) a **real adaptive durability bug** (a recovered state matching no `fp[P]`) → STOP, capture the seed, file a GA-blocker, do not weaken a bar.
...
- [ ] **Step 2: Run to verify.** Expected: PASS (and the disabled arm demonstrably recovers less at ≥1 k). The one-epoch model (Task 4: `CAIRO_ADAPTIVE_EPOCH_INTERVAL_MS = 3_600_000`, one batch-1 epoch) leaves batches 2..M as a maximal lazy gap, so with roll-forward disabled recovery lands back near the batch-1 epoch — a large, load-bearing difference. If disabled and enabled are somehow identical everywhere, deepen the gap by **increasing `transactionCount`** (more post-epoch rows); do *not* change the interval (it must stay ≫ commit duration to keep exactly one deterministic epoch — a smaller value is flaky, per the Budget Mechanism note).
