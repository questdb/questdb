...
- [ ] **Step 3: Implement the fallback as a convert PRE-PASS that commits at the prior seqTxn `S-1`.** (Design revised after investigation — see below. A single *physical* commit is impossible: `convertPartitionParquetToNative(false)`'s `bumpPartitionTableVersion` trips `replaceRange`'s front `commit()`, and the convert housekeeping only flushes via the self-committing `commitPendingParquetToNativeConversions()`. So convert-then-replace is inherently **two** physical commits. Crash-safety is preserved by making only the SECOND commit advance the delete's seqTxn.)

  Restructure `executeDelete` so the convert pre-pass runs **before** `setSeqTxn(seqTxn)`:
  1. Enumerate partitions via `tableWriter` getters and collect the Parquet partitions that the upcoming `replaceRange` will **rewrite** (not fully drop):
     - **Time-range route** (empty-replace over `[dLo, dHiExcl)`): only the ≤2 **boundary** partitions — the one whose `[floor, ceil)` contains `dLo` with `floor < dLo`, and the one containing `dHiExcl - 1` with `ceil > dHiExcl` — that are `getPartitionFormat(pi) == PartitionFormat.PARQUET`. (Interior fully-covered Parquet partitions drop inline via Task 2.2 — do NOT convert them.)
     - **Arbitrary route** (whole-range survivor-replace over `[minTs, maxTs+1)`): **every** Parquet partition (the whole-range replace rewrites all partitions; there is no cheap per-partition match check). Document this heavy side-effect (an arbitrary DELETE un-tiers all Parquet partitions) as a v1 limitation, improved when the per-partition survivor fast-path lands.
  2. For each collected partition: `tableWriter.convertPartitionParquetToNative(partitionTimestamp, false)`. If any were collected, `tableWriter.commitPendingParquetToNativeConversions()` — this is **commit #1**, which persists the CURRENT in-memory seqTxn (`S-1`, because `setSeqTxn(S)` has NOT run yet) plus the native-format change and its reopen/old-dir housekeeping.
  3. THEN the existing delete: `tableWriter.setSeqTxn(seqTxn)`, `txnBefore = getTxn()`, `replaceRange(...)` (now over native partitions) — **commit #2** advances seqTxn to `S`.
  4. Crash-safety: a crash between commit #1 and commit #2 leaves the table at persisted `S-1` with some partitions converted parquet→native (data fully intact); `ApplyWal2TableJob` re-runs txn `S`; the re-issued `convertPartitionParquetToNative(false)` on a now-native partition is an **idempotent no-op**, then the replace proceeds. No lost or partial delete. This mirrors the non-destructive/idempotent guarantee `ALTER CONVERT PARTITION` already relies on. **Do NOT** leave `setSeqTxn(S)` before the convert — that would persist `S` at commit #1 and a crash would silently lose the DELETE.
  Keep the `catch (Throwable)` rollback scaffolding around the whole thing; a throw in the convert pre-pass rolls back the uncommitted convert and leaves the table at `S-1` for retry.
