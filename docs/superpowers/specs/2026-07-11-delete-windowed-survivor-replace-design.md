...
## 6a. Key implementation risk — PROVEN (spike done, gate cleared)

The whole design rested on one unproven property of `TableWriter`: that **N per-window `processWalCommitFinishApply` calls accumulate into a single `commit00()`** with no intermediate txn persist and no cross-window corruption. This has now been **proven both analytically and empirically** (2026-07-11), ahead of any production wiring, so the design is viable as drawn.

**Analytical proof.** `processWalCommitFinishApply` (`TableWriter.java:11283-11333`) contains no `txWriter.commit()` and no `commit00()`; it only appends partition column data (`processO3Block`) and mutates in-memory lag/row-count state. The durable `_txn`/seqTxn write happens exclusively in `commit00()` → `commitTxWriterAndPublishPendingPostingSealPurges()` → `txWriter.commit()` (`TableWriter.java:5288`); `TxWriter.setSeqTxn` is an in-memory field assignment. So N applies before one `commit00()` persist exactly one txn/seqTxn. A bonus discovered here: each apply *consumes and resets* the lag min/max (`getLagMinTimestamp` then `setLag…(MAX/MIN)` at 11292-11295) — the clean per-window slate the windowing needs.

**Empirical proof.** `TableWriter.replaceRange` was refactored into `beginReplaceRange` / `applyReplaceRangeWindow` (×N) / `finishReplaceRange` (+ `abortReplaceRange`), with the old single-call `replaceRange` retained as a behaviour-preserving wrapper. Three spike tests were added to `TableWriterReplaceRangeDirectTest` and pass:

1. **Two disjoint windows, single commit** — `getTxn()` stays frozen across *both* `applyReplaceRangeWindow` calls and advances **exactly once** at `finishReplaceRange`; the result is byte-identical to the whole-range `NOT`-predicate oracle. (Resolves unknown #1 and #2.)
2. **Four windows (one per partition), survivors in every window** — no commit mid-sweep, single commit at finish, result matches oracle. (Resolves unknown #3: one `beginPartitionSizeUpdate` tolerates N surgeries.)
3. **Rollback-before-finish** — after applying a window then `rollback()` (the faithful crash model — it reloads `_txn` exactly as a post-crash reopen does), row count and txn are restored to the durable state, a fresh writer sees src == orig, and re-applying the full delete reaches the correct final state (idempotent retry). (Resolves crash-safety property (d).)

Regression: the 8 pre-existing `TableWriterReplaceRangeDirectTest` cases, `DeleteTest` (39), and `WalWriterReplaceRangeTest` (59) all remain green (**109 tests, 0 failures**) — the wrapper is a faithful decomposition. The `beginReplaceRange` / `applyReplaceRangeWindow` / `finishReplaceRange` primitive is therefore **already implemented and committed**; the plan's Task 1 is this spike (done), and Task 2+ wire `OperationExecutor.executeDelete` onto it.

Two mechanical decisions locked in by the spike: (i) `lastPartitionTimestamp` + the empty-table check are refreshed **per window** (an earlier window may have dropped/trimmed partitions), and (ii) `memColumnShifted` is cleared **per window** (each window is a self-contained apply), not once at the end.
