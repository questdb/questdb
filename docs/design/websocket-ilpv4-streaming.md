    ...
    public void testWriteUpgradeResponseWithProtocol() {
        ...
        try {
            ...
            handshake.writeUpgradeResponse(
                socket,
                new DirectUtf8String().of("dGhlIHNhbXBsZSBub25jZQ=="),
                new DirectUtf8String().of("ilpv4")
            );

            String response = socket.getWrittenString();

            Assert.assertTrue(response.contains("Sec-WebSocket-Protocol: ilpv4\r\n"));
        } finally {
            freeBuffer(buf, 512);
        }
    }
            ...
            try (TestServerMain server = startWithEnvVariables()) {
                ...
                WebSocketClient client = new WebSocketClient(
                    ...
                    new Draft_6455(Collections.emptyList(),
                        Collections.singletonList(new Protocol("ilpv4")))
                ) {
                    @Override
                    public void onOpen(ServerHandshake handshake) {
                        protocolRef.set(
                ...
                Assert.assertEquals("ilpv4", protocolRef.get());
...
```java
public class IlpV4WebSocketProcessorTest { /* 50+ tests */ }
public class IlpV4WebSocketProcessorStateTest { /* 30+ tests */ }
```
...
```java
public class IlpV4WebSocketE2ETest { /* 40+ tests */ }
public class IlpV4WebSocketStressTest { /* 20+ tests */ }
public class IlpV4WebSocketBenchmarkTest { /* 10+ tests */ }
```

### Phase 7: Client SDK Tests

```java
public class IlpV4WebSocketSenderTest { /* 40+ tests */ }
public class IlpV4WebSocketSenderIntegrationTest { /* 25+ tests */ }
```
