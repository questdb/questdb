...
javac -cp "$allocation_asm_jar:benchmarks/target/benchmarks.jar" -d "$allocation_agent_dir" benchmarks/allocation/*.java
...
jar cfm "$allocation_agent_dir/agent.jar" "$allocation_agent_dir/MANIFEST.MF" -C "$allocation_agent_dir" .
{
    ...
    sha256sum benchmarks/target/benchmarks.jar benchmarks/allocation/*.java "$allocation_asm_jar"
...
run_allocation_case() {
    ...
    for pass in bytes sites; do
        ...
        printf '%q ' -cp "$allocation_agent_dir:$allocation_asm_jar:benchmarks/target/benchmarks.jar" org.questdb.HashJoinGroupByAllocationBenchmark "$storage" "$mode" "$pass" "$rows" "$owners" "$join" >> "$allocation_output_dir/commands.txt"
        ...
        java "${allocation_java_args[@]}" "-javaagent:$allocation_agent_dir/agent.jar=$agent_mode" \
            -cp "$allocation_agent_dir:$allocation_asm_jar:benchmarks/target/benchmarks.jar" \
