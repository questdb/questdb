...
typedef void (*merge_shuffle64_jni_fn)(void *, void *, int64_t, int64_t, int64_t, int64_t, int64_t);
...
static int64_t mix64(int value) {
    uint64_t x = (uint64_t) value * UINT64_C(0x9E3779B97F4A7C15);
    x ^= x >> 30;
...
static void build_merge_index(buffers_t *buffers, int rows, int ooo_percent) {
    ...
    buffers->src_in_order = xaligned_alloc(64, (size_t) buffers->in_order_rows * sizeof(int64_t));
    buffers->src_ooo = xaligned_alloc(64, (size_t) buffers->ooo_rows * sizeof(int64_t));
    buffers->dst = xaligned_alloc(64, (size_t) rows * sizeof(int64_t));
    buffers->expected = xaligned_alloc(64, (size_t) rows * sizeof(int64_t));

    for (int i = 0; i < buffers->in_order_rows; i++) {
        buffers->src_in_order[i] = mix64(i);
    }
    for (int i = 0; i < buffers->ooo_rows; i++) {
        buffers->src_ooo[i] = mix64(i + buffers->in_order_rows);
    }
...
static void validate_once(merge_shuffle64_jni_fn fn, buffers_t *buffers, int rows) {
    memset(buffers->dst, 0, (size_t) rows * sizeof(int64_t));
    fn(
            NULL,
            NULL,
    ...
    if (memcmp(buffers->dst, buffers->expected, (size_t) rows * sizeof(int64_t)) != 0) {
        for (int i = 0; i < rows; i++) {
...
static uint64_t run_iterations(merge_shuffle64_jni_fn fn, buffers_t *buffers, int rows, int iterations) {
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        checksum += (uint64_t) buffers->dst[(i + rows - 1) % rows];
    }
    return checksum;
}

static uint64_t run_seconds(merge_shuffle64_jni_fn fn, buffers_t *buffers, int rows, double seconds, int *completed) {
    volatile uint64_t checksum = 0;
    ...
    while (now_ns() < deadline) {
        ...
        checksum += (uint64_t) buffers->dst[(iterations + rows - 1) % rows];
        iterations++;
    }
...
int main(int argc, char **argv) {
    ...
    dlerror();
    merge_shuffle64_jni_fn merge64 = (merge_shuffle64_jni_fn) dlsym(handle, "Java_io_questdb_std_Vect_mergeShuffle64Bit");
    ...
    validate_once(merge64, &buffers, opts.rows);

    if (opts.warmup > 0) {
        run_iterations(merge64, &buffers, opts.rows, opts.warmup);
    }
    ...
    uint64_t checksum;
    if (opts.seconds > 0.0) {
        checksum = run_seconds(merge64, &buffers, opts.rows, opts.seconds, &iterations);
    } else {
        checksum = run_iterations(merge64, &buffers, opts.rows, iterations);
    }
