    ...
    $.fn.quickVis = function (msgBus) {
        ...
        function resize() {
            ...
            const viewportHeight =
                Math.max(
                    Math.round((wh - viewport.getBoundingClientRect().top)) - defaults.bottomMargin,
                    defaults.minVpHeight
                );
            viewport.style.height = viewportHeight + 'px';
            const containerHeight = Math.max(
                Math.round((wh - div[0].getBoundingClientRect().top)) - defaults.bottomMargin,
        ...
        function executeQueryAndDraw() {
            ...
            if (arrayEquals(selectedXAxis, xAxis) && arrayEquals(selectedYAxis, yAxis) && query === cachedQuery) {
                draw(cachedResponse);
            } else {
