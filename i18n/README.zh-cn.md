...
也兼容 InfluxDB Line 协议以提供不受数据库模式影响的高吞吐数据获取能力，
并提供用于查询、批量导入和导出的 REST API。QuestDB 使用了 ANSI SQL ，其包含时间导向的原生扩展语言功能。这些扩展能更简单的连接（JOIN）多个来源的关联数据以及时间序列数据。QuestDB 通过列导向的存储模型、大规模
