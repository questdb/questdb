...
Line 协议提供模式无关高吞吐数据获取能力，并提供用于查询、批量导入和导出的 REST
API。 QuestDB 实现了 ANSI SQL 与面向时序的语言功能本地扩展。这些扩展使得使用传统
关系模式和时间序列模式完成多个来源的数据 join 起来变得更加简单。QuestDB 通过面向
列的存储模型、大规模并行化矢量执行、SIMD 指令和各种低延迟技术实现了高性能。整个
代码库是用 Java 和 C++从头开始构建的，没有任何外部依赖，并且 100% 不受垃圾回收的
影响。
