...
<p align="center">
  <a href="#contribute">
    <img src="https://img.shields.io/github/contributors/questdb/questdb" alt="QuestDB дописувачі з відкритим кодом"/>
  </a>
</p>
...
QuestDB надає багаторівневий механізм зберігання (WAL → нативний → Parquet на об'єктному сховищі), і вся кодова база
побудована з нуля на zero-GC Java, C++ та Rust (QuestDB Enterprise).

> Готові розпочати? Переходьте до
> розділу [Початок роботи](#спробуйте-questdb).
...
- Прийом з низькою затримкою та високою пропускною здатністю — від одиничних подій до мільйонів/сек
- SQL з низькою затримкою з розширеннями часових рядів (ASOF JOIN, SAMPLE BY, LATEST ON)
- SIMD-прискорене, паралельне виконання; швидко працює на скромному обладнанні
- Багаторівневе зберігання: WAL → нативне колонкове → Parquet (розділене за часом та упорядковане за часом)
...
- [Фінансові функції](https://questdb.com/docs/reference/function/finance/)
  та [аналітика книги замовлень](https://questdb.com/docs/guides/order-book/)

### QuestDB відмінно справляється з:

- даними фінансових ринків (тикові дані, угоди, книги замовлень, OHLC)
...
## Спробуйте QuestDB, демо та панелі
...
Для безпечної роботи у більшому масштабі або в рамках великих організацій.

Додаткові функції включають:

- високу Доступність та реплікацію для читання
- прийом з кількох основних джерел
- інтеграцію холодного зберігання
...
- [Посібники:](https://questdb.io/tutorial/) дізнайтесь що можливо з QuestDB, крок за кроком
...
- [Публічний Slack:](https://slack.questdb.io/) спілкуйтеся з командою QuestDB та членами спільноти
...
- Застрягли? Приєднуйтесь до нашого [публічного Slack](https://slack.questdb.io/) для допомоги

✨ Як знак нашої вдячності, ми надсилаємо QuestDB swag нашим учасникам!

Велике спасибі цим чудовим людям, які внесли свій вклад у
QuestDB [ключ емодзі](https://allcontributors.org/docs/en/emoji-key):
