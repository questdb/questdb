...
QuestDB 是一個開源的 SQL 時序數據庫，支持高吞吐量數據和快速 SQL 查詢並且操作簡單
。其中應用範圍包括金融市場數據、感測器數據、實時數據、儀表板和基礎設施監控。它兼
容 InfluxDB 行協議以提供不受模式影響的高吞吐量數據，也兼容 PostgreSQL 線路協議，並
提供 REST API 以便於批量導入和導出。

QuestDB 使用了 ANSI SQL，並包含了時間導向的 SQL 語句。這些 SQL 語句能更簡單的連
接（JOIN）多個來源的關聯數據以及時間序列數據。QuestDB 通過列導向的存儲模型、大規
