...
<p align="center">
  <a href="https://slack.questdb.io">
    <img src="https://slack.questdb.io/badge.svg" alt="Slack-канал сообщества QuestDB"/>
  </a>
  <a href="#contribute">
...
[English](https://github.com/questdb/questdb) | [简体中文](./i18n/README.zh-cn.md) | [繁體中文](./i18n/README.zh-hk.md) | [العربية](./i18n/README.ar-dz.md) | [Italiano](./i18n/README.it-it.md) | [Українська](./i18n/README.ua-ua.md) | [Español](./i18n/README.es-es.md) | [Português](./i18n/README.pt.md) | [日本](./i18n/README.ja-ja.md) | Русский

# QuestDB
QuestDB — это база данных временных рядов с открытым исходным кодом для высокопроизводительного приема и быстрых SQL-запросов с простотой эксплуатации. Он поддерживает независимый от схемы прием с использованием линейного протокола InfluxDB, проводного протокола PostgreSQL и REST API для массового импорта и экспорта.

QuestDB хорошо подходит для данных финансового рынка, метрик приложений, данных датчиков, аналитики в реальном времени, информационных панелей и мониторинга инфраструктуры.

QuestDB реализует ANSI SQL с собственными расширениями SQL временных рядов. Эти расширения SQL упрощают сопоставление данных из нескольких источников с помощью соединений реляционных и временных рядов. Мы достигаем высокой производительности, применяя модель хранения данных, ориентированную на столбцы, параллельное выполнение векторов, инструкции SIMD и методы с малой задержкой. Вся кодовая база построена с нуля на Java и C++, без зависимостей и нулевой сборки мусора.

<div align="center">
  <a href="https://demo.questdb.io">
    <img alt="Веб-консоль QuestDB, показывающая оператор SQL и результат запроса" src="https://raw.githubusercontent.com/questdb/questdb/master/.github/console.png" width="800" />
  </a>
</div>

## Пытаться QuestDB

Мы предоставляем [демонстрацию](https://demo.questdb.io/) с последними
Выпуск QuestDB и образцы наборов данных:

- Поездки: 10 лет поездок на такси в Нью-Йорке, 16 миллиарда строк
- Сделки: данные рынка криптовалют в реальном времени с более чем 30 млн строк в месяц
- Pos: расположение 250 тысяч уникальных кораблей с течением времени



## Установить QuestDB

Для запуска QuestDB, [Docker](https://www.docker.com/) можно использовать для быстрого старта:
...
[QuestDB страница загрузок](https://questdb.io/get-questdb/) обеспечивает прямую загрузку двоичных файлов и содержит сведения о других методах установки и развертывания.

### Подключиться к QuestDB

Вы можете взаимодействовать с QuestDB, используя следующие интерфейсы:

- [Веб-консоль](https://questdb.io/docs/develop/web-console/) для интерактивного редактора SQL на порту `9000`
- [InfluxDB линейный протокол](https://questdb.io/docs/reference/api/influxdb/) для приема данных с высокой пропускной способностью через порт `9009`
- [REST API](https://questdb.io/docs/reference/api/rest/) в порту `9000`
- [PostgreSQL проводной протокол](https://questdb.io/docs/reference/api/postgres/) в порту `8812`

### Вставить данные

Ниже представлены наши официальные клиенты questdb для популярных языков программирования.:
...
## HКак QuestDB сравнивается с другими TSDB с открытым исходным кодом

[](https://questdb.io/blog/2021/07/05/comparing-questdb-timescaledb-influxdb/) статье QuestDB сравнивается с другими базами данных временных рядов с открытым исходным кодом по функциональности, зрелости и производительности.

Вот высокомощные
[Пакет тестов временных рядов](https://questdb.io/blog/2021/06/16/high-cardinality-time-series-data-performance/)
rрезультаты с помощью `cpu-only` сценарий использования от 6 до 16 рабочих на 32 ЦП и 64 ГБ ОЗУ:

<div align="center">
  <a href="https://github.com/questdb/questdb/releases/tag/7.0.1">
    <img alt="Диаграмма, сравнивающая скорость загрузки QuestDB, InfluxDB и TimescaleDB" src="https://github.com/questdb/questdb/raw/master/.github/questdb7.0-benchmark.png" width="600"/>
  </a>
</div>
...
- [Документация QuestDB:](https://questdb.io/docs/introduction/) понять, как запускать и настраивать QuestDB.
- [Учебники:](https://questdb.io/tutorial/) lзарабатывайте то, что возможно с QuestDB шаг за шагом.
- [Дорожная карта продукта:](https://github.com/questdb/questdb/projects) ознакомьтесь с нашим планом для предстоящих выпусков.

### ❓ Получать поддержку

- [Сообщество Slack:](https://slack.questdb.io) присоединяйтесь к техническим обсуждениям, задавайте вопросы и знакомьтесь с другими пользователями!
- [Проблемы с GitHub:](https://github.com/questdb/questdb/issues) rсообщать об ошибках или проблемах с QuestDB.
- [Переполнение стека:](https://stackoverflow.com/questions/tagged/questdb) искать общие решения по устранению неполадок.

### 🚢 Развернуть QuestDB

- [AWS AMI](https://questdb.io/docs/guides/aws-official-ami)
- [Облачная платформа Google](https://questdb.io/docs/guides/google-cloud-platform)
- [Официальный образ Docker](https://questdb.io/docs/get-started/docker)
- [Капли DigitalOcean](https://questdb.io/docs/guides/digitalocean)
- [Графики Kubernetes Helm](https://questdb.io/docs/guides/kubernetes)

## Способствовать

Мы всегда рады внести свой вклад в проект, будь то исходный код, документация, отчеты об ошибках, запросы функций или отзывы. Чтобы начать вносить свой вклад:

- Просмотрите проблемы GitHub с пометкой
  "[Хороший первый выпуск](https://github.com/questdb/questdb/issues?q=is%3Aissue+is%3Aopen+label%3A%22Good+first+issue%22)".
- Читать
  [руководство по взносам](https://github.com/questdb/questdb/blob/master/CONTRIBUTING.md).
- Подробнее о сборке QuestDB см.
  [инструкции по сборке](https://github.com/questdb/questdb/blob/master/core/README.md).
- [Создать вилку](https://docs.github.com/en/github/getting-started-with-github/fork-a-repo)
  QuestDB и отправьте запрос на вытягивание с предлагаемыми изменениями.

✨ В знак нашей благодарности мы также отправляем **добычу QuestDB** нашим участникам. [Заявите о своей добыче здесь.](https://questdb.io/community)

Большое спасибо следующим замечательным людям, которые внесли свой вклад в QuestDB: ([смайлик ключ](https://allcontributors.org/docs/en/emoji-key)):
...
Этот проект придерживается [все участники](https://github.com/all-contributors/all-contributors) Спецификация. Пожертвования любого рода приветствуются!
