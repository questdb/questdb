# Branch Research: `jh_experiment_new_ilp` - QWP v1 WebSocket Protocol
...
This branch introduces **QWP v1** (InfluxDB Line Protocol version 4), a new **binary, column-oriented
...
| **Client**              | New `QwpWebSocketSender` with double-buffering, async I/O, in-flight window                    | ~10 new client classes     |
| **WebSocket transport** | New zero-GC native WebSocket client with platform-specific implementations                     | ~7 new transport classes   |
| **Server**              | WebSocket upgrade processor, frame parsing, connection management, QWP v1 message processing   | ~12 new server classes     |
| **WAL**                 | New `ColumnarRowAppender` interface and `WalColumnarRowAppender` for column-first writes       | 3 new/modified WAL classes |
| **Bridge**              | `QwpWalAppender` connecting server decoder to WAL writer                                       | 1 new bridge class         |
| **Tests**               | Comprehensive unit, integration, and E2E tests                                                 | ~55 new test files         |

---

## 2. Protocol: QWP v1 Binary Wire Format

### 2.1 Overview

The protocol is fully specified in `QWP_SPECIFICATION.md` (674 lines). Key properties:
...
Defined in `QwpConstants.java` (506 lines). Includes magic bytes, header offsets, flag bits,
...
table().symbol().at()                        |
    |                                        |
    v                                        |
QwpTableBuffer (per-table column buffers)  |
    |                                        |
    v                                        |
QwpWebSocketEncoder                        |
    |                                        |
...
**Package**: `io.questdb.cutlass.qwp.client`

| Class                    | Lines | Responsibility                                                                               |
|--------------------------|-------|----------------------------------------------------------------------------------------------|
| `QwpWebSocketSender`     | 1418  | Main sender implementing `Sender` interface. Fluent API + fast-path API.                     |
| `QwpWebSocketEncoder`    | 790   | Encodes table buffers into QWP v1 binary format with gorilla, schema caching, delta symbols. |
...
| `WebSocketResponse`      | 283   | Response parsing for QWP v1 protocol responses.                                              |
...
**Additional QWP v1 column types beyond `Sender` interface:**
...
Backpressure chain: Server slow -> InFlightWindow fills -> I/O thread blocks -> SendQueue backs up -> User thread blocks
on enqueue.
...
This means: QWP v1 message -> WebSocket frame -> TCP send with **zero intermediate copies**.
...
HTTP Request (GET /write/v4)
    |
    v
QwpWebSocketUpgradeProcessor (HttpRequestProcessor)
...
resumeRecv() loop
    ...
    v
handleBinaryMessage()
    |-- QwpProcessorState.addData() + processMessage()
    |-- QwpStreamingDecoder: decodes QWP v1 binary format
    |-- QwpWalAppender: appends decoded data to WAL
    |-- commit()
    |
    v
...
| Class                          | Lines | Responsibility                                                             |
|--------------------------------|-------|----------------------------------------------------------------------------|
| `QwpWebSocketUpgradeProcessor` | 718   | HTTP processor handling upgrade + WebSocket frame loop                     |
| `QwpProcessorState`            | 382   | Per-connection state: buffer, decoder, appender, symbol dict, ACK tracking |
| `QwpStreamingDecoder`          | 162   | Streaming decoder for QWP v1 messages                                      |
| `QwpTudCache`                  | 373   | Table Update Details cache (maps table name -> writer)                     |
| `QwpWebSocketProcessor`        | 126   | Adapter for binary message processing                                      |
| `QwpWebSocketProcessorState`   | 258   | WebSocket-level connection state                                           |
| `QwpWebSocketHttpProcessor`    | 156   | Handshake validation helper                                                |
...
- Uses `LocalValue<QwpProcessorState>` for thread-safe per-connection state
- Single-threaded per connection (all processing on one HTTP worker thread)

### 5.7 Known Server Architecture Issues

- **Dual state classes**: `QwpWebSocketProcessorState` (simpler, buffer-only) and
  `QwpProcessorState` (full state with WAL) coexist. The simpler one appears to be
  an earlier/alternative design that may be unused.
- **`QwpWebSocketProcessor`** (callback-based adapter): Appears to be an alternative/unused
  design. Actual frame dispatch is done directly in `QwpWebSocketUpgradeProcessor.handleWebSocketFrame()`.
- **Status code mismatch**: `QwpWebSocketUpgradeProcessor` uses its own status codes
  (0, 1, 3, 4, 255) while `QwpStatusCode` defines different codes (0-7). These don't fully align.
...
The decoder layer provides **flyweight cursor-based** access to QWP v1 binary data,
...
| Class                 | Lines | Responsibility                                      |
|-----------------------|-------|-----------------------------------------------------|
| `QwpMessageHeader`    | 356   | Parses 12-byte message header                       |
| `QwpMessageCursor`    | 303   | Iterates over table blocks in a message             |
| `QwpTableBlockCursor` | 562   | Iterates over columns/rows in a table block         |
| `QwpTableHeader`      | 273   | Parses table header (name, row count, column count) |
| `QwpSchema`           | 511   | Schema parsing and validation                       |
| `QwpSchemaCache`      | 104   | XXH64-based schema lookup cache                     |
| `QwpSchemaHash`       | 574   | Schema hashing for reference mode                   |
| `QwpColumnDef`        | 167   | Column definition (name + type code)                |
...
| Cursor                      | Decodes                                                    |
|-----------------------------|------------------------------------------------------------|
| `QwpFixedWidthColumnCursor` | BYTE, SHORT, INT, LONG, FLOAT, DOUBLE, DATE, UUID, LONG256 |
| `QwpTimestampColumnCursor`  | TIMESTAMP, TIMESTAMP_NANOS (handles gorilla decoding)      |
| `QwpBooleanColumnCursor`    | BOOLEAN (bit-packed)                                       |
| `QwpStringColumnCursor`     | STRING, VARCHAR (offset array + data)                      |
| `QwpSymbolColumnCursor`     | SYMBOL (dictionary + varint indices)                       |
| `QwpGeoHashColumnCursor`    | GEOHASH (variable precision)                               |
| `QwpArrayColumnCursor`      | DOUBLE_ARRAY, LONG_ARRAY (n-dimensional)                   |
| `QwpDecimalColumnCursor`    | DECIMAL64/128/256 (big-endian)                             |

### 6.4 Supporting Decoders

| Class                                     | Responsibility                         |
|-------------------------------------------|----------------------------------------|
| `QwpGorillaDecoder` / `QwpGorillaEncoder` | Delta-of-delta timestamp compression   |
| `QwpBitReader` / `QwpBitWriter`           | Bit-level I/O for gorilla and booleans |
| `QwpNullBitmap`                           | Null bitmap read/write utilities       |
| `QwpVarint`                               | LEB128 varint encoding/decoding        |
| `QwpZigZag`                               | ZigZag encoding for signed integers    |
| `QwpResponseEncoder`                      | Encodes server response messages       |
| `QwpStatusCode`                           | Status code enum                       |
| `QwpParseException`                       | Protocol parse error                   |
...
### 7.5 Bridge: `QwpWalAppender`

**File**: `core/src/main/java/io/questdb/cutlass/line/tcp/QwpWalAppender.java` (640 lines)
...
3. **Type conversion**: Maps QWP v1 types to QuestDB column types, handles nano/micro timestamp conversion
...
| WAL Appender       | 1          | QwpWalAppenderTest                                                                      |
...
| Benchmarks         | 2          | QwpAllocationTestClient, StacBenchmarkClient                                            |

### 8.2 Notable Test Highlights

- `WalColumnarRowAppenderTest` (3859 lines) - Extremely thorough columnar write testing
- `QwpWebSocketSenderReceiverTest` (4102 lines) - Full client-server protocol testing
- `WebSocketIntegrationTest` (1362 lines) - WebSocket protocol integration
- `QwpWebSocketEncoderTest` (1262 lines) - Encoder unit tests
...
- QWP v1 binary wire protocol specification and all type encoders/decoders
...
- Server-side frame processing and QWP v1 message decoding
- `QwpWebSocketSender` with fluent API and fast-path API
...
- `QwpWalAppender` bridge (streaming cursor to WAL)
...
- **Two WebSocket client implementations coexist**: `WebSocketChannel` (custom, in qwp/client/)
  and `WebSocketClient` (in cutlass/http/client/). Suggests in-progress migration.
- **Server status code alignment**: `QwpWebSocketUpgradeProcessor` uses codes (0,1,3,4,255)
  while `QwpStatusCode` defines codes (0-7). These need reconciliation.
...
- `putBooleanColumn()`, `putCharColumn()`, `putVarcharColumn()`, `putStringColumn()`,
  `putSymbolColumn()` wrap `QwpParseException` in `RuntimeException` instead of propagating
...
2. CLIENT: QwpTableBuffer
   Accumulates column data in per-column buffers

3. CLIENT: QwpWebSocketEncoder
   Encodes table buffers to QWP v1 binary:
   - Header (12 bytes)
...
7. SERVER: QwpWebSocketUpgradeProcessor.resumeRecv()
   Reads from socket, parses WebSocket frames, unmasks

8. SERVER: QwpProcessorState.processMessage()
   QwpStreamingDecoder decodes binary to cursor API

9. SERVER: QwpWalAppender.appendToWalStreaming()
...
12. SERVER: QwpWebSocketUpgradeProcessor
...
| Aspect       | ILP v1-v3 (Line Protocol)                  | QWP v1 (Binary)                                    |
|--------------|--------------------------------------------|----------------------------------------------------|
...
## 13. Files Changed (Non-QWP v1, from master merges)

The branch also includes changes unrelated to QWP v1 that were merged from master
...
1. **Wire up authentication** - The TODO in `QwpWebSocketSender.create()` for storing auth credentials
...
- Start with `QwpWebSocketSender.java` - it's the entry point
- Tests in `test/cutlass/line/websocket/` cover all sender functionality
- `QwpWebSocketEncoderTest.java` for encoding specifics

### If you want to work on the **server**:

- Start with `QwpWebSocketUpgradeProcessor.java` - HTTP -> WebSocket upgrade + frame processing
- `QwpProcessorState.java` orchestrates message decoding and WAL writing
...
- `QwpWalAppender.java` bridges protocol to WAL
- `WalColumnarRowAppenderTest.java` (3859 lines) is the most comprehensive test file

### If you want to work on the **protocol**:

- Start with `QWP_SPECIFICATION.md` for the formal spec
- `QwpConstants.java` for all constants
- Tests in `test/cutlass/http/qwp/` for each type's encoder/decoder

### If you want to work on the **WebSocket transport**:

- Client: `cutlass/http/client/WebSocketClient.java`
- Server: `cutlass/qwp/websocket/WebSocketFrameParser.java` + `WebSocketFrameWriter.java`
