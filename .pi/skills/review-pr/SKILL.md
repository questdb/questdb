...
The diff plus surface map can be large — write them to a shared file (e.g., under a temp/chain dir) and point each task at it via its `reads`/task text rather than pasting the whole payload into every task. The fresh-context adversarial agents (Agent 10, Agent 11) must NOT receive the change surface map, the test coverage map, or checklists; give them only the diff and changed-file names, per their instructions. The parent session owns synthesis, deduplication, and the final report — children only return findings.
...
Before launching review subagents, produce a structured change surface map. This step is mandatory and must use ripgrep (`rg`) and `fd` via Bash — do not reason about callsites from memory. The output of this step is required input for every subagent in Step 3.
...
Run this step with the `subagent` tool (see the `pi-subagents` skill). Launch the agents below as **parallel tasks**, one task per agent, each using the fresh-context `reviewer` builtin agent so every reviewer starts unanchored. Pass each task a read-only mandate (no edits) and the inputs it needs.
...
Launch the following agents in parallel (each as one `reviewer` subagent task).
...
Launch verification subagents in parallel (fresh-context `reviewer` agents via the `subagent` tool) where findings are independent. Each verification agent should read surrounding source files, not just the diff.
