...
5. Run `subagent({ action: "list" })` once before launching any child. Use only
   executable, non-disabled agents from that result. Prefer fresh-context
   `reviewer` agents for independent validation and review. If no executable
   `reviewer` is available, stop with `BLOCKED`; the parent's self-review cannot
...
Set the item to `REVIEWING`. Launch a fresh-context `reviewer` asynchronously,
then use `wait()` when no independent parent work remains. The review task must
be self-contained and must instruct the child not to modify project/source
files.
