...
- [x] **Phase 17: Address `/review-pr 6946` follow-ups (plan document at `~/.claude/plans/let-s-discuss-issues-one-gentle-elephant.md`)** — 2 Moderate + 9 Minor verified findings covering M1 (pre-1970 guard doc), M2 (predicate-pushdown test), M3 (four stale commits + one wrong impl-note sentence + test-count drift), and m1-m9 (slot-null fix in SqlCodeGenerator, field ordering, IntList/BitSet refactor, FillRecord property test, assert/throw rationale comment, Long256 CharSink contract comment, fillOffset in toSink0, DST spring-forward + single-row keyed + tight error-match tests, PR title rename). Plus Plan 05 test-hygiene addendum: 3 sibling D-23 findings (testFillKeyedRespectsCircuitBreaker try/finally field reset, testSortedRecordCursorFactoryConstructorThrow typed catch, SqlOptimiserTest#testSampleByFromToKeyedQuery plan-assertion upgrade). 5 plans, 11 git commits, 2 external PR metadata artefacts. SC#8 (/review-pr 6946 re-run) pending user action (completed 2026-04-22).
...
| 16. Fix multi-key FILL(PREV) with inline FUNCTION grouping keys | 1/1 | Complete | 2026-04-21 |
| 17. Address `/review-pr 6946` follow-ups (whole plan, M3 needs discuss) | 5/5 | Complete (SC#8 /review-pr 6946 re-run pending user action) | 2026-04-22 |
...
- [x] 17-03-PLAN.md — Test-only additions: testFillNullPushdownEliminatesFilteredKeyFills with data + multi-line EXPLAIN assertion (filter: s='s2' inside Async JIT Group By); 4 D-13 cross-reference comments on testSampleByAlignToCalendarFillNullWithKey1/2 in BOTH SampleByTest and SampleByNanoTimestampTest; m8a testFillWithOffsetAndTimezoneAcrossDstSpringForward (Europe/Riga 2021-03-28 companion to fall-back fixture); m8b testFillKeyedSingleRowFromTo (hasKeyPrev false->true transition); m8c testFillPrevRejectNoArg tightened from contains-any-of to exact-substring + position (actual message "PREV argument must be a single column name" at position 43 -- RESEARCH.md prediction was wrong; deviation documented). SampleByFillTest 124 -> 127 tests. (2 commits b2408afc9b + 5d1d12451c; completed 2026-04-22)
- [x] 17-04-PLAN.md — PR body + title edits + D-26 todo retirement: 5 body edits (D-11 M1 pre-1970 row, D-03 K x B bullet rewrite with cancellation-CB disambiguation, D-05 two stale M3.4 sentence rewrites, D-06 drop test-count lines), D-24 title renamed to "feat(sql): add cross-column FILL(PREV) and move SAMPLE BY FILL onto fast path", D-28 todo moved to .planning/todos/completed/ with completed_at + completed_in: 17-01-PLAN.md. Phase-wide ASCII normalization applied: 41 non-ASCII bytes -> 0 (Rule 2 deviation to satisfy acceptance criterion). Optional "sub-day + TZ + FROM + OFFSET" row NOT added per RESEARCH.md D-05 master-support verdict. 1 commit 4529631666 (PR body + title are external artefacts). SC#8 (/review-pr 6946 re-run) deferred to user (Rule 4 deviation; all 11 findings addressed in code/tests/body). (completed 2026-04-22)
- [x] 17-05-PLAN.md — Test-hygiene addendum (D-23 siblings surfaced during Plan 03 execution via two auto-created todos): D-29 testFillKeyedRespectsCircuitBreaker try/finally + circuitBreakerConfiguration = null in finally + D-30 testSortedRecordCursorFactoryConstructorThrow catch narrowed to CairoException with single "Maximum number of pages" canonical substring (bundle commit d6d4b1628b); D-31 SqlOptimiserTest#testSampleByFromToKeyedQuery upgraded from bare smoke-test to assertPlanNoLeakCheck pinning Long Top K + Async Group By fast-path plan via probe-and-freeze (commit 6bdb014f2d). D-32 both source todos retired to .planning/todos/completed/ with completed_in: 17-05-PLAN.md. 7 unrelated pre-existing SqlOptimiserTest failures in testSampleByFromToBasicWhereOptimisation* cluster logged to deferred-items.md (out of scope). SC#8 still pending user action. (completed 2026-04-22)
