...
impl ParquetDecoder {
    ...
    /// Absolute file offset of `column`'s values in `row_group`, or `-1` when
    /// they cannot be addressed directly.
    ///
    /// Answers "where do this chunk's raw values start" for the one shape a
    /// caller can read without a decoder: a single uncompressed PLAIN data page
    /// with no dictionary and no nulls, whose payload is the values laid end to
    /// end. That is what the covering index writes for `row_id`, and reading it
    /// straight out of the mapping skips a JNI crossing, a thrift page header
    /// and a buffer per lookup -- which is the whole cost of a point read once
    /// the pruning has narrowed it to a handful of rows.
    ///
    ...
    pub unsafe fn plain_column_data_offset(
        ...
        if row_group_index >= self.row_group_count || column_index >= self.col_count {
            return Ok(-1);
        }
        let row_group = &self.metadata.row_groups[row_group_index as usize];
        let column = &row_group.columns()[column_index as usize];
        if column.compression() != parquet2::compression::Compression::Uncompressed {
            return Ok(-1);
        }
        let (col_start, col_len) = column.byte_range();
        let col_start = col_start as usize;
        let col_len = col_len as usize;
        if col_start + col_len > file_size as usize {
            return Ok(-1);
        }
        ...
        for maybe_page in page_reader {
            match maybe_page? {
                // A dictionary means the values are indices, not values.
                SlicedPage::Dict(_) => return Ok(-1),
                SlicedPage::Data(page) => {
                    if found.is_some() {
                        // More than one page: the caller would have to walk
                        // headers to find the right one, which is the cost this
                        // exists to avoid.
                        return Ok(-1);
                    }
                    ...
                    let encoding_id = match &page.header {
                        parquet2::page::DataPageHeader::V1(h) => h.encoding,
                        parquet2::page::DataPageHeader::V2(h) => h.encoding,
                    };
                    if encoding_id != parquet_format_safe::Encoding::PLAIN {
                        return Ok(-1);
                    }
                    // A definition level would prefix the payload and mean some
                    // values are absent; the index writes neither.
                    if page.descriptor.max_def_level != 0 || page.descriptor.max_rep_level != 0 {
                        return Ok(-1);
                    }
                    let base = file_ptr as usize;
                    let payload = page.buffer.as_ptr() as usize;
                    if payload < base || payload - base > file_size as usize {
                        return Ok(-1);
                    }
                    found = Some((payload - base) as i64);
                }
            }
        }
