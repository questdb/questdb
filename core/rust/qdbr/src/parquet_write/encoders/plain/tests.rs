...
/// M2 - Same mixed-shape input through the DEFAULT STRING page encoding,
/// DeltaLengthByteArray (`encode_string_delta` via `string_to_page`). The
/// delta path pre-writes all lengths (from `compute_utf8_length`) into the
/// delta-bitpacked stream and then appends the bytes, so a length/byte drift
/// only visible on a multi-row page would corrupt the file. Unlike the plain
/// test above, the lone surrogate sits BEFORE the valid rows: a drift that
/// only manifests when a substituted row precedes a good row must be caught.
#[test]
fn encode_string_delta_multi_row_mixed_lossy_and_strict() {
    fn build_col(rows: &[Option<&[u16]>]) -> (Vec<u8>, Vec<i64>) {
        let mut data: Vec<u8> = Vec::new();
        let mut offsets: Vec<i64> = Vec::with_capacity(rows.len());
        for row in rows {
    ...
    // Row 0: lone low surrogate 0xDFDA (the bad row FIRST).
    // Row 1: NULL.
    // Row 2: ASCII "hi".
    // Row 3: valid surrogate pair U+1F600 (UTF-8 F0 9F 98 80).
    let row_lone: Vec<u16> = vec![0xDFDA];
    let row_ascii: Vec<u16> = vec![b'h' as u16, b'i' as u16];
    let row_pair: Vec<u16> = vec![0xD83D, 0xDE00];
    let rows: &[Option<&[u16]>] = &[Some(&row_lone), None, Some(&row_ascii), Some(&row_pair)];
    let (data, offsets) = build_col(rows);
    let pt = primitive_type_for(ColumnTypeTag::String);

    // ---- Lossy: page must carry the substituted AND the preserved bytes ----
    {
        ...
        .expect("lossy delta multi-row encode must succeed");
        ...
        assert!(
            body.windows(3).any(|w| w == [0xEF, 0xBF, 0xBD]),
            "lone surrogate row must produce U+FFFD UTF-8 bytes on the delta path"
        );
        assert!(
            body.windows(4).any(|w| w == [0xF0, 0x9F, 0x98, 0x80]),
            "valid surrogate pair after the bad row must keep its real UTF-8 bytes"
        );
        assert!(
            body.windows(2).any(|w| w == [0x68, 0x69]),
            "ASCII row after the bad row must round-trip"
        );
    }
    ...
    // ---- Strict: must fail with the cross-type wording ----
    {
        ...
        assert!(
            result.is_err(),
            "strict delta encode must reject a lone surrogate regardless of row position"
        );
        let msg = result.unwrap_err().to_string();
        assert!(
            msg.contains("invalid UTF-16 data in string column"),
