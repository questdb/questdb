...
/// Reader over the footer of a `.qdbp` file.
///
...
impl<'a> Footer<'a> {
    ...
    pub fn new(data: &'a [u8]) -> ParquetResult<Self> {
        if data.len() < FOOTER_FIXED_SIZE + FOOTER_CHECKSUM_SIZE {
            return Err(qdbp_err!(QdbpErrorKind::Truncated, "footer too small"));
        }
    ...
    /// Total byte size of the footer including entries and checksum.
        ...
        FOOTER_FIXED_SIZE
            .checked_add(
                (row_group_count as usize)
                    ...
                    .ok_or_else(|| {
                        qdbp_err!(QdbpErrorKind::Truncated, "row_group_count overflow")
                    })?,
            )
            .and_then(|s| s.checked_add(FOOTER_CHECKSUM_SIZE))
            .ok_or_else(|| qdbp_err!(QdbpErrorKind::Truncated, "footer size overflow"))
    }
    ...
    pub fn row_group_block_offset(&self, index: usize) -> ParquetResult<u64> {
        ...
        let entry_data = self
            .data
            .get(o..o + 4)
            .ok_or_else(|| qdbp_err!(QdbpErrorKind::Truncated, "row group entry out of bounds"))?;
    ...
    pub fn checksum(&self) -> ParquetResult<u32> {
        ...
        let crc_data = self
            .data
            .get(o..o + 4)
            .ok_or_else(|| qdbp_err!(QdbpErrorKind::Truncated, "checksum field out of bounds"))?;
...
/// Builds a `.qdbp` footer into a `Vec<u8>`.
...
impl FooterBuilder {
    ...
    /// Writes the footer to `buf` (fixed fields + entries + 4-byte CRC placeholder).
