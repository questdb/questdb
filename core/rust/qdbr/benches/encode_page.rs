     ...
     $swap:expr, |$np:ident| $data:expr) => {
        for &$np in null_pcts(true) {
            ...
            $cases.push(EncodeBenchCase {
                ...
                row_count: ROW_COUNT,
                encode_fn: Box::new(move || {
                    black_box(bytes_to_page(&data, $swap, 0, opts, pt.clone()).expect("encode"));
                }),
            });
        }
    };
    ...
    ($cases:ident, $opts:ident, $src_len:expr, $precision:expr, $scale:expr, $label:literal) => {{
        ...
        for &null_pct in null_pcts(true) {
            ...
            $cases.push(EncodeBenchCase {
                ...
                row_count: ROW_COUNT,
                encode_fn: Box::new(move || {
                    black_box(bytes_to_page(&data, false, 0, opts, pt.clone()).expect("encode"));
                }),
            });
        }
    }};
    ...
    // Int96 timestamp (plain) — bytes_to_page
    {
        ...
        for &null_pct in null_pcts(true) {
            ...
            cases.push(EncodeBenchCase {
                ...
                row_count: ROW_COUNT,
                encode_fn: Box::new(move || {
                    black_box(bytes_to_page(&data, false, 0, options, pt.clone()).expect("encode"));
                }),
            });
        }
    }
