...
public class CachingRowIdAnalyticRecordSource extends AbstractCombinedRecordSource {
    ...
    public CachingRowIdAnalyticRecordSource(int pageSize, RecordSource parentSource, ObjList<AnalyticFunction> functions) {
    ...
    public RecordCursor prepareCursor(JournalReaderFactory factory, CancellationHandler cancellationHandler) throws JournalException {
        ...
        for (int i = 0; i < n; i++) {
            functions.getQuick(i).prepare(parentCursor);
        }

        long rowid = -1;
        while (parentCursor.hasNext()) {
            Record record = parentCursor.next();
            rowid = records.append(fakeRecord.of(record.getRowId()), rowid);
            for (int i = 0; i < n; i++) {
    ...
    public void toSink(CharSink sink) {
        sink.put('{');
        sink.putQuoted("op").put(':').putQuoted("CachingRowIdAnalyticRecordSource").put(',');
