...
public class JsonPathDefaultTypedExplicitDefaultFunctionFactory extends JsonPathFunctionFactoryBase {
    ...
    public JsonPathFunction newInstance(
        ...
        // Supply an explicit default value in case of execution errors.
    ...
    private Function parseDefaultValueFunction(int position, int targetType, ObjList<Function> args) throws SqlException {
        ...
        // TODO: Must also support SHORT, also not nullable. And write tests for this :-)
        final boolean isNullDefault = (defaultValueFn == null) || (defaultValueFn.getType() == ColumnType.NULL);

        if ((targetType == ColumnType.BOOLEAN) && isNullDefault) {
            throw SqlException
                    .position(position)
                    .put("json_path's the default value cannot be NULL for the boolean target type");
        }
