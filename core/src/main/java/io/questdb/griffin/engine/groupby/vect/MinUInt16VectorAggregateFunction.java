...
public class MinUInt16VectorAggregateFunction extends UInt16Function implements VectorAggregateFunction {
    ...
    public MinUInt16VectorAggregateFunction(int keyKind, int columnIndex, int timestampIndex, int workerCount) {
        ...
        if (keyKind == GKK_MICRO_HOUR_INT) {
            ...
            this.keyValueBitmapFunc = Rosti::keyedMicroHourMinShortBitmapNull;
            ...
            this.keyValueBitmapFunc = Rosti::keyedNanoHourMinShortBitmapNull;
        } else {
            this.distinctFunc = Rosti::keyedIntDistinct;
            this.keyValueFunc = Rosti::keyedIntMinShort;
            this.keyValueBitmapFunc = Rosti::keyedIntMinShortBitmapNull;
        }
    }

    @Override
    public void aggregate(long address, long frameRowCount, int workerId) {
        // Non-bitmap path: UINT16 always uses bitmap nulls, so this is a fallback.
        // Vect.minShort() uses signed comparison which is incorrect for unsigned values,
        // so this method is effectively a no-op for UINT16.
    }
    ...
    public void aggregate(long address, long bitmapAddr, long bitOffset, long frameRowCount, int workerId) {
        if (address != 0) {
            if (bitmapAddr != 0) {
                final long value = (long) Vect.minUInt16BitmapNull(address, bitmapAddr, bitOffset, frameRowCount);
