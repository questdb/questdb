...
public class SumUInt32VectorAggregateFunction extends LongFunction implements VectorAggregateFunction {
    ...
    public void aggregate(long address, long frameRowCount, int workerId) {
        if (address != 0) {
            sum.add(Vect.sumInt(address, frameRowCount));
            count.increment();
        }
    }
    ...
    public void aggregate(long address, long bitmapAddr, long bitOffset, long frameRowCount, int workerId) {
        if (address != 0) {
            if (bitmapAddr != 0) {
                ...
                if (nonNullCount > 0) {
                    sum.add(Vect.sumUInt32BitmapNull(address, bitmapAddr, bitOffset, frameRowCount));
                    count.increment();
                }
            } else {
