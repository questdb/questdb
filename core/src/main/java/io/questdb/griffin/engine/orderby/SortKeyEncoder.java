...
public class SortKeyEncoder implements QuietCloseable {
    ...
    /**
     * Checks whether all sort columns encode into a single fixed-width inline key
     * of at most 32 bytes - the layout the parallel top-K buffer and the limited
     * light cursor require, since neither carries a variable-length key heap.
     */
    public static boolean isFixedWidthSupported(RecordMetadata metadata, IntList sortColumnFilter) {
        int totalBytes = 0;
        for (int i = 0, n = sortColumnFilter.size(); i < n; i++) {
            int encoded = sortColumnFilter.getQuick(i);
            ...
            if (ColumnType.isSymbol(columnType)) {
                ...
                width = fixedColumnByteWidth(columnType);
            }
            if (width < 0) {
                return false;
            }
