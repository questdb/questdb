...
public class CountGeoHashGroupByFunctionLong extends AbstractCountGroupByFunction {
    ...
    public CountGeoHashGroupByFunctionLong(@NotNull Function arg) {
        super(arg);
        this.argColumnIndex = GroupByUtils.directArgColumnIndexByTag(arg, ColumnType.GEOLONG);
    }
    ...
    public void computeBatch(MapValue mapValue, long dataAddr, int rowCount, long startRowId) {
        if (rowCount > 0) {
            ...
            for (; dataAddr < hi; dataAddr += 8L) {
                if (Unsafe.getUnsafe().getLong(dataAddr) != GeoHashes.NULL) {
                    nonNullCount++;
                }
            }
    ...
    public void computeKeyedBatch(
        ...
        // Fast path: arg is a direct geolong column with data on the current frame.
