...
public class GroupByLong256HashSet {
    ...
    public long keyIndex(long k0, long k1, long k2, long k3) {
        long hashCode = Hash.hash64Long256(k0, k1, k2, k3);
    ...
    public GroupByLong256HashSet of(long ptr) {
        if (ptr == 0) {
            if (noKeyValue == 0) {
                this.ptr = allocator.calloc(HEADER_SIZE + 32L * initialCapacity);
            } else {
    ...
    private void rehash(int newCapacity, int newSizeLimit) {
        ...
        long oldPtr = ptr;
        if (noKeyValue == 0) {
            ptr = allocator.calloc(HEADER_SIZE + 32L * newCapacity);
        } else {
            ptr = allocator.malloc(HEADER_SIZE + 32L * newCapacity);
