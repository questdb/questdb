...
/**
 * Dev-mode test function {@code test_close_counter(s)}: a runtime-constant STRING function that
 * returns its argument and counts {@link Function#close()} calls. Tests use it to pin exact-once
 * closure of functions whose ownership moves between factories and cursors - for example the
 * SAMPLE BY timezone/offset/from/to parameters, which accept runtime-constant expressions.
 * Outside dev mode it folds to its constant argument, so it is inert in production.
 */
public class TestCloseCounterFunctionFactory implements FunctionFactory {
    ...
    private static class Func extends StrFunction {
        ...
        public void toPlan(PlanSink sink) {
            sink.val("test_close_counter('").val(value).val("')");
        }
    }
}
