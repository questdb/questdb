...
public class RegressionRmseFunctionFactory implements FunctionFactory {
    ...
    private static class RegressionRmseFunction extends DoubleFunction implements GroupByFunction, BinaryFunction {
        ...
        public double getDouble(Record rec) {
            ...
            if (count > 0) {
                ...
                // For one value, RMSE is 0 (perfect fit trivially, no error)
                if (count == 1) {
                    return 0.0;
                }

                // If there's no variance in X (all values are the same), regression is undefined
                if (sumX == 0) {
                    return Double.NaN;
                }

                // RMSE = sqrt(SS_res / n) where SS_res = sumY - (sumXY²/sumX)
                double ssRes = sumY - (sumXY * sumXY / sumX);

                // Ensure we don't have negative SS_res due to numerical errors
                if (ssRes < 0) {
                    ssRes = 0;
                }

                return Math.sqrt(ssRes / count);
            }
            return Double.NaN;
        }
