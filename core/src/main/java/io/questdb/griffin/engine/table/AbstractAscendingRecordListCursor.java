...
abstract class AbstractAscendingRecordListCursor extends AbstractPageFrameRecordCursor {
    ...
    public AbstractAscendingRecordListCursor(
    ...
    public void calculateSize(SqlExecutionCircuitBreaker circuitBreaker, Counter counter) {
        ...
        long remaining = rows.size() - rowIndex;
        if (remaining > 0) {
    ...
    public void skipRows(Counter rowCount) {
        ...
        long remaining = rows.size() - rowIndex;
        if (remaining > 0) {
            long rowsToSkip = Math.min(remaining, rowCount.get());
