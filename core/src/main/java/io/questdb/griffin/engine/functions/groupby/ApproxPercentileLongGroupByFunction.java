...
public class ApproxPercentileLongGroupByFunction extends DoubleFunction implements GroupByFunction, BinaryFunction {
    ...
    public ApproxPercentileLongGroupByFunction(Function left, Function right) {
        this.left = left;
        this.right = right;
    }
    ...
    public void computeFirst(MapValue mapValue, Record record) {
        ...
        if (histograms.size() <= histogramIndex) {
            histograms.extendAndSet(histogramIndex, histogram = new Histogram(numberOfSignificantValueDigits));
        } else {
            histogram = histograms.getQuick(histogramIndex);
        }
        histogram.reset();

        final long val = right.getLong(record);
