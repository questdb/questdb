...
public class RegressionTStatsFunctionFactory implements FunctionFactory {
    @Override
    public String getSignature() {
        return "regr_tstats(DD)";
    }
    ...
    public Function newInstance(int position, ObjList<Function> args, IntList argPositions, CairoConfiguration configuration, SqlExecutionContext sqlExecutionContext) {
        return new RegressionTStatsFunction(args.getQuick(0), args.getQuick(1));
    }


    private static class RegressionTStatsFunction extends DoubleFunction implements GroupByFunction, BinaryFunction {
        protected final Function xFunc;
        protected final Function yFunc;
        protected int valueIndex;

        public RegressionTStatsFunction(@NotNull Function arg0, @NotNull Function arg1) {
            this.yFunc = arg0;
            this.xFunc = arg1;
        }
        ...
        public double getDouble(Record rec) {
            ...
            if (count > 2) {
                ...
                // Avoid division by zero for X variance
                if (sumX == 0) {
                    return Double.NaN;
                }

                // Calculate slope: b = sumXY / sumX
                double slope = sumXY / sumX;

                // Calculate residual sum of squares: SS_res = sumY - (sumXY²/sumX)
                ...
                // Perfect fit: infinite t-statistic (zero standard error)
                if (ssRes == 0) {
                    return slope >= 0 ? Double.POSITIVE_INFINITY : Double.NEGATIVE_INFINITY;
                }

                // Mean squared error: MSE = SS_res / (n - 2)
                double mse = ssRes / (count - 2);

                // Standard error of the slope: SE_b = sqrt(MSE / sumX)
                double seSlope = Math.sqrt(mse / sumX);
