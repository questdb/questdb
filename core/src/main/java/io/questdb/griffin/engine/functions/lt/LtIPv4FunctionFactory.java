...
public class LtIPv4FunctionFactory implements FunctionFactory {
    ...
    public Function newInstance(
        ...
        // operand's side compiles to a plain unsigned comparison that reads 0.0.0.0 as
        // data, and only a nullable operand's side keeps its NULL exclusion. No per-row
        // isNotNull() calls, no per-row nullability flags.
    ...
    private static boolean isNeverNull(Function f) {
        return f.isNotNull() || (f.isConstant() && f.getIPv4(null) != Numbers.IPv4_NULL);
    }
    ...
    private static class LeftNullableFunc extends AbstractLtBinaryFunction {
        ...
        public boolean getBool(Record rec) {
            ...
            if (l == Numbers.IPv4_NULL) {
                return false;
            }
            return negated
                    ? Numbers.ipv4ToLong(l) >= Numbers.ipv4ToLong(r)
                    : Numbers.ipv4ToLong(l) < Numbers.ipv4ToLong(r);
        }
    }
