...
public class MinUInt64VectorAggregateFunction extends UInt64Function implements VectorAggregateFunction {
    ...
    public MinUInt64VectorAggregateFunction(int keyKind, int columnIndex, int timestampIndex, int workerCount) {
        ...
        if (keyKind == GKK_MICRO_HOUR_INT) {
            ...
            this.keyValueBitmapFunc = Rosti::keyedMicroHourMinUInt64BitmapNull;
            ...
            this.keyValueBitmapFunc = Rosti::keyedNanoHourMinUInt64BitmapNull;
        } else {
            this.distinctFunc = Rosti::keyedIntDistinct;
            this.keyValueFunc = Rosti::keyedIntMinLong;
            this.keyValueBitmapFunc = Rosti::keyedIntMinUInt64BitmapNull;
        }
    }
    ...
    public void aggregate(long address, long frameRowCount, int workerId) {
        ...
        // Vect.minLong() uses signed comparison which is incorrect for unsigned values,
    ...
    public void aggregate(long address, long bitmapAddr, long bitOffset, long frameRowCount, int workerId) {
        if (address != 0) {
            if (bitmapAddr != 0) {
                final long value = Vect.minUInt64BitmapNull(address, bitmapAddr, bitOffset, frameRowCount);
