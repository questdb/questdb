...
public class MaxShortGroupByFunction extends IntFunction implements GroupByFunction, UnaryFunction {
    ...
    public void computeBatch(MapValue mapValue, long ptr, int count, long startRowId) {
        if (count > 0) {
            mapValue.putInt(valueIndex, Vect.maxShort(ptr, count));
        }
    }
    ...
    public void merge(MapValue destValue, MapValue srcValue) {
        final int srcMax = srcValue.getInt(valueIndex);
        final int destMax = destValue.getInt(valueIndex);
        if (srcMax != Numbers.INT_NULL && (srcMax > destMax || destMax == Numbers.INT_NULL)) {
