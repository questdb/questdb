...
public class CastArrayToStrFunctionFactory implements FunctionFactory {

    @Override
    public String getSignature() {
        // any array to string
        // we use a generic signature or wildcard if possible,
        // but explicit wiring in FunctionParser handles the routing.
        return "cast(As)";
    }
    ...
    private static void printArray(ArrayView view, CharSink<?> sink) {
        if (view == null || view.isNull()) {
            // Null array prints as empty for now, or null?
            // ArrayTest output suggests generic behavior.
            // If it's the array object itself being null, usually it's handled by caller.
            // But if elements are null...
            // Standard Postgres array printing:
            // {val, val} or [[val,val]]?
            // QuestDB ArrayTest expects [[1.0,2.0],[4.0,5.0]]
            return;
        }
        printRecursive(view, 0, 0, sink);
    }

    // access logic from ArrayView
                ...
                switch (view.getElemType()) {
                    case ColumnType.DOUBLE:
                        ...
                        if (Double.isNaN(d) || Double.isInfinite(d)) {
                            sink.put("null"); // QuestDB null double usually prints as NaN or specific null?
                            // Numbers.isNull(d) check?
                            // ArrayTest output: [[1.0,2.0],[4.0,5.0]]
                            // Null values? check test.
                        } else {
                            sink.put(d);
                        }
                        break;
