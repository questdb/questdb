...
class AvgDecimal8Rescale64GroupByFunction extends Decimal64Function implements GroupByFunction, UnaryFunction {
    private final Function arg;
    private final Decimal64 decimal64A = new Decimal64();
    private final int position;
    private int valueIndex;

    public AvgDecimal8Rescale64GroupByFunction(@NotNull Function arg, int position, int targetType) {
        super(targetType);
        this.arg = arg;
    ...
    public int getDecimal32(Record rec) {
        if (calc(rec)) {
            return (int) decimal64A.getValue();
        }
    ...
    private boolean calc(Record rec) {
        ...
        if (count > 0) {
            try {
                decimal64A.of(rec.getLong(valueIndex), ColumnType.getDecimalScale(arg.getType()));
                decimal64A.divide(count, 0, ColumnType.getDecimalScale(type), RoundingMode.HALF_EVEN);
