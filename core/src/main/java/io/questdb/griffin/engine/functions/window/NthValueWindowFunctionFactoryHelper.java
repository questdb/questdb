 ...
 * TIMESTAMP arguments. nth_value returns the value of {@code expr} evaluated at the n-th row (1-based)
 * of the window frame, or NULL when {@code n} exceeds the current frame size. Each window shape has one
 * abstract base here (holding the framing logic and the native-long value buffers), and a thin DATE or
 * TIMESTAMP subclass in {@link NthValueTimestampWindowFunctionFactory} /
 * {@link NthValueDateWindowFunctionFactory} that only adds the per-type accessor.
 * <p>
 ...
 */
public class NthValueWindowFunctionFactoryHelper {
    ...
    static Function newInstance(
            ...
            UnboundedPartitionConstructor unboundedPartitionConstructor,
            PartitionRangeConstructor partitionRangeConstructor,
            PartitionRowsUnboundedConstructor partitionRowsUnboundedConstructor,
            PartitionRowsConstructor partitionRowsConstructor,
            WholeResultSetConstructor wholeResultSetConstructor,
            UnboundedRowsConstructor unboundedRowsConstructor,
            RangeConstructor rangeConstructor,
            RowsUnboundedConstructor rowsUnboundedConstructor,
        ...
        if (rowsHi < rowsLo) {
            return new AbstractWindowFunctionFactory.TimestampNullFunction(
                    args.get(0),
                    NAME,
                    rowsLo,
                    rowsHi,
        ...
        if (partitionByRecord != null) {
            if (framingMode == WindowExpression.FRAMING_RANGE) {
                ...
                if (windowContext.isDefaultFrame() && (!windowContext.isOrdered() || windowContext.getRowsHi() == Long.MAX_VALUE)) {
                    ...
                    try {
                        return unboundedPartitionConstructor.newFunction(
                                map,
                                partitionByRecord,
                                partitionBySink,
                                args.get(0),
                ...
                if (rowsLo == Long.MIN_VALUE && rowsHi == 0) {
                    ...
                    return currentRowConstructor.newFunction(args.get(0), n);
                } // whole partition
            ...
            if (framingMode == WindowExpression.FRAMING_RANGE) {
                ...
                if (!windowContext.isOrdered() && windowContext.isDefaultFrame()) {
                    return wholeResultSetConstructor.newFunction(args.get(0), n);
                } // between unbounded preceding and unbounded following -- whole result set
                else if (rowsLo == Long.MIN_VALUE && rowsHi == Long.MAX_VALUE) {
                    return wholeResultSetConstructor.newFunction(args.get(0), n);
                } // between unbounded preceding and current row
                else if (rowsLo == Long.MIN_VALUE && rowsHi == 0) {
                    return unboundedRowsConstructor.newFunction(args.get(0), n);
                ...
                if (rowsLo == Long.MIN_VALUE && rowsHi == Long.MAX_VALUE) {
                    ...
                    return rowsUnboundedConstructor.newFunction(args.get(0), rowsHi, n);
                } // between X preceding and [Y preceding | current row]
                else {
    ...
    interface CurrentRowConstructor {
        WindowFunction newFunction(Function arg, int n);
    }

    @FunctionalInterface
    interface PartitionConstructor {
        WindowFunction newFunction(Map map, VirtualRecord partitionByRecord, RecordSink partitionBySink, Function arg, int n);
    }
    ...
    @FunctionalInterface
    interface PartitionRowsUnboundedConstructor {
        WindowFunction newFunction(
                Map map,
                VirtualRecord partitionByRecord,
                RecordSink partitionBySink,
    ...
    interface RangeConstructor {
        WindowFunction newFunction(long rangeLo, long rangeHi, Function arg, CairoConfiguration configuration, int timestampIdx, int n);
    }

    @FunctionalInterface
    interface RowsConstructor {
        WindowFunction newFunction(Function arg, long rowsLo, long rowsHi, MemoryARW memory, int n);
    }

    @FunctionalInterface
    interface RowsUnboundedConstructor {
        WindowFunction newFunction(Function arg, long rowsHi, int n);
    }
    ...
    @FunctionalInterface
    interface UnboundedRowsConstructor {
        WindowFunction newFunction(Function arg, int n);
    }
    ...
    abstract static class NthValueOverCurrentRowBase extends BaseWindowFunction {

        private final int n;
        protected long value;

        NthValueOverCurrentRowBase(Function arg, int n) {
            super(arg);
            this.n = n;
        }
    ...
    abstract static class NthValueOverPartitionBase extends BasePartitionedWindowFunction {

        private final int n;

        public NthValueOverPartitionBase(Map map, VirtualRecord partitionByRecord, RecordSink partitionBySink, Function arg, int n) {
    ...
    abstract static class NthValueOverPartitionRangeFrameBase extends BasePartitionedWindowFunction {
        ...
        public NthValueOverPartitionRangeFrameBase(
                Map map,
                VirtualRecord partitionByRecord,
                RecordSink partitionBySink,
                long rangeLo,
        ...
        public void restoreCheckpointRingState(LiveViewCheckpointRingStateSource source, MapValue value) {
            ...
            if (ringRestore.rows != size) {
                throw CairoException.critical(0)
                        .put("live view checkpoint nth_value RANGE ring row count mismatch [expected=").put(size)
        ...
        public void freezeCheckpointRingState(LiveViewCheckpointRingStateSink sink, MapValue value) {
            // nth_value carries no running aggregate, so the scalar slot goes unused
            // (restore recomputes the emitted value from the ring) and stores 0.
            // frameSize is the in-frame count restore needs. The ring holds every
            // buffered row including nulls, which the shared reader now admits.
            ...
            sink.putScalarState(0L, frameSize);
            for (long i = 0; i < size; i++) {
         ...
         * allocates nothing per partition. The value column is a raw 64-bit word:
         * an nth_value ring row can be a NULL argument (LONG_NULL).
         */
    ...
    // simpler state-machine class (NthValueOverPartitionRowsFrameUnboundedBase) that
    // does not allocate a per-partition buffer.
    // removable cumulative aggregation
    abstract static class NthValueOverPartitionRowsFrameBase extends BasePartitionedWindowFunction {
        ...
        public NthValueOverPartitionRowsFrameBase(
                Map map,
                VirtualRecord partitionByRecord,
                RecordSink partitionBySink,
                long rowsLo,
            ...
            super(map, partitionByRecord, partitionBySink, arg);
            assert rowsLo > Long.MIN_VALUE; // use NthValueOverPartitionRowsFrameUnboundedBase for the unbounded-lo case
    ...
    abstract static class NthValueOverPartitionRowsFrameUnboundedBase extends BasePartitionedWindowFunction {
        ...
        public NthValueOverPartitionRowsFrameUnboundedBase(
                Map map,
                VirtualRecord partitionByRecord,
                RecordSink partitionBySink,
                long rowsHi,
    ...
    abstract static class NthValueOverRangeFrameBase extends BaseWindowFunction implements Reopenable {
        protected static final int RECORD_SIZE = 2 * Long.BYTES;
        protected final boolean frameIncludesCurrentValue;
        protected final boolean frameLoBounded;
        protected final LongList freeList = new LongList();
        ...
        public NthValueOverRangeFrameBase(
                long rangeLo,
                long rangeHi,
                Function arg,
                CairoConfiguration configuration,
    ...
    // uses a simpler state-machine class (NthValueOverRowsFrameUnboundedBase) that does
    // not allocate a ring buffer.
    // Removable cumulative aggregation.
    abstract static class NthValueOverRowsFrameBase extends BaseWindowFunction implements Reopenable {
        ...
        public NthValueOverRowsFrameBase(Function arg, long rowsLo, long rowsHi, MemoryARW memory, int n) {
            super(arg);
            // unbounded-lo cases route elsewhere: (MIN_VALUE, MAX_VALUE) -> NthValueOverWholeResultSetBase,
            // (MIN_VALUE, 0) -> NthValueOverUnboundedRowsFrameBase, (MIN_VALUE, K<0) -> NthValueOverRowsFrameUnboundedBase
    ...
    abstract static class NthValueOverRowsFrameUnboundedBase extends BaseWindowFunction implements Reopenable {
        protected final int bufferSize;
        protected final int n;
        ...
        public NthValueOverRowsFrameUnboundedBase(Function arg, long rowsHi, int n) {
            super(arg);
            assert rowsHi < 0 && rowsHi != Long.MIN_VALUE; // K preceding with K > 0; (int) Math.abs would overflow at MIN_VALUE
            this.bufferSize = (int) Math.abs(rowsHi);
            this.n = n;
    ...
    abstract static class NthValueOverUnboundedPartitionFrameBase extends BasePartitionedWindowFunction {
        ...
        public NthValueOverUnboundedPartitionFrameBase(
                Map map,
                VirtualRecord partitionByRecord,
                RecordSink partitionBySink,
                Function arg,
    ...
    abstract static class NthValueOverUnboundedRowsFrameBase extends BaseWindowFunction {
        protected final int n;
        protected long count;
        protected boolean isFound;
        protected long value = Numbers.LONG_NULL;

        public NthValueOverUnboundedRowsFrameBase(Function arg, int n) {
            super(arg);
            this.n = n;
        }
    ...
    abstract static class NthValueOverWholeResultSetBase extends BaseWindowFunction {
        protected final int n;
        protected long count;
        protected boolean isFound;
        protected long value = Numbers.LONG_NULL;

        public NthValueOverWholeResultSetBase(Function arg, int n) {
            super(arg);
            this.n = n;
        }
