...
public class FirstValueDateWindowFunctionFactory extends AbstractWindowFunctionFactory {
    ...
    /**
     * Create a date "first_value" window function instance appropriate for the current window context.
     *
     * <p>Validates the WindowContext, handles the special case where the frame is empty
     * (rowsHi &lt; rowsLo) by returning a DateNullFunction, and otherwise dispatches
    ...
    public Function newInstance(
        ...
        if (rowsHi < rowsLo) {
            return new DateNullFunction(args.get(0),
                    NAME,
                    rowsLo,
                    rowsHi,
     ...
     * date; otherwise it throws a SqlException.
        ...
        if (partitionByRecord != null) {
            if (framingMode == WindowExpression.FRAMING_RANGE) {
                ...
                if (windowContext.isDefaultFrame() && (!windowContext.isOrdered() || windowContext.getRowsHi() == Long.MAX_VALUE)) {
                    ...
                    // moving average over range between date - rowsLo and date + rowsHi (inclusive)
            ...
            if (framingMode == WindowExpression.FRAMING_RANGE) {
                ...
                if (!windowContext.isOrdered() && windowContext.isDefaultFrame()) {
                    ...
                    // first_value() ignore nulls over range between date - rowsLo and date + rowsHi (inclusive)
                ...
                if (rowsLo == Long.MIN_VALUE && (rowsHi == 0 || rowsHi == Long.MAX_VALUE)) {
                    ...
                    return new FirstValueDateWindowFunctionFactory.FirstValueOverCurrentRowFunction(args.get(0), true);
     ...
     * @return a Function implementing FIRST_VALUE for date values that respects NULLs
        ...
        if (partitionByRecord != null) {
            ...
            if (framingMode == WindowExpression.FRAMING_RANGE) {
                ...
                if (!windowContext.isOrdered() && windowContext.isDefaultFrame()) {
                    ...
                    // first_value() over range between date - rowsLo and date + rowsHi (inclusive)
    ...
    static class FirstNotNullValueOverPartitionFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
        /**
         * Construct a function that computes the first non-null date value for each partition.
         * <p>
         * The instance stores per-partition state in the provided map and uses the given record/sink
         * to identify partitions. The supplied argument function is used to read the date value
         * for each input record; the function emits the first non-null value observed for a partition.
         *
         * @param arg function that produces the date value for the current record
         */
        ...
        /**
         * First pass: record the first non-null date observed for the current partition.
         * <p>
         * If the partition has no stored value and the argument's date for this record is not
         * NULL, stores that date into the partition map. Existing per-partition values are never
        ...
        /**
         * Emit the stored first-value date for the record's partition into the output column during pass 2.
         * <p>
         * Looks up the partition key in the per-partition map (populated during pass1) and writes the persisted
         * date or SQL NULL (Numbers.LONG_NULL) into the SPI-backed row at the given recordOffset.
         *
         * @param record       the current record used to derive the partition key
         * @param recordOffset byte offset within the SPI row where the date should be written
         */
        @Override
        public void pass2(Record record, long recordOffset, WindowSPI spi) {
            ...
            long val = value != null ? value.getDate(0) : Numbers.LONG_NULL;
    ...
    public static class FirstNotNullValueOverPartitionRangeFrameFunction extends FirstValueOverPartitionRangeFrameFunction {
         ...
         * <p>
         * The instance maintains per-partition buffers of (date, value) pairs and updates the
         ...
         * @param arg               the value-producing function (the column/expression whose date values are tracked)
         ...
         * (based on maxDiff/minDiff relative to the current row's date), optionally expands
         * the ring buffer when full, and appends the current row's [date, value] pair if
         * non-null.
         ...
         *
         * @param record the current input record to process (its date and argument value are used
        ...
        public void computeNext(Record record) {
            ...
            // actual frame data - [date, value] pairs - is stored in mem at [ offset + first_idx*16, offset + last_idx*16]
    ...
    public static class FirstNotNullValueOverPartitionRowsFrameFunction extends FirstValueOverPartitionRowsFrameFunction {
         ...
         * <p>
         * This function computes the first non-null date value within a sliding row-based frame for each partition.
         *
         ...
         * @param arg    the input date argument function supplying values to evaluate for the first non-null entry
         */
         ...
         * ring-buffer state to compute the current first (non-null) date value.
         *
         * <p>This method:
         ...
         * - Updates the buffer with the current row's date argument and computes {@code firstValue}
         ...
         * buffer to find the first non-null date; if none found it may use the current row's value
         * depending on {@code frameIncludesCurrentValue}. The oldest index is advanced and the buffer
         * slot for the evicted position is overwritten with the current date.
         * <p>
         * Side effects:
         * - Mutates map-backed per-partition state (stored longs at specified offsets).
         * - Writes to the MemoryARW buffer for the partition.
         * - Sets the instance field {@code firstValue} to the first date to be emitted for this row
    ...
    public static class FirstNotNullValueOverRangeFrameFunction extends FirstValueOverRangeFrameFunction implements Reopenable, WindowDateFunction {
        /**
         * Constructs a FirstNotNullValueOverRangeFrameFunction for a RANGE-based, non-partitioned
         * window that returns the first non-null date within the moving time window.
         * <p>
         * This function maintains an in-memory ring buffer of (date, value) entries bounded by
         * the provided range offsets and uses the specified argument function to read values.
         *
         * @param rangeLo      lower bound of the range frame (inclusive offset, in the same units as dates)
         * @param rangeHi      upper bound of the range frame (inclusive offset)
         * @param arg          function that produces the date values to be considered
         ...
         * it removes entries that fall outside the [date - maxDiff, date + maxDiff] range,
         ...
         * relative to the current date and locates the first element that satisfies {@code |date - ts| >= minDiff}
         * to set {@code firstValue}.
         * - Retrieves the current row value via {@code arg.getDate(record)}; if non-null, appends it to the ring buffer,
    ...
    // Removable cumulative aggregation.
    public static class FirstNotNullValueOverRowsFrameFunction extends FirstValueOverRowsFrameFunction implements Reopenable, WindowDateFunction {
        private long firstNotNullIdx = -1;

        /**
         * Construct a row-based frame implementation that returns the first non-null date value in the frame.
         *
         * @param arg    function that produces the date value to consider
        ...
        /**
         * Advance the window computation for the given record and update the cached first non-null date.
         *
         * <p>Depending on whether the frame has a bounded lower bound, this method either:
         * <ul>
         *   <li>For unbounded-lower frames: track the first non-null date seen across rows (using
         *       `firstNotNullIdx`, `count` and `buffer`) and set `firstValue` to that date when the
         ...
         *       non-null date, update `firstNotNullIdx` and `firstValue` accordingly, then write the
         *       current row's date into the ring buffer and advance `loIdx`.</li>
         * </ul>
         * <p>
         ...
         *
         * @param record current input record used to read the date argument
         */
        @Override
        public void computeNext(Record record) {
            if (!frameLoBounded) {
    ...
    static class FirstNotNullValueOverUnboundedPartitionRowsFrameFunction extends FirstValueOverUnboundedPartitionRowsFrameFunction {
        /**
         * Create a function that computes the first non-NULL date over an unbounded-rows frame
         ...
         * @param arg               the argument function that produces the date value for the current row
         */
        ...
        /**
         * Advances computation for the current input row and ensures the partition's first date is recorded.
         * <p>
         * If a first value for the record's partition already exists in the map, this sets the function's current
         * value to that stored date. If no value exists, it reads the date from the provided record;
         * if that date is non-null it is stored as the partition's first value and becomes the current value;
         * otherwise the current value is set to SQL NULL (Numbers.LONG_NULL).
         *
         * @param record the input record for the current row; used to determine the partition key and to read the date argument
         */
        @Override
        public void computeNext(Record record) {
            partitionByRecord.of(record);
    ...
    public static class FirstNotNullValueOverWholeResultSetFunction extends FirstValueOverWholeResultSetFunction {
        ...
        /**
         * Create a window function that computes the first non-null date value over the entire result set.
         *
         ...
         * capture the record's date as the first value (if it is not SQL NULL).
         *
         * <p>This is a no-op if a value has already been found. If the record's date
         * equals Numbers.LONG_NULL it is treated as NULL and ignored.</p>
         *
         * @param record the current row record to inspect for a date value
         */
        @Override
        public void computeNext(Record record) {
            if (!found) {
        ...
        /**
         * Pass 1: inspect the current record and capture the first non-null date from the argument.
         * <p>
         * If a non-null date is found and no value has been recorded yet, stores it in {@code this.value}
         * and marks {@code this.found} true. Null dates (Numbers.LONG_NULL) are ignored.
         */
        @Override
        public void pass1(Record record, long recordOffset, WindowSPI spi) {
            if (!found) {
        ...
        /**
         * Write the stored date value into the output column for the current row during pass 2.
         * <p>
         ...
         * date value to the sentinel LONG_NULL.
         */
        @Override
        public void reset() {
            super.reset();
         ...
         * and setting the cached date to {@link Numbers#LONG_NULL}.
         */
        @Override
        public void toTop() {
            super.toTop();
    ...
    static class FirstValueOverCurrentRowFunction extends BaseWindowFunction implements WindowDateFunction {
        private final boolean ignoreNulls;
        private long value;

        /**
         * Constructs a first_value implementation that returns the argument's date for the current row.
         * <p>
         ...
         *
         * @param arg         the input date expression evaluated for the current row
        ...
        /**
         * Reads the date from the provided record's argument and stores it as the current value.
         *
         * @param record the input record (current row) from which the argument date is retrieved
         */
        @Override
        public void computeNext(Record record) {
        ...
        /**
         * Return the cached date value for this function.
         * <p>
         * The input record is ignored; this function always returns the stored first-value
         * date for the current window state.
         *
         * @param rec ignored
         ...
         * computed date value into the SPI output column for that record.
         * <p>
         * This method updates internal state by invoking {@code computeNext(record)} and
         * then stores the resulting long date value into the SPI memory at
    ...
    // order by is absent so default frame mode includes all rows in the partition
    static class FirstValueOverPartitionFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
        private long firstValue;

        /**
         * Create a partitioned FirstValue window function instance.
         * <p>
         * The function records and returns the first (earliest) date value seen for each partition.
         *
         ...
         * @param arg               the argument function that produces the date value to be tracked
         */
         ...
         * date for the record's partition.
         * <p>
         * If the partition is seen for the first time, reads the date from the argument
        ...
        public void computeNext(Record record) {
            ...
            if (value.isNew()) {
                firstValue = arg.getDate(record);
                value.putLong(0, firstValue);
            } else {
                firstValue = value.getDate(0);
            }
        }
        ...
        /**
         * Return the cached first value's date for the current window frame.
         * <p>
         * The supplied Record parameter is not consulted; this method always returns
         * the stored `firstValue`.
         *
         * @param rec ignored
         * @return the first-value date as a long
         */
        @Override
        public long getDate(Record rec) {
            return firstValue;
         ...
         * date into the window SPI output column.
         * <p>
         * This method updates the function's state for `record` (via {@code computeNext})
         * and stores the resulting date value into the SPI at the provided record slot
    ...
    // Removable cumulative aggregation with date & value stored in resizable ring buffers
    public static class FirstValueOverPartitionRangeFrameFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
         ...
         * includes the current row's date (when rangeHi == 0).
         *
         ...
         * @param timestampIdx      column index of the ordering date within the input record (used to compare/expire entries)
         */
         ...
         * - Appends the current record's date/value pair to the ring buffer, expanding
    ...
    // removable cumulative aggregation
    public static class FirstValueOverPartitionRowsFrameFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
         ...
         *       hold up to {@code bufferSize} date slots (initialized to NULL).</li>
         ...
         *   <li>Advances buffer metadata, writes the current row's date into the ring buffer,
         *       and updates the instance field {@code firstValue} with the computed first-date
         ...
         *
         * @param record current input record used to derive the partition key and date value
         */
        @Override
        public void computeNext(Record record) {
            // map stores:
    ...
    public static class FirstValueOverRangeFrameFunction extends BaseWindowFunction implements Reopenable, WindowDateFunction {
        ...
        /**
         * Constructs a range-framed first_value implementation for date windows.
         * <p>
         ...
         * @param arg           the argument function that produces the date value for each record
         * @param configuration runtime configuration used to size the buffer (not documented as a service)
         * @param timestampIdx  index of the date column within stored records used for range comparisons
         */
         ...
         *
         * <p>Processes the provided record's date to:
         ...
         * - Append the current (date, value) pair into the ring buffer (the value may be null).
         ...
         *
         * @param record the current input row used to advance the window frame; its date is read
         *               using the function's configured timestampIndex and the associated argument
         *               function (arg.getDate(record))
         */
        @Override
        public void computeNext(Record record) {
            if (!frameLoBounded && frameSize > 0) {
    ...
    // Removable cumulative aggregation.
    public static class FirstValueOverRowsFrameFunction extends BaseWindowFunction implements Reopenable, WindowDateFunction {
         ...
         *
         * <p>The method reads the record's date and appends it into an internal circular buffer,
         ...
         * date becomes the first value.
         * - Otherwise the cached first value is set to {@code LONG_NULL}.
         *
         * <p>Side effects: writes the date into {@code buffer}, advances {@code loIdx}, updates
    ...
    static class FirstValueOverUnboundedPartitionRowsFrameFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
         ...
         * produces the first (earliest) date value per partition, respecting NULL values.
         *
         ...
         * @param arg               argument function that produces the date value for each row
         */
         ...
         * look up (or create) the per-partition map entry, and ensure the partition's first date
         * value is recorded.
         * <p>
         * If the partition entry is new, the method reads the date from `arg` for the current
         * record, stores it into the map entry and updates the instance `value` field. If the entry
         * already exists, it loads the stored date into `value` without modifying the map.
         *
         * @param record the input record whose partition key and candidate date are processed
         */
        @Override
        public void computeNext(Record record) {
            partitionByRecord.of(record);
    ...
    public static class FirstValueOverWholeResultSetFunction extends BaseWindowFunction implements WindowDateFunction {
         ...
         *
         * @param arg function that produces the date values to evaluate
         */
        ...
        /****
         * Capture the first date value from the supplied record.
         *
         * If a value has not yet been recorded for this function instance, reads the date
         * from the provided record using the configured argument function and stores it;
         * subsequent calls do nothing.
         *
         * @param record the current input record to read the date from
         */
        @Override
        public void computeNext(Record record) {
            if (!found) {
                this.value = arg.getDate(record);
                this.found = true;
            }
        }
        ...
        /**
         * Returns the cached date value representing the first value for the current context.
         * <p>
         * The provided Record parameter is ignored; the method always returns the stored date.
         *
         * @param rec (ignored) record passed by the caller
         * @return the stored date value (milliseconds since epoch)
         */
        @Override
        public long getDate(Record rec) {
            return this.value;
    ...
    static {
        ...
        FIRST_VALUE_COLUMN_TYPES.add(ColumnType.DATE);
    }
}
