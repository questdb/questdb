...
public class ContainsIPv4FunctionFactory implements FunctionFactory {
    ...
    private Function createHalfConstantFunc(Function varFunc, Function constFunc) throws SqlException {
        ...
        if(subnet == -2 && netmask == -2) { //catches negative netmasks
            throw SqlException.$(18, "invalid argument: ").put(constValue);
        }
        else if(subnet == -2 || netmask == -2) { // If true, the argument is either invalid OR is a subnet instead of an ip address (-2 used as sentinel because 0xffffffff (which is valid) is -1)
    ...
    public static class NullCheckFunc extends NegatableBooleanFunction implements UnaryFunction {
        ...
        @Override
        public boolean getBool(Record rec) { return negated != (arg.getStrLen(rec) == -1L); }
