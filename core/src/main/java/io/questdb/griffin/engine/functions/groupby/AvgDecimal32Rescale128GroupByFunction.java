...
class AvgDecimal32Rescale128GroupByFunction extends Decimal128Function implements GroupByFunction, UnaryFunction {
    ...
    public AvgDecimal32Rescale128GroupByFunction(@NotNull Function arg, int position, int targetType) {
        super(targetType);
        this.arg = arg;
    ...
    public short getDecimal16(Record rec) {
        if (calc(rec)) {
            return (short) decimal128A.getLow();
        }
    ...
    public byte getDecimal8(Record rec) {
        if (calc(rec)) {
            return (byte) decimal128A.getLow();
        }
    ...
    private boolean calc(Record rec) {
        ...
        if (count > 0) {
            try {
                ...
                decimal128A.setScale(ColumnType.getDecimalScale(arg.getType()));
