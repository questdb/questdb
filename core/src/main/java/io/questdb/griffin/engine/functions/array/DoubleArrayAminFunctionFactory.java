...
public class DoubleArrayAminFunctionFactory implements FunctionFactory {
    ...
    public Function newInstance(
        ...
        if (args == null || args.size() < 2) {
            throw SqlException.$(position, "amin requires at least 2 arguments");
        }
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    if (Numbers.isFinite(val)) {
                        ...
                        if (Numbers.isFinite(cur)) {
                            arrayOut.putDouble(i, Math.min(cur, val));
                        } else {
