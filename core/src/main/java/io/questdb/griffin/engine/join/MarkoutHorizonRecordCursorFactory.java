...
public class MarkoutHorizonRecordCursorFactory extends AbstractJoinRecordCursorFactory {
    ...
    public RecordCursor getCursor(SqlExecutionContext executionContext) throws SqlException {
        ...
        try {
            // hasNext() round-robins the active iterators, but they form a contiguous,
            // forward-sliding window of master rows whose width is the offset grid (small
            // relative to a row group), so at frame granularity the recordAt() access is
            // sequential with a tiny working set: monotonic, not scattered.
