...
public class SumUInt32GroupByFunction extends LongFunction implements GroupByFunction, UnaryFunction {
    private final Function arg;
    private int valueIndex;

    public SumUInt32GroupByFunction(@NotNull Function arg) {
        this.arg = arg;
    }
    ...
    public void computeNext(MapValue mapValue, Record record, long rowId) {
        if (!arg.isNull(record)) {
            ...
            if (sum != Numbers.LONG_NULL) {
                mapValue.putLong(valueIndex, sum + arg.getLong(record));
            } else {
                mapValue.putLong(valueIndex, arg.getLong(record));
            }
        }
