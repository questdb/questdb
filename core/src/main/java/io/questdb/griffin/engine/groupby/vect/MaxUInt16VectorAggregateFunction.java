...
public class MaxUInt16VectorAggregateFunction extends UInt16Function implements VectorAggregateFunction {
    ...
    public MaxUInt16VectorAggregateFunction(int keyKind, int columnIndex, int timestampIndex, int workerCount) {
    ...
    public void aggregate(long address, long frameRowCount, int workerId) {
        ...
        // Vect.maxShort() uses signed comparison which is incorrect for unsigned values,
    ...
    public void aggregate(long address, long bitmapAddr, long bitOffset, long frameRowCount, int workerId) {
        if (address != 0) {
            if (bitmapAddr != 0) {
                final long value = (long) Vect.maxUInt16BitmapNull(address, bitmapAddr, bitOffset, frameRowCount);
