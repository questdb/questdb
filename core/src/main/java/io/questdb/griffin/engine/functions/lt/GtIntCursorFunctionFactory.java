 ...
 * providing exactly one column and (conceptually) one row. The comparison is performed at the width
 * of the cursor scalar: an {@code int} comparison for narrow integer scalars, a {@code long}
 * comparison for {@code long} scalars (so a wide scalar is never narrowed to int) and a
 * {@code double} comparison for {@code float}/{@code double} scalars.
 * <p>
...
public class GtIntCursorFunctionFactory extends AbstractIntCursorFunctionFactory {
    ...
    protected Function newIntFunc(RecordCursorFactory factory, Function leftFunc, Function rightFunc, int cursorTag, int rightPos) {
        return new GtIntCursorFunction(factory, leftFunc, rightFunc, cursorTag, rightPos);
    }
