...
public class LastNotNullValueDoubleWindowFunctionFactory extends AbstractWindowFunctionFactory {
        ...
        if (partitionByRecord != null) {
            if (framingMode == WindowColumn.FRAMING_RANGE) {
                ...
                if (windowContext.isDefaultFrame() && (!windowContext.isOrdered() || windowContext.getRowsHi() == Long.MAX_VALUE)) {
                    Map map = MapFactory.createUnorderedMap(
                            configuration,
                            partitionByKeyTypes,
                            LastValueDoubleWindowFunctionFactory.LAST_VALUE_COLUMN_TYPES
                    );
            ...
            if (framingMode == WindowColumn.FRAMING_RANGE) {
                ...
                if (!windowContext.isOrdered() && windowContext.isDefaultFrame()) {
                    ...
                    // last_not_null_value() over range between timestamp - rowsLo and timestamp + rowsHi (inclusive)
    ...
    static class LastNotNullValueOverCurrentRowFunction extends BaseDoubleWindowFunction {
    ...
    static class LastNotNullValueOverPartitionFunction extends BasePartitionedDoubleWindowFunction {
    ...
    // Handles last_value() over (partition by x order by ts range between y preceding and [z preceding | current row])
    // Removable cumulative aggregation with timestamp & value stored in resizable ring buffers
    public static class LastNotNullValueOverPartitionRangeFrameFunction extends LastValueDoubleWindowFunctionFactory.LastValueOverPartitionRangeFrameFunction {
    ...
    public static class LastNotNullValueOverPartitionRowsFrameFunction extends BasePartitionedDoubleWindowFunction {
    ...
    // Handles last_not_null_value() over ([order by ts] range between x preceding and [ y preceding | current row ] ); no partition by key
    public static class LastNotNullValueOverRangeFrameFunction extends LastValueDoubleWindowFunctionFactory.LastValueOverRangeFrameFunction {
    ...
    // Handles last_not_null_value() over ([order by o] rows between y and z); there's no partition by.
    // Removable cumulative aggregation.
    public static class LastNotNullValueOverRowsFrameFunction extends BaseDoubleWindowFunction implements Reopenable {
        private final int bufferSize;
        private final boolean frameIncludesCurrentValue;
        private final boolean frameLoBounded;
        private final int frameSize;
        ...
        public void computeNext(Record record) {
            ...
            if (Numbers.isFinite(d) && frameIncludesCurrentValue) {
                ...
                double last = buffer.getDouble((loIdx + frameSize - 1) % bufferSize * Double.BYTES);
                if (Numbers.isFinite(last)) {
                    this.lastValue = last;
                } else if (!Numbers.isFinite(this.lastValue)) {
                    for (int i = frameSize - 2; 0 <= i; i--) {
                        double v = buffer.getDouble((loIdx + i) % bufferSize * Double.BYTES);
                        if (Numbers.isFinite(v)) {
                            this.lastValue = v;
                            break;
                        }
            ...
            // set lastValue as invalid
            if (frameLoBounded && buffer.getDouble(loIdx % bufferSize * Double.BYTES) == this.lastValue) {
    ...
    // last_not_null_value() over () - empty clause, no partition by no order by
    public static class LastNotNullValueOverWholeResultSetFunction extends BaseDoubleWindowFunction {
