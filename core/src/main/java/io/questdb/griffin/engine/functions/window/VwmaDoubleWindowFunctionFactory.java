...
public class VwmaDoubleWindowFunctionFactory implements FunctionFactory {
    ...
    public static final ArrayColumnTypes VWMA_OVER_PARTITION_RANGE_COLUMN_TYPES;
        ...
        if (partitionByRecord != null) {
                ...
                if (windowContext.isDefaultFrame() && (!windowContext.isOrdered() || windowContext.getRowsHi() == Long.MAX_VALUE)) {
                    ...
                    try {
                        map = MapFactory.createOrderedMap(
                                configuration,
                                partitionByKeyTypes,
                                VWMA_OVER_PARTITION_RANGE_COLUMN_TYPES
                        );
                        ...
                        return new VwmaDoubleWindowFunctionFactory.AvgOverPartitionRangeFrameFunction(
                                map,
                                partitionByRecord,
                                partitionBySink,
                                rowsLo,
                ...
                //between [unbounded | x] preceding and [x preceding | current row]
                    ...
                    try {
                        map = MapFactory.createOrderedMap(
                                configuration,
                                partitionByKeyTypes,
                                VWMA_OVER_PARTITION_ROWS_COLUMN_TYPES
                        );
                        mem = Vm.getARWInstance(
                                configuration.getSqlWindowStorePageSize(),
                                configuration.getSqlWindowStoreMaxPages(),
                        ...
                        return new AvgDoubleWindowFunctionFactory.AvgOverPartitionRowsFrameFunction(
                                map,
                                partitionByRecord,
                                partitionBySink,
                                rowsLo,
            ...
            if (framingMode == WindowColumn.FRAMING_RANGE) {
                ...
                if (!windowContext.isOrdered() && windowContext.isDefaultFrame()) {
                    return new VwmaDoubleWindowFunctionFactory.AvgOverWholeResultSetFunction(args.get(0));
                } // between unbounded preceding and current row
                else if (rowsLo == Long.MIN_VALUE && rowsHi == 0) {
                    // same as for rows because calculation stops at current rows even if there are 'equal' following rows
                    return new VwmaDoubleWindowFunctionFactory.AvgOverUnboundedRowsFrameFunction(args.get(0));
                    ...
                    // moving average over range between timestamp - rowsLo and timestamp + rowsHi (inclusive)
                    return new VwmaDoubleWindowFunctionFactory.AvgOverRangeFrameFunction(
                            rowsLo,
                            rowsHi,
                            args.get(0),
                            configuration,
                ...
                if (rowsLo == Long.MIN_VALUE && rowsHi == 0) {
                    ...
                    return new VwmaDoubleWindowFunctionFactory.AvgOverRowsFrameFunction(
                            args.get(0),
                            rowsLo,
                            rowsHi,
                            mem
    ...
    static class VwmaOverCurrentRowFunction extends BinaryDoubleWindowFunction {

        private double price;
        private double volume;

        VwmaOverCurrentRowFunction(Function price, Function volume) {
            super(price, volume);
        }

        @Override
        public void computeNext(Record record) {
            price = arg1.getDouble(record);
            volume = arg2.getDouble(record);
        }
        ...
        public void pass1(Record record, long recordOffset, WindowSPI spi) {
            computeNext(record);
            Unsafe.getUnsafe().putDouble(spi.getAddress(recordOffset, columnIndex), price * volume);
        }
    }

    // handles avg() over (partition by x)
    // order by is absent so default frame mode includes all rows in partition
    static class VwmaOverPartitionFunction extends BinaryPartitionedDoubleWindowFunction {

        public VwmaOverPartitionFunction(Map map, VirtualRecord partitionByRecord, RecordSink partitionBySink, Function price, Function volume) {
    ...
    static {
        ...
        VWMA_OVER_PARTITION_RANGE_COLUMN_TYPES = new ArrayColumnTypes();
        VWMA_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.DOUBLE);// current frame sum
        VWMA_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.LONG);  // number of (non-null) values in current frame
        VWMA_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.LONG);  // native array start offset, requires updating on resize
        VWMA_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.LONG);   // native buffer size
        VWMA_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.LONG);   // native buffer capacity
        VWMA_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.LONG);   // index of first buffered element
