...
public class AlterOperation extends AbstractOperation implements Mutable {
    ...
    public final static short SET_WAL_APPLY_REORDER_WINDOW = SET_TABLE_FORMAT + 1; // 28
    ...
    public void authorize() {
        ...
        switch (command) {
            ...
            case SET_PARAM_MAX_UNCOMMITTED_ROWS, SET_PARAM_COMMIT_LAG, SET_TTL, SET_WAL_APPLY_REORDER_WINDOW ->
    ...
    public boolean isStructural() {
        return switch (command) {
            case ADD_COLUMN, RENAME_COLUMN, DROP_COLUMN, RENAME_TABLE, SET_DEDUP_DISABLE, SET_DEDUP_ENABLE,
                 CHANGE_COLUMN_TYPE, SET_WAL_APPLY_REORDER_WINDOW -> true;
            default -> false;
        };
    }
    ...
    private void applyWalApplyReorderWindow(MetadataService svc) {
        svc.setMetaWalApplyReorderWindow(extraInfo.get(0));
    }
    ...
    private void doApply(MetadataService svc, boolean contextAllowsAnyStructureChanges) {
        switch (command) {
            ...
            case SET_PARAM_COMMIT_LAG:
                ...
                break;
            case SET_WAL_APPLY_REORDER_WINDOW:
                applyWalApplyReorderWindow(svc);
                break;
            case SET_TTL:
                applyTtl(svc);
                break;
