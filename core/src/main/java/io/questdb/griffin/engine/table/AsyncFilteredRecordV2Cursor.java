...
class AsyncFilteredRecordV2Cursor implements RecordCursor {

    private static final Log LOG = LogFactory.getLog(AsyncFilteredRecordV2Cursor.class);
    private final Function filter;
    private final boolean hasDescendingOrder;
    private final ColumnChunkRecord record;
    ...
    public AsyncFilteredRecordV2Cursor(Function filter, int scanDirection) {
        this.filter = filter;
        this.hasDescendingOrder = scanDirection == RecordCursorFactory.SCAN_DIRECTION_BACKWARD;
        record = new ColumnChunkRecord();
    }
        ...
        try {
            do {
                ...
                if (cursor > -1) {
                    ...
                    frameRowCount = task.getFilteredRows().size();
                    frameIndex = task.getFrameIndex();
                    frameRowIndex = 0;
                    if (frameRowCount > 0 && frameSequence.isActive()) {
                        record.setColumnChunks(task.getColumnChunks());
                        break;
                    } else {
