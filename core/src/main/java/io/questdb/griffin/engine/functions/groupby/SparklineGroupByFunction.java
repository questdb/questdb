...
public class SparklineGroupByFunction extends VarcharFunction implements UnaryFunction, GroupByFunction {
    ...
    private long renderForPtr(long ptr, long descPtr, long descCount, int size) {
        ...
        if (Double.isNaN(userMin) || Double.isNaN(userMax)) {
            ...
            // Override a computed bound only when the user supplied that
            // bound (a NaN userMin/userMax means "auto"). This decision is
            // about the optional bound arguments, never about the data
