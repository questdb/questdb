...
public class CountGeoHashGroupByFunctionShort extends AbstractCountGroupByFunction {
    ...
    public CountGeoHashGroupByFunctionShort(@NotNull Function arg) {
        super(arg);
        this.argColumnIndex = GroupByUtils.directArgColumnIndexByTag(arg, ColumnType.GEOSHORT);
    }
    ...
    public void computeBatch(MapValue mapValue, long dataAddr, int rowCount, long startRowId) {
        if (rowCount > 0) {
            ...
            for (; dataAddr < hi; dataAddr += 2) {
                if (Unsafe.getShort(dataAddr) != GeoHashes.SHORT_NULL) {
                    nonNullCount++;
                }
            }
    ...
    public void computeFirst(MapValue mapValue, Record record, long rowId) {
        ...
        if (isArgNotNull || value != GeoHashes.SHORT_NULL) {
    ...
    public void computeKeyedBatch(
        ...
        // Fast path: arg is a direct geoshort column with data on the current frame.
