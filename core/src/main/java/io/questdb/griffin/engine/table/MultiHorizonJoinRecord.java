...
/**
 * Combined record for multi-slave HORIZON JOIN that maps column indices to source records.
 * Each row combines data from:
 * - Master record (from page frame)
 * - Sequence offset value (from long_sequence)
 * - N slave records (from ASOF JOIN with each slave table)
 * <p>
 * Slave source IDs start at {@link #SOURCE_SLAVE_BASE} and are indexed as
 * SOURCE_SLAVE_BASE + slaveIndex.
 */
public class MultiHorizonJoinRecord implements Record {
