...
public class StdDevPopDoubleWindowFunctionFactory extends AbstractWindowFunctionFactory {

    private static final String NAME = "stddev_pop";
    private static final String SIGNATURE = NAME + "(D)";
    private static final ArrayColumnTypes STDDEV_POP_COLUMN_TYPES;
    private static final ArrayColumnTypes STDDEV_POP_OVER_PARTITION_RANGE_COLUMN_TYPES;
    private static final ArrayColumnTypes STDDEV_POP_OVER_PARTITION_ROWS_COLUMN_TYPES;
        ...
        if (partitionByRecord != null) {
            if (framingMode == WindowExpression.FRAMING_RANGE) {
                ...
                if (windowContext.isDefaultFrame() && (!windowContext.isOrdered() || windowContext.getRowsHi() == Long.MAX_VALUE)) {
                    Map map = MapFactory.createUnorderedMap(
                            configuration,
                            partitionByKeyTypes,
                            STDDEV_POP_COLUMN_TYPES
                    );

                    return new StdDevPopOverPartitionFunction(
                            map,
                            partitionByRecord,
                            partitionBySink,
                            args.get(0)
                    ...
                    return new StdDevPopOverUnboundedPartitionRowsFrameFunction(
                            map,
                            partitionByRecord,
                            partitionBySink,
                            args.get(0)
                    ...
                    try {
                        map = MapFactory.createUnorderedMap(
                                configuration,
                                partitionByKeyTypes,
                                STDDEV_POP_OVER_PARTITION_RANGE_COLUMN_TYPES
                        );
                        ...
                        return new StdDevPopOverPartitionRangeFrameFunction(
                                map,
                                partitionByRecord,
                                partitionBySink,
                                rowsLo,
                ...
                if (rowsLo == Long.MIN_VALUE && rowsHi == 0) {
                    ...
                    return new StdDevPopOverCurrentRowFunction(args.get(0));
                } // whole partition
                ...
                // between [unbounded | x] preceding and [x preceding | current row]
                    ...
                    try {
                        map = MapFactory.createUnorderedMap(
                                configuration,
                                partitionByKeyTypes,
                                STDDEV_POP_OVER_PARTITION_ROWS_COLUMN_TYPES
                        );
                        mem = Vm.getCARWInstance(
                                configuration.getSqlWindowStorePageSize(),
                                configuration.getSqlWindowStoreMaxPages(),
                        ...
                        return new StdDevPopOverPartitionRowsFrameFunction(
                                map,
                                partitionByRecord,
                                partitionBySink,
                                rowsLo,
            ...
            if (framingMode == WindowExpression.FRAMING_RANGE) {
                ...
                if (!windowContext.isOrdered() && windowContext.isDefaultFrame()) {
                    return new StdDevPopOverWholeResultSetFunction(args.get(0));
                } // between unbounded preceding and current row
                else if (rowsLo == Long.MIN_VALUE && rowsHi == 0) {
                    return new StdDevPopOverUnboundedRowsFrameFunction(args.get(0));
                } // range between [unbounded | x] preceding and [x preceding | current row]
                else {
                    if (windowContext.isOrdered() && !windowContext.isOrderedByDesignatedTimestamp()) {
                        throw SqlException.$(windowContext.getOrderByPos(), "RANGE is supported only for queries ordered by designated timestamp");
                    ...
                    return new StdDevPopOverRangeFrameFunction(
                            rowsLo,
                            rowsHi,
                            args.get(0),
                            configuration,
                ...
                if (rowsLo == Long.MIN_VALUE && rowsHi == 0) {
                    ...
                    return new StdDevPopOverRowsFrameFunction(
                            args.get(0),
                            rowsLo,
                            rowsHi,
                            mem
    ...
    private static double stddevPop(double sum, double sumSq, long count) {
        if (count <= 0) {
            return Double.NaN;
        }
    ...
    // (rows between current row and current row) processes a 1-element set, so finite value maps to 0.
    static class StdDevPopOverCurrentRowFunction extends BaseWindowFunction implements WindowDoubleFunction {
        private double value;

        StdDevPopOverCurrentRowFunction(Function arg) {
            super(arg);
        }
        ...
        public void computeNext(Record record) {
            ...
            value = Numbers.isFinite(d) ? 0.0 : Double.NaN;
        }
    ...
    // Handles stddev_pop() over (partition by x), order by absent or whole-partition frame.
    static class StdDevPopOverPartitionFunction extends BasePartitionedWindowFunction implements WindowDoubleFunction {

        StdDevPopOverPartitionFunction(Map map, VirtualRecord partitionByRecord, RecordSink partitionBySink, Function arg) {
        ...
        public void preparePass2() {
            ...
            while (cursor.hasNext()) {
                ...
                value.putDouble(0, stddevPop(sum, sumSq, count));
            }
        }
    }

    // Handles stddev_pop() over (partition by x order by ts range between [unbounded | y] preceding and [z preceding | current row]).
    // Removable cumulative aggregation with timestamp & value stored in resizable ring buffers.
    static class StdDevPopOverPartitionRangeFrameFunction extends BasePartitionedWindowFunction implements WindowDoubleFunction {
        ...
        StdDevPopOverPartitionRangeFrameFunction(
                Map map,
        ...
        public void computeNext(Record record) {
            ...
            if (mapValue.isNew()) {
                ...
                stddev = stddevPop(sum, sumSq, count);
            }
    ...
    // Handles stddev_pop() over (partition by x [order by o] rows between y and z).
    // Removable cumulative aggregation.
    static class StdDevPopOverPartitionRowsFrameFunction extends BasePartitionedWindowFunction implements WindowDoubleFunction {
        ...
        StdDevPopOverPartitionRowsFrameFunction(
                Map map,
                VirtualRecord partitionByRecord,
                RecordSink partitionBySink,
                long rowsLo,
    ...
    // Handles stddev_pop() over ([order by ts] range between [unbounded | x] preceding and [x preceding | current row]); no partition key.
    static class StdDevPopOverRangeFrameFunction extends BaseWindowFunction implements Reopenable, WindowDoubleFunction {
        private static final int RECORD_SIZE = Long.BYTES + Double.BYTES;
        private final boolean frameLoBounded;
        ...
        StdDevPopOverRangeFrameFunction(
                long rangeLo,
                long rangeHi,
                Function arg,
                CairoConfiguration configuration,
    ...
    // Handles stddev_pop() over ([order by o] rows between y and z); no partition key.
    // Removable cumulative aggregation.
    static class StdDevPopOverRowsFrameFunction extends BaseWindowFunction implements Reopenable, WindowDoubleFunction {
        ...
        StdDevPopOverRowsFrameFunction(Function arg, long rowsLo, long rowsHi, MemoryARW memory) {
            super(arg);
    ...
    // Handles:
    // - stddev_pop(a) over (partition by x rows between unbounded preceding and current row)
    // - stddev_pop(a) over (partition by x order by ts range between unbounded preceding and current row)
    // Doesn't require value buffering.
    static class StdDevPopOverUnboundedPartitionRowsFrameFunction extends BasePartitionedWindowFunction implements WindowDoubleFunction {
        private double stddev;

        StdDevPopOverUnboundedPartitionRowsFrameFunction(Map map, VirtualRecord partitionByRecord, RecordSink partitionBySink, Function arg) {
            super(map, partitionByRecord, partitionBySink, arg);
        }
    ...
    // Handles stddev_pop() over (rows between unbounded preceding and current row); no partition key.
    static class StdDevPopOverUnboundedRowsFrameFunction extends BaseWindowFunction implements WindowDoubleFunction {
        private long count = 0;
        private double stddev = Double.NaN;
        private double sum = 0.0;
        private double sumSq = 0.0;

        StdDevPopOverUnboundedRowsFrameFunction(Function arg) {
            super(arg);
        }
    ...
    // stddev_pop() over () - empty clause, no partition by, no order by, default frame.
    static class StdDevPopOverWholeResultSetFunction extends BaseWindowFunction implements WindowDoubleFunction {
        private long count;
        private double stddev;
        private double sum;
        private double sumSq;

        StdDevPopOverWholeResultSetFunction(Function arg) {
            super(arg);
        }
    ...
    static {
        STDDEV_POP_COLUMN_TYPES = new ArrayColumnTypes();
        STDDEV_POP_COLUMN_TYPES.add(ColumnType.DOUBLE); // sum (pass1), stddev result (pass2)
        STDDEV_POP_COLUMN_TYPES.add(ColumnType.DOUBLE); // sumSq
        STDDEV_POP_COLUMN_TYPES.add(ColumnType.LONG);   // count

        STDDEV_POP_OVER_PARTITION_ROWS_COLUMN_TYPES = new ArrayColumnTypes();
        STDDEV_POP_OVER_PARTITION_ROWS_COLUMN_TYPES.add(ColumnType.DOUBLE); // sum
        STDDEV_POP_OVER_PARTITION_ROWS_COLUMN_TYPES.add(ColumnType.DOUBLE); // sumSq
        STDDEV_POP_OVER_PARTITION_ROWS_COLUMN_TYPES.add(ColumnType.LONG);   // finite count in frame
        STDDEV_POP_OVER_PARTITION_ROWS_COLUMN_TYPES.add(ColumnType.LONG);   // lo index
        STDDEV_POP_OVER_PARTITION_ROWS_COLUMN_TYPES.add(ColumnType.LONG);   // start offset

        STDDEV_POP_OVER_PARTITION_RANGE_COLUMN_TYPES = new ArrayColumnTypes();
        STDDEV_POP_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.DOUBLE); // sum
        STDDEV_POP_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.DOUBLE); // sumSq
        STDDEV_POP_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.LONG);   // finite count in frame
        STDDEV_POP_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.LONG);   // start offset
        STDDEV_POP_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.LONG);   // ring size
        STDDEV_POP_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.LONG);   // ring capacity
        STDDEV_POP_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.LONG);   // first index
    }
}
