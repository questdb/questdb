...
public class MinShortGroupByFunction extends IntFunction implements GroupByFunction, UnaryFunction {
    private final Function arg;
    private final int argColumnIndex;
    private int valueIndex;

    public MinShortGroupByFunction(@NotNull Function arg) {
        this.arg = arg;
    ...
    public void computeBatch(MapValue mapValue, long dataAddr, int rowCount, long startRowId) {
        if (rowCount > 0) {
            // Vect.minShort returns the int min of native short values; SHORT has no NULL.
            final int batchMin = Vect.minShort(dataAddr, rowCount);
            final int existing = mapValue.getInt(valueIndex);
            if (existing == Numbers.INT_NULL || batchMin < existing) {
    ...
    public void computeNext(MapValue mapValue, Record record, long rowId) {
        mapValue.minInt(valueIndex, arg.getShort(record));
    }
