...
public class MinShortGroupByFunction extends IntFunction implements GroupByFunction, UnaryFunction {
    ...
    public void computeNext(MapValue mapValue, Record record, long rowId) {
        if (!arg.isNull(record)) {
            ...
            final int cur = mapValue.getInt(valueIndex);
            if (cur == Numbers.INT_NULL || val < cur) {
                mapValue.putInt(valueIndex, val);
            }
        }
    }
    ...
    public void merge(MapValue destValue, MapValue srcValue) {
        final int srcMin = srcValue.getInt(valueIndex);
        final int destMin = destValue.getInt(valueIndex);
