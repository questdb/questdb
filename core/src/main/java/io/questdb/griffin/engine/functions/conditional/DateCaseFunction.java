...
class DateCaseFunction extends DateFunction {
    ...
    public DateCaseFunction(int position, ObjList<Function> args, Function elseBranch) {
        super(position);
        this.args = args;
    ...
    public long getDate(Record rec) {
        for (int i = 0; i < argsLen; i += 2) {
            if (args.getQuick(i).getBool(rec)) {
                return args.getQuick(i + 1).getDate(rec);
            }
        }
        return elseBranch == null ? Numbers.LONG_NaN : elseBranch.getDate(rec);
    }
}
