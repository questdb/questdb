...
class SumDecimal64GroupByFunction extends Decimal128Function implements GroupByFunction, UnaryFunction {
    ...
    public void computeNext(MapValue mapValue, Record record, long rowId) {
        ...
        if (decimal64A != Decimals.DECIMAL64_NULL) {
            try {
                if (!overflow) {
                    long decimal64B = mapValue.getDecimal64(valueIndex + 1);
                    if (decimal64B == Decimals.DECIMAL64_NULL) {
                        ...
                        try {
                            ...
                            overflow = true;
                            decimal128B.ofRaw(0, 0);
                            Decimal128.uncheckedAdd(decimal128B, decimal64B);
                            Decimal128.uncheckedAdd(decimal128B, decimal64A);
                            mapValue.putDecimal128(valueIndex, decimal128B);
                            mapValue.putBool(valueIndex + 2, overflow);
                        }
                    }
                } else {
    ...
    public void merge(MapValue destValue, MapValue srcValue) {
        ...
        if (!srcOverflow && !destOverflow) {
            ...
            if (!destNull && !srcNull) {
                // both not null
                try {
                    destValue.putLong(valueIndex + 1, Decimal64.addSameScaleNoMaxValueCheck(decimal64A, decimal64B));
                } catch (NumericException e) {
                    // overflow
            ...
            boolean destNull = decimal128B.isNull();
