...
public class LastValueDateWindowFunctionFactory extends AbstractWindowFunctionFactory {
     ...
     *
     * @return the function signature (e.g. "last_value(M)")
     */
    @Override
     ...
     * - returns a DateNullFunction when the configured frame is empty (rowsHi &lt; rowsLo),
     ...
     * @param args                function arguments (the first argument is the date expression)
     ...
     * DateNullFunction when the frame is empty)
     ...
     * @param args          argument list for the function (expects the date argument at index 0)
        ...
        if (partitionByRecord != null) {
            if (framingMode == WindowExpression.FRAMING_RANGE) {
                ...
                if (windowContext.isDefaultFrame() && (!windowContext.isOrdered() || windowContext.getRowsHi() == Long.MAX_VALUE)) {
                    ...
                    // moving last over range between date - rowsLo and date + rowsHi (inclusive)
            ...
            if (framingMode == WindowExpression.FRAMING_RANGE) {
                ...
                if (!windowContext.isOrdered() && windowContext.isDefaultFrame()) {
                    ...
                    // last_value() ignore nulls over range between date - rowsLo and date + rowsHi (inclusive)
     ...
     * @param args     list of function arguments (expects a single date argument at index 0)
        ...
        if (partitionByRecord != null) {
            ...
            if (framingMode == WindowExpression.FRAMING_RANGE) {
                ...
                if (windowContext.isDefaultFrame() && (!windowContext.isOrdered() || windowContext.getRowsHi() == Long.MAX_VALUE)) {
                    ...
                    // last_value() over range between date - rowsLo and date + rowsHi (inclusive)
    ...
    public static class LastNotNullOverUnboundedRowsFrameFunction extends BaseWindowFunction implements WindowDateFunction {
         ...
         *
         * @param arg the function that produces the date values to be considered
         */
        ...
        /**
         * Process a single input record and update the stored last date.
         * <p>
         * If the function argument's date for the given record is not NULL,
         * updates the internal lastValue to that date. NULL dates are ignored.
         *
         * @param record input record from which the date is read
         */
        @Override
        ...
        /**
         * Returns the current last_value date computed for the window.
         * <p>
         * The provided record is ignored; this method returns the internally tracked
         * last date value for the current frame/partition.
         *
         * @param rec the record passed by the caller (ignored)
         * @return the last seen date value for the current window
         */
        @Override
         ...
         * current {@code lastValue} (stored as a long date) into the memory address for the row
         ...
         * <p>
         * Calls the superclass reset then clears the cached last date by setting
    ...
    static class LastNotNullValueOverCurrentRowFunction extends BaseWindowFunction implements WindowDateFunction {
        private long value = Numbers.LONG_NULL;

        /**
         * Creates a window function that returns the last non-null date value for the current row frame.
         * <p>
         * This implementation ignores NULLs and yields the argument's date for the current row (used for
         * frames equivalent to `CURRENT ROW`).
         *
         * @param arg the function providing the date value evaluated for each row
         */
        ...
        /**
         * Reads the date value from the given record and stores it as the next last-value.
         *
        ...
        /**
         * Returns the last tracked date value.
         * <p>
         * The supplied record is ignored; this function returns the internally stored date.
         *
        ...
        /**
         * Advance the function's state for the given input record and write the resulting date
         ...
         * the computed date into the output column slot identified by recordOffset.
         *
    ...
    static class LastNotNullValueOverPartitionFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
        /**
         * Creates a partition-aware implementation of `last_value(...)` that ignores NULLs.
         *
         * @param arg the input date expression whose last non-NULL value will be tracked per partition
         */
        ...
        /**
         * First-pass handler that records the last non-null date for the current partition if not already present.
         * <p>
         * For the given input record this method:
         * - materializes the partition key from `record`,
         * - looks up the partition in `map`,
         * - if the partition has no stored value and the argument date is not null, stores that date as the partition's last value.
         * <p>
         * This method does not overwrite an existing per-partition value and ignores null dates.
         */
        @Override
        public void pass1(Record record, long recordOffset, WindowSPI spi) {
         ...
         * <p>
         * Looks up the current partition's stored date in the per-partition map and writes it
         ...
         * partitionBySink, and writes the resulting long date directly to the output memory
    ...
    public static class LastNotNullValueOverPartitionRangeFrameFunction extends LastValueOverPartitionRangeFrameFunction {
         ...
         * @param arg               argument function that produces the date value to track
         ...
         * @param timestampIdx      index of the ordering date column within stored entries
         */
         ...
         * - appends the current record (date and value) into the ring buffer when the argument value
         ...
         * partition's buffer is: consecutive pairs of 8-byte date and 8-byte value (record size = 16
         ...
         *               partition key, the ordering date and the argument value)
         */
        @Override
        public void computeNext(Record record) {
            // map stores
    ...
    public static class LastNotNullValueOverPartitionRowsFrameFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
        //number of values we need to keep to compute over frame
        private final int bufferSize;
        private final boolean frameIncludesCurrentValue;
        private final boolean frameLoBounded;
        private final int frameSize;
        // holds fixed-size ring buffers of date values
        private final MemoryARW memory;
        private long lastValue = Numbers.LONG_NULL;

        /**
         * Creates a partitioned rows-frame function that computes the last non-null date within a ROWS frame.
         *
         ...
         * @param arg               the argument function that produces the date value to be considered
         ...
         * "last value" date into the partition map and underlying ring buffer.
         *
         ...
         * - reads the current row date from the argument function and updates the in-memory ring buffer;
         * - computes the last non-null date visible in the current frame according to
         ...
         *
         * <p>Side effects: updates the provided partition {@code MapValue} (date, lo index, start offset)
         * and writes the current row date into {@code memory} at the partition's ring-buffer slot.</p>
         *
         * @param record the input record whose partition and date are processed for the next frame step
         */
        @Override
        public void computeNext(Record record) {
            partitionByRecord.of(record);
            ...
            if (value.isNew()) {
                ...
                this.lastValue = value.getDate(0);
         ...
         * cached last date value so the function starts fresh for the next evaluation.
         */
        @Override
        public void toTop() {
            super.toTop();
    ...
    public static class LastNotNullValueOverRangeFrameFunction extends LastValueOverRangeFrameFunction {
         ...
         * @param arg           function producing the date value to evaluate
         * @param configuration Cairo configuration used for memory sizing and limits
         * @param timestampIdx  index of the ordering date within the input record (used to compare range bounds)
         */
         ...
         *       current record date.</li>
         *   <li>Appends the current record's argument date/value pair to the ring buffer if the
         ...
         * </ul>
         *
         * <p>Buffer layout: entries are stored as consecutive pairs of longs (date, value). The
         ...
         * accordingly.
         *
         * @param record input record whose date and argument are used to advance the frame
         */
        @Override
        public void computeNext(Record record) {
            long newFirstIdx = firstIdx;
    ...
    // Removable cumulative aggregation.
    public static class LastNotNullValueOverRowsFrameFunction extends BaseWindowFunction implements Reopenable, WindowDateFunction {
        // holds fixed-size ring buffers of date values
         ...
         * ring buffer of recent date values.
         * <p>
         ...
         *
         * @param arg    the input function that produces date values for each row
         ...
         * @param memory preallocated MemoryARW used as the ring buffer storage for dates
         */
         ...
         *
         * <p>This reads the date from {@code arg} for the supplied {@code record}, updates
         * internal state used to produce last_value (including {@code lastValue} and {@code cacheValue}),
         * pushes the read date into the circular {@code buffer} at the current {@code loIdx},
         * and advances {@code loIdx} by one.</p>
         *
         * <p>Behavior summary:
         * - If the current row's date is non-null and {@code frameIncludesCurrentValue} is true,
         * the current date becomes the new {@code lastValue}.
         ...
         * to find the most recent non-null date and use it as {@code lastValue}.
         ...
         * date is written into the buffer at {@code loIdx} and {@code loIdx} is incremented
         * modulo {@code bufferSize}.</p>
         *
         * @param record the current input record whose argument date is used to update the window state
         */
        @Override
        public void computeNext(Record record) {
            long d = arg.getDate(record);
         ...
         * <p>
         * Sets the cached last date to NULL, resets the lowest buffer index and
         ...
         * date values (setting them to NULL), and resets the low index pointer.
         */
        @Override
        public void reset() {
            super.reset();
    ...
    public static class LastNotNullValueOverUnboundedPartitionRowsFrameFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
        private long value = Numbers.LONG_NULL;
         ...
         * @param arg               function that produces the date value evaluated by this window function
         */
        ...
        /**
         * Compute and store the last non-null date for the partition of the supplied record.
         * <p>
         * Looks up (or creates) the partition entry in the map using partitionByRecord and partitionBySink,
         * then reads the argument date for the current record:
         * - If this is a newly created partition entry, the date value (including LONG_NULL) is stored
         * into the map and assigned to the instance field `value`.
         * - If the partition entry already exists and the argument date is not LONG_NULL, the map is
         * updated with the new date and `value` is set to it.
         * - If the partition entry exists and the argument date is LONG_NULL, the previously stored
         * date from the map is assigned to `value` (no map update).
         * <p>
    ...
    public static class LastNotNullValueOverWholeResultSetFunction extends BaseWindowFunction implements WindowDateFunction {

        private boolean found;
        private long value = Numbers.LONG_NULL;

        /**
         * Creates a LastNotNullValueOverWholeResultSetFunction that computes the last non-null date
         * across the entire result set (no partitioning, no ordering).
         *
         * @param arg source date function whose non-null values are considered for the last_value result
         */
        ...
        /**
         * First pass handler that captures the first non-null date from the input stream.
         * <p>
         * When invoked, if no value has been found yet, reads the date from {@code arg} for
         * the given {@code record}. If the date is not SQL NULL, stores it as the function's
        ...
        /**
         * Writes the function's current date value into the output column for the given row.
         * <p>
         * This method computes the destination address for the output column using the provided
         * WindowSPI and recordOffset, then stores the long date value directly into memory.
         *
         ...
         * <p>
         * Calls super.reset() and clears the stored date and found flag so the
         ...
         * sets the stored date to `LONG_NULL` and clears the `found` flag so
    ...
    public static class LastValueIncludeCurrentFrameFunction extends BaseWindowFunction implements WindowDateFunction {
         ...
         * @param arg     the argument function that provides the date values this window function evaluates
         */
         ...
         * <p>
         * This clears any stored last-date value and delegates common reset work
         * to the superclass.
         */
        @Override
        public void reset() {
         ...
         * <p>
         * Delegates general reset logic to the superclass and clears the stored date value
    ...
    public static class LastValueIncludeCurrentPartitionRowsFrameFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
        private final boolean isRange;
        private final long rowsLo;
        private long value = Numbers.LONG_NULL;
         ...
         * @param arg               the argument function producing the date values evaluated by last_value
         */
    ...
    // order by is absent so default frame mode includes all rows in the partition
    static class LastValueOverPartitionFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
        /**
         * Create a partitioned implementation of `last_value(TIMESTAMP)` that keeps the most
         * recent date value for each partition.
         *
         * @param arg function that produces the date value to track (the argument to `last_value`)
         */
        ...
        /**
         * Process a single input record for the first pass: update per-partition last-non-null date
         * and write the current partition's last date to the output column.
         * <p>
         * For the record's partition key this method either creates a new map entry with the current
         * argument date (if none existed) or reads the existing stored date. The chosen
         * date is then written to the output address computed from recordOffset and columnIndex.
         *
         * @param record       the input record being processed
         * @param recordOffset output memory offset for the current record where the result date is written
    ...
    // Removable cumulative aggregation with date & value stored in resizable ring buffers
    public static class LastValueOverPartitionRangeFrameFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
         ...
         * @param timestampIdx      index of the ordering date column within the function's record layout
         */
         ...
         * date/value pair, maintaining frame bounds and updating the partition's last non-null value.
         * <p>
         * This method:
         ...
         * current record's `[date, value]` pair.
         ...
         *
         * @param record the current input record whose partition key, date and value are processed
         */
        @Override
        public void computeNext(Record record) {
            // map stores
         ...
         * <p>
         * Calls the superclass reopen() and clears the stored last date (sets it to LONG_NULL)
         ...
         * and clears the cached last date value.</p>
         */
        @Override
        public void toTop() {
            super.toTop();
    ...
    // removable cumulative aggregation
    public static class LastValueOverPartitionRowsFrameFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
        //number of values we need to keep to compute over frame
         ...
         *   <li>writes the current argument date into the buffer slot that was just read.</li>
         * </ul>
         *
         ...
         *
         * @param record source record whose partition key and date argument are used
         */
        @Override
        public void computeNext(Record record) {
            // map stores:
         ...
         * <p>
         * Calls {@code super.reopen()} and resets the cached last date to {@code Numbers.LONG_NULL}
    ...
    public static class LastValueOverRangeFrameFunction extends BaseWindowFunction implements Reopenable, WindowDateFunction {
        protected static final int RECORD_SIZE = Long.BYTES + Long.BYTES;
        protected final boolean frameLoBounded;
         ...
         * @param arg           argument function that produces the date values
         * @param configuration Cairo configuration used to determine page size and max pages for the buffer
         * @param timestampIdx  index of the date column within stored records (used by range logic)
         */
         ...
         * <p>This method:
         * <ul>
         *   <li>Reads the current row date and argument value from {@code record}.</li>
         *   <li>Evicts buffered entries whose date is outside the allowed range relative to the current
         *       date (greater than {@code maxDiff} or handled by {@code minDiff} semantics).</li>
         ...
         *       then appends the current [date, value] pair into the buffer and updates {@code size},
         ...
         *
         * @param record the current input record from which the date and value are read
         */
        @Override
        public void computeNext(Record record) {
            long timestamp = record.getTimestamp(timestampIndex);
    ...
    // Removable cumulative aggregation.
    public static class LastValueOverRowsFrameFunction extends BaseWindowFunction implements Reopenable, WindowDateFunction {
        private final MemoryARW buffer;
        private final int bufferSize;
        private final long rowsLo;
        private long lastValue = Numbers.LONG_NULL;
         ...
         * <p>
         * Reads the date currently at the buffer position indicated by {@code loIdx}
         * into the field {@code lastValue}, overwrites that slot with the date
         * produced by {@code arg.getDate(record)}, and advances {@code loIdx}
         * (wrapping around by {@code bufferSize}).
         *
         * @param record the current input record whose date is written into the buffer
         */
        @Override
        public void computeNext(Record record) {
            lastValue = buffer.getLong((long) loIdx * Long.BYTES);
            buffer.putLong((long) loIdx * Long.BYTES, arg.getDate(record));
            loIdx = (loIdx + 1) % bufferSize;
        }
         ...
         * <p>
         * Clears the stored last date, resets the low index used by the ring buffer,
    ...
    public static class LastValueOverWholeResultSetFunction extends BaseWindowFunction implements WindowDateFunction {
        private boolean found;
        private long value = Numbers.LONG_NULL;
        ...
        /**
         * Captures the first non-null date returned by the argument and writes that stored value
         * into the output memory for the given record offset. On the very first call this reads the
         * date from {@code arg} and stores it; on every call it writes the stored date to
    ...
    static {
        ...
        LAST_VALUE_COLUMN_TYPES.add(ColumnType.DATE);
        ...
        LAST_NOT_NULL_VALUE_PARTITION_ROWS_COLUMN_TYPES.add(ColumnType.DATE);
