...
public class MaxDecimalWindowFunctionFactory extends AbstractWindowFunctionFactory {
    ...
    public static final ArrayColumnTypes MAX_DECIMAL128_OVER_PARTITION_RANGE_TYPES;
    public static final ArrayColumnTypes MAX_DECIMAL128_OVER_PARTITION_ROWS_TYPES;
    public static final ArrayColumnTypes MAX_DECIMAL128_TYPES;
    public static final ArrayColumnTypes MAX_DECIMAL256_OVER_PARTITION_RANGE_TYPES;
    public static final ArrayColumnTypes MAX_DECIMAL256_OVER_PARTITION_ROWS_TYPES;
    public static final ArrayColumnTypes MAX_DECIMAL256_TYPES;
    public static final ArrayColumnTypes MAX_DECIMAL64_OVER_PARTITION_RANGE_BOUNDED_TYPES;
    public static final ArrayColumnTypes MAX_DECIMAL64_OVER_PARTITION_RANGE_TYPES;
    public static final ArrayColumnTypes MAX_DECIMAL64_OVER_PARTITION_ROWS_BOUNDED_TYPES;
    ...
    public Function newInstance(
        ...
        return newMaxMinInstance(this, position, args, configuration, sqlExecutionContext, GREATER_THAN, NAME);
    }
    ...
    static Function newMaxMinInstance(
        ...
        if (tag == ColumnType.DECIMAL8) {
            return newMaxMinInstanceDecimal8(position, args, configuration, sqlExecutionContext, comparator, name);
        }
        if (tag == ColumnType.DECIMAL16) {
            return newMaxMinInstanceDecimal16(position, args, configuration, sqlExecutionContext, comparator, name);
        }
        ...
        public void computeNext(Record record) {
            ...
            if (value.isNew()) {
                ...
                if (frameIncludesCurrentValue) {
                    ...
                    memory.getDecimal128(startOffset + ((loIdx + frameSize - 1) % bufferSize) * 16L, hiValue);
                }
                if (frameLoBounded) {
                    dequeStartOffset = value.getLong(2);
                    dequeStartIndex = value.getLong(3);
            ...
            if (frameLoBounded) {
                ...
                for (long i = frameSize, n = size; i < n; i++) {
                    ...
                    if (diff <= maxDiff && diff >= minDiff) {
                        memory.getDecimal128(startOffset + idx * RECORD_SIZE + Long.BYTES, value);
                        while (dequeStartIndex != dequeEndIndex) {
                            dequeMemory.getDecimal128(dequeStartOffset + ((dequeEndIndex - 1) % dequeCapacity) * 16L, dequeBack);
                            if (isBetter(value, dequeBack)) {
                                dequeEndIndex--;
                            } else {
                                break;
                        ...
                        if (dequeEndIndex - dequeStartIndex == dequeCapacity) {
                            long newAddress = dequeMemory.appendAddressFor((dequeCapacity << 1) * 16L);
                            long oldAddress = dequeMemory.getPageAddress(0) + dequeStartOffset;
                            if (dequeStartIndex == 0) {
                                Vect.memcpy(newAddress, oldAddress, dequeCapacity * 16L);
                            } else {
                                dequeStartIndex %= dequeCapacity;
                                long firstPieceSize = (dequeCapacity - dequeStartIndex) * 16L;
                                Vect.memcpy(newAddress, oldAddress + dequeStartIndex * 16L, firstPieceSize);
                                Vect.memcpy(newAddress + firstPieceSize, oldAddress, dequeStartIndex * 16L);
                                dequeStartIndex = 0;
                            }
                            dequeStartOffset = newAddress - dequeMemory.getPageAddress(0);
                            dequeEndIndex = dequeStartIndex + dequeCapacity;
                ...
                if (dequeStartIndex == dequeEndIndex) {
                    ...
                    dequeMemory.getDecimal128(dequeStartOffset + (dequeStartIndex % dequeCapacity) * 16L, maxMin);
                }
            } else {
                newFirstIdx = firstIdx;
                Decimal128 val = new Decimal128();
    ...
    public static class Decimal128MaxMinOverRowsFrameFunction extends BaseWindowFunction implements Reopenable, WindowDecimal128Function {
        ...
        public void computeNext(Record record) {
            ...
            if (frameLoBounded && !frameIncludesCurrentValue) {
                ...
                buffer.getDecimal128((long) (loIdx % bufferSize) * 16L, hiValue);
            } else {
                hiValue.copyFrom(scratch);
            }
            if (frameLoBounded) {
        ...
        public void computeNext(Record record) {
            ...
            if (value.isNew()) {
                ...
                if (frameIncludesCurrentValue) {
                    ...
                    memory.getDecimal256(startOffset + ((loIdx + frameSize - 1) % bufferSize) * 32L, hiValue);
                }
                if (frameLoBounded) {
                    dequeStartOffset = value.getLong(2);
                    dequeStartIndex = value.getLong(3);
            ...
            if (frameLoBounded) {
                ...
                for (long i = frameSize, n = size; i < n; i++) {
                    ...
                    if (diff <= maxDiff && diff >= minDiff) {
                        memory.getDecimal256(startOffset + idx * RECORD_SIZE + Long.BYTES, value);
                        while (dequeStartIndex != dequeEndIndex) {
                            dequeMemory.getDecimal256(dequeStartOffset + ((dequeEndIndex - 1) % dequeCapacity) * 32L, dequeBack);
                            if (isBetter(value, dequeBack)) {
                                dequeEndIndex--;
                            } else {
                                break;
                        ...
                        if (dequeEndIndex - dequeStartIndex == dequeCapacity) {
                            long newAddress = dequeMemory.appendAddressFor((dequeCapacity << 1) * 32L);
                            long oldAddress = dequeMemory.getPageAddress(0) + dequeStartOffset;
                            if (dequeStartIndex == 0) {
                                Vect.memcpy(newAddress, oldAddress, dequeCapacity * 32L);
                            } else {
                                dequeStartIndex %= dequeCapacity;
                                long firstPieceSize = (dequeCapacity - dequeStartIndex) * 32L;
                                Vect.memcpy(newAddress, oldAddress + dequeStartIndex * 32L, firstPieceSize);
                                Vect.memcpy(newAddress + firstPieceSize, oldAddress, dequeStartIndex * 32L);
                                dequeStartIndex = 0;
                            }
                            dequeStartOffset = newAddress - dequeMemory.getPageAddress(0);
                            dequeEndIndex = dequeStartIndex + dequeCapacity;
                ...
                if (dequeStartIndex == dequeEndIndex) {
                    ...
                    dequeMemory.getDecimal256(dequeStartOffset + (dequeStartIndex % dequeCapacity) * 32L, maxMin);
                }
            } else {
                newFirstIdx = firstIdx;
                Decimal256 val = new Decimal256();
    ...
    public static class Decimal256MaxMinOverRowsFrameFunction extends BaseWindowFunction implements Reopenable, WindowDecimal256Function {
        ...
        public void computeNext(Record record) {
            ...
            if (frameLoBounded && !frameIncludesCurrentValue) {
                ...
                buffer.getDecimal256((long) (loIdx % bufferSize) * 32L, hiValue);
            } else {
                hiValue.copyRaw(scratch);
            }
            if (frameLoBounded) {
