...
final class StringLongTuplesRecordCursor implements RecordCursor {
    ...
    private class TableWriterMetricsRecord implements Record {
        ...
        public CharSequence getStr(int col) {
            if (col != 0) {
                throw CairoException.instance(-1).put("unsupported string column number [column=").put(col).put("]");
            }
            return keys[pos];
        }
        ...
        public long getLong(int col) {
            if (col != 1) {
                throw CairoException.instance(-1).put("unsupported long column number [column=").put(col).put("]");
            }
            return values[pos];
        }
    }
