...
public class EqSymFunctionFactory implements FunctionFactory {
    ...
    public Function newInstance(
        ...
        if (getStaticSymbolTable(leftFunc) != null && getStaticSymbolTable(rightFunc) != null) {
    ...
    private static StaticSymbolTable getStaticSymbolTable(Function symFunc) {
        return ((SymbolFunction) symFunc).getStaticSymbolTable();
    }

    private static class Func extends AbstractEqBinaryFunction {
        private final StaticSymbolTable leftTable;
        private final StaticSymbolTable rightTable;

        public Func(Function left, Function right) {
            super(left, right);
            this.leftTable = getStaticSymbolTable(left);
            this.rightTable = getStaticSymbolTable(right);
        }
