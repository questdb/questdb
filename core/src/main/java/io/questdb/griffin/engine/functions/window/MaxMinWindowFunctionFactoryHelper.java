...
public class MaxMinWindowFunctionFactoryHelper {
    ...
    // Live-view value layout for the unbounded shape: the max/min value plus a
    // trailing tombstone BYTE for anchor-driven frontier compaction.
    ...
    static Function newInstance(
            ...
            UnboundedPartitionConstructor overUnboundedPartitionRowsConstructor,
    ...
    // Constructor for the unbounded-preceding-to-current-row partitioned shape. Unlike the
    // whole-partition PartitionConstructor it carries the live-view snapshot args
    // (partitionByKeyTypes/liveView) plus the configuration needed to allocate the frontier
    // compaction scratch map.
    @FunctionalInterface
    ...
    abstract static class MaxMinOverPartitionRangeFrameBase extends BasePartitionedWindowFunction {
        ...
        public void restoreCheckpointRingState(LiveViewCheckpointRingStateSource source, MapValue value) {
            ...
            for (long i = 0; i < frameSize; i++) {
                ...
                while (dequeEndIndex > 0
                        && comparator.compare(v, dequeMemory.getLong(newDequeStartOffset + (dequeEndIndex - 1) * DEQUE_RECORD_SIZE))) {
                    dequeEndIndex--;
                }
        ...
        public void freezeCheckpointRingState(LiveViewCheckpointRingStateSink sink, MapValue value) {
            ...
            // the same pages. The scalar slot is unused (0): the emitted max/min is the
            // deque front, which restore recomputes, and frameSize carries the in-frame
            // count the deque reconstruction replays.
         ...
         * value column is a raw 64-bit word - the argument's long/date/timestamp
         * payload; max/min skip NULL rows, so a stored value is never LONG_NULL.
         */
    ...
    abstract static class MaxMinOverPartitionRowsFrameBase extends BasePartitionedWindowFunction {
        ...
        public void close() {
            super.close();
            memory.close();
            // The live-view close path runs close() without reset(), so the deque
            // slab must be freed here too (matching reset()); otherwise it leaks.
            if (dequeMemory != null) {
                dequeMemory.close();
            }
        }
        ...
        public void resetPartition(Record record) {
            ...
            if (value != null) {
                ...
                if (frameLoBounded) {
                    ...
                    value.putTimestamp(2, Numbers.LONG_NULL);
                }
                if (!value.isNew() && tombstoneValueIndex >= 0 && value.getByte(tombstoneValueIndex) != 1) {
                    value.putByte(tombstoneValueIndex, (byte) 1);
                    tombstoneCount++;
