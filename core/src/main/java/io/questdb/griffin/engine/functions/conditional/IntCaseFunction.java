...
class IntCaseFunction extends IntFunction {
    ...
    public IntCaseFunction(int position, ObjList<Function> args, Function elseBranch) {
        super(position);
        this.args = args;
    ...
    public int getInt(Record rec) {
        for (int i = 0; i < argsLen; i += 2) {
            if (args.getQuick(i).getBool(rec)) {
                return args.getQuick(i + 1).getInt(rec);
            }
        }
        return elseBranch == null ? Numbers.INT_NaN : elseBranch.getInt(rec);
    }
}
