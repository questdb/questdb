...
public class MaxDateWindowFunctionFactory extends AbstractWindowFunctionFactory {
    ...
    public static final DateComparator GREATER_THAN = (a, b) -> a > b;
     ...
     *
     * @return the signature string (e.g. "max(M)")
     */
    @Override
    ...
    /**
     * Create a MAX date window function instance appropriate for the current
     * window context.
     * <p>
     ...
     * @param args                function arguments (first argument is the date expression)
            ...
            if (framingMode == WindowExpression.FRAMING_RANGE) {
                ...
                if (windowContext.isDefaultFrame() && (!windowContext.isOrdered() || windowContext.getRowsHi() == Long.MAX_VALUE)) {
                    ...
                    try {
                        ...
                        // moving max over range between date - rowsLo and date + rowsHi (inclusive)
    ...
    static class MaxMinOverCurrentRowFunction extends BaseWindowFunction implements WindowDateFunction {
         ...
         *
         * @param arg  function that produces the date value for the current row
        ...
        /**
         * Read the date value for the current input record and store it as this function's current value.
         *
        ...
        /**
         * Return the last-computed date value for the current row.
         *
         * @param rec ignored; this implementation does not read from the provided record
         * @return the stored date value produced by the most recent computeNext call
         */
        @Override
         ...
         * <p>
         * This evaluates the function for the given input Record, then stores the resulting date
    ...
    static class MaxMinOverPartitionFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
         ...
         * @param comparator comparator used to compare two date values to determine the current aggregate (returns true when the first argument should replace the second)
        ...
        /**
         * First-pass accumulator that updates the per-partition maximum date.
         * <p>
         * For the given input record, reads the date from the argument function and, if not null,
         * looks up the partition key in the map and updates the stored date to the greater value
         * according to the provided comparator. New partition entries are initialized with the record's
         * date. Null dates are ignored.
         *
         * @param record       input record to read the date and partition key from
        ...
        public void pass1(Record record, long recordOffset, WindowSPI spi) {
            ...
            if (l != Numbers.LONG_NULL) {
                ...
                if (!value.isNew()) {
                    if (comparator.compare(l, value.getDate(0))) {
                        value.putDate(0, l);
                    }
                } else {
                    value.putDate(0, l);
        ...
        /**
         * Second pass: materializes the precomputed per-partition maximum date into the result.
         * <p>
         * Looks up the partition key for the current input record, reads the stored max date
         ...
         * @param recordOffset offset passed to WindowSPI to obtain the output address where the date is written
         */
        @Override
    ...
    public static class MaxMinOverPartitionRangeFrameFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
         ...
         * @param arg               argument function that produces the input dates
         ...
         * @param timestampIdx      column index within the frame entries that holds the date value
         * @param comparator        comparator used to choose the maximum date (non-boxing)
         ...
         * partitioned maximum date/value (stored in this.maxMin).
         *
         * <p>This method:
         ...
         * - Appends the incoming (date, value) pair when the value is not null.
         ...
         *
         * @param record the input record whose partition key, ordering date, and value are used
        ...
        public void computeNext(Record record) {
            ...
            if (mapValue.isNew()) {
                ...
                if (frameLoBounded) {
                    ...
                    long oldMax = mapValue.getDate(5);
            ...
            if (frameLoBounded) {
                ...
                mapValue.putDate(5, this.maxMin);
            }
        }
        ...
        /**
         * Returns the current computed maximum date for this window function.
         * <p>
         * The provided Record argument is not used by this implementation; the result is taken
         * from the function's internal state.
         *
         * @param rec ignored
         * @return the current max date stored in this function's state
         */
        @Override
        public long getDate(Record rec) {
            return maxMin;
    ...
    public static class MaxMinOverPartitionRowsFrameFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
         ...
         * @param arg         value-producing function for each row (date source)
         ...
         * @param comparator  comparator used to decide max/min between two dates
        ...
        /**
         * Advances the sliding-window state for the current input record and computes the windowed maximum date.
         *
         ...
         * - For existing partitions it pushes the new date into the ring buffer, updates or maintains the monotonic
         ...
         *
         * <p>Behavior notes:
         * - Null dates (Numbers.LONG_NULL) are kept in the ring buffer but ignored for max computation.
         ...
         *
         * @param record the input record whose date is pushed into the window and used to update per-partition state
         */
        @Override
        public void computeNext(Record record) {
            // map stores:
         ...
         * date value into the output row at the address returned by spi.getAddress(recordOffset, columnIndex).
         *
    ...
    // However, when the lower bound is not unbounded, we need a monotonically deque to maintain the history of records.
    public static class MaxMinOverRangeFrameFunction extends BaseWindowFunction implements Reopenable, WindowDateFunction {
        private static final int RECORD_SIZE = Long.BYTES + Long.BYTES;
        private final DateComparator comparator;
        private final boolean frameLoBounded;
         ...
         * <p>
         * Parameters:
         * - rangeLo, rangeHi: inclusive offsets (relative to the row date) that define the
         * window frame. `rangeLo == Long.MIN_VALUE` indicates an unbounded preceding lower bound.
         * - arg: the argument function that produces dates for each input row.
         * - configuration: provides SQL runtime configuration (used to derive the initial buffer size).
         * - memory: primary ring-buffer memory region for storing date/value records.
         * - dequeMemory: optional memory region used to store deque indices when the lower bound is bounded.
         * - timestampIdx: index of the date column within stored records.
         * - comparator: non-boxing comparator used to decide the max between two dates.
         ...
         *
         * <p>This method reads the record's date and the argument function's date value,
         ...
         *
         * <p>Notes:
         * - Null argument dates (equal to `Numbers.LONG_NULL`) are not added to the buffers.
    ...
    // Removable cumulative aggregation.
    public static class MaxMinOverRowsFrameFunction extends BaseWindowFunction implements Reopenable, WindowDateFunction {
         ...
         * (and an optional monotonic deque) to compute the maximum date over a rows-based frame.
         *
         ...
         *
         * @param arg         function producing the date values to aggregate
         ...
         * @param comparator  comparator used to decide the max between two dates
        ...
        /**
         * Advance the sliding window by one row: ingest the date from the provided record,
         ...
         * <ul>
         *   <li>Reads the date via the wrapped argument function (arg.getDate(record)).</li>
         ...
         *       that is updated when encountering larger dates.</li>
         ...
         *   <li>Ignores null dates (Numbers.LONG_NULL) when updating maxima or deque.</li>
         *   <li>Evicts the oldest element from the deque when it leaves the frame.</li>
         *   <li>Overwrites the oldest slot in the circular buffer with the new date and
         *       advances loIdx.</li>
         * </ul>
         *
         * @param record the input record whose date will be added to the window
         */
        @Override
        public void computeNext(Record record) {
            long l = arg.getDate(record);
         ...
         * and reset indices and cached maximum date to null sentinel.
         *
        ...
        /**
         * Initialize the ring buffer by setting each slot to the sentinel null date.
         * <p>
    ...
    static class MaxMinOverUnboundedPartitionRowsFrameFunction extends BasePartitionedWindowFunction implements WindowDateFunction {
        private final DateComparator comparator;
        private final String name;
        private long maxMin;

        /**
         * Create a window function that computes the maximum date for each partition with an unbounded
         ...
         * @param arg               function that produces the date value for the current row
         * @param comparator        comparator used to decide which date is greater
         * @param name              function name used in plan/output
         */
        ...
        /**
         * Process the given record and update the per-partition maximum date.
         * <p>
         * Advances the partition key using the supplied record, reads the date from
         ...
         * Sets {@code this.maxMin} to the partition's current maximum date after
         ...
         *               and the date to consider for the partition's maximum
         */
        @Override
        public void computeNext(Record record) {
            partitionByRecord.of(record);
            ...
            if (l != Numbers.LONG_NULL) {
                ...
                MapValue value = key.findValue();
                this.maxMin = value != null ? value.getDate(0) : Numbers.LONG_NULL;
            }
        }
    ...
    public static class MaxMinOverUnboundedRowsFrameFunction extends BaseWindowFunction implements WindowDateFunction {
        private final DateComparator comparator;
        private final String name;
        private long maxMin = Numbers.LONG_NULL;
         ...
         * <p>
         * This constructor builds a function that maintains the maximum date seen so far
         * across all rows (no partitioning, unbounded preceding). The provided `arg` supplies
         * the date value for each row; `comparator` is used to compare dates; `name`
         * is the function name returned by getName().
         *
         * @param arg        the input function that produces a date value for each row
         * @param comparator comparator used to decide which date is greater
         * @param name       the reported function name (e.g., "max")
         */
        public MaxMinOverUnboundedRowsFrameFunction(Function arg, DateComparator comparator, String name) {
            super(arg);
            this.comparator = comparator;
            this.name = name;
        }
        ...
        /**
         * Consume the next input record and update the running maximum date.
         * <p>
         * Reads the date from {@code arg} for the given {@code record}; if the value is not null
         * and is greater (per {@code comparator}) than the current stored value, replaces the stored
         * maximum with that date.
         *
         * @param record input record to read the date from
         */
        @Override
        public void computeNext(Record record) {
            long l = arg.getDate(record);
         ...
         * <p>
         * Clears the accumulated maximum date by setting {@code maxMin} to the null date marker
         ...
         * <p>
         * Clears any accumulated maximum date by setting {@code maxMin} to {@code Numbers.LONG_NULL}
    ...
    static class MaxMinOverWholeResultSetFunction extends BaseWindowFunction implements WindowDateFunction {
        private final DateComparator comparator;
        private final String name;
        private long maxMin = Numbers.LONG_NULL;

        /**
         * Constructs a window function that computes the maximum date across the entire result set.
         *
         * @param arg        the input date-producing function
         * @param comparator comparator used to determine which date is greater
         * @param name       the function name used for labeling/plan output
         */
        public MaxMinOverWholeResultSetFunction(Function arg, DateComparator comparator, String name) {
            super(arg);
            this.comparator = comparator;
            this.name = name;
        }
        ...
        /**
         * Processes a single input row for the first pass: reads the date from the argument
         * function and updates the running maximum (`maxMin`) if the date is non-null and
         * greater according to the provided comparator.
         *
         * @param record       the input record to read the date from
        ...
        /**
         * Writes the computed maximum date for the current partition into the output column for the specified row.
         *
    ...
    static {
        ...
        MAX_COLUMN_TYPES.add(ColumnType.DATE); // max value
        ...
        MAX_OVER_PARTITION_RANGE_COLUMN_TYPES.add(ColumnType.DATE); // max value case when unbounded preceding
        ...
        MAX_OVER_PARTITION_ROWS_COLUMN_TYPES.add(ColumnType.DATE); // max value case when unbounded preceding
