    ...
    public void computeBatch(MapValue mapValue, long dataAddr, int rowCount, long startRowId) {
        if (rowCount > 0) {
            ...
            if (lastRowId > existingRowId || existingRowId == Numbers.LONG_NULL) {
                ...
                mapValue.putShort(valueIndex + 1, Unsafe.getShort(dataAddr + ((long) rowCount - 1) * Short.BYTES));
            }
        }
    }
        ...
        if (argAddr != 0) {
            for (long i = 0; i < rowCount; i++) {
                ...
                if (rowId > Unsafe.getLong(entryBase + rowIdOffset)) {
                    ...
                    Unsafe.putShort(entryBase + valueColumnOffset, Unsafe.getShort(argAddr + (rowIndex << 1)));
                }
            }
        } else {
            for (long i = 0; i < rowCount; i++) {
                ...
                if (rowId > Unsafe.getLong(entryBase + rowIdOffset)) {
                    ...
                    Unsafe.putLong(entryBase + rowIdOffset, rowId);
                    Unsafe.putShort(entryBase + valueColumnOffset, arg.getShort(record));
                }
            }
        }
    }
