    ...
    public void computeBatch(MapValue mapValue, long dataAddr, int rowCount, long startRowId) {
        if (rowCount > 0) {
            ...
            if (lastRowId > existingRowId || existingRowId == Numbers.LONG_NULL) {
                ...
                mapValue.putShort(valueIndex + 1, Unsafe.getUnsafe().getShort(dataAddr + ((long) rowCount - 1) * Short.BYTES));
            }
        }
    }
        ...
        if (argAddr != 0) {
            for (long i = 0; i < rowCount; i++) {
                ...
                if (rowId > Unsafe.getUnsafe().getLong(entryBase + rowIdOffset)) {
                    ...
                    Unsafe.getUnsafe().putShort(entryBase + valueColumnOffset, Unsafe.getUnsafe().getShort(argAddr + (rowIndex << 1)));
                }
            }
        } else {
            for (long i = 0; i < rowCount; i++) {
                ...
                if (rowId > Unsafe.getUnsafe().getLong(entryBase + rowIdOffset)) {
                    ...
                    Unsafe.getUnsafe().putLong(entryBase + rowIdOffset, rowId);
                    Unsafe.getUnsafe().putShort(entryBase + valueColumnOffset, arg.getShort(record));
                }
            }
        }
    }
