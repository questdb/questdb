...
public class LTrimFunctionFactory implements FunctionFactory {
    ...
    public static class TrimFunc extends StrFunction implements UnaryFunction {
        ...
        public CharSequence getStr(final Record rec) {
            return lTrim(getArg().getStr(rec), sink1);
        }

        @Override
        public CharSequence getStrB(final Record rec) {
            return lTrim(getArg().getStr(rec), sink2);
        }

        @Nullable
        private static CharSequence lTrim(CharSequence str, StringSink sink) {
            if (str == null) {
                return null;
            }
            int startIdx = 0;
            final int endIdx = str.length()-1;
