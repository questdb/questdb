...
public class CountGeoHashGroupByFunctionByte extends AbstractCountGroupByFunction {
    ...
    public CountGeoHashGroupByFunctionByte(@NotNull Function arg) {
        super(arg);
        this.argColumnIndex = GroupByUtils.directArgColumnIndexByTag(arg, ColumnType.GEOBYTE);
    }
    ...
    public void computeBatch(MapValue mapValue, long dataAddr, int rowCount, long startRowId) {
        if (rowCount > 0) {
            ...
            for (; dataAddr < hi; dataAddr++) {
                if (Unsafe.getUnsafe().getByte(dataAddr) != GeoHashes.BYTE_NULL) {
                    nonNullCount++;
                }
            }
    ...
    public void computeKeyedBatch(
        ...
        // Fast path: arg is a direct geobyte column with data on the current frame.
