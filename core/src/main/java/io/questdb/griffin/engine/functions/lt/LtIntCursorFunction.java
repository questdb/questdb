...
/**
 * Per-row {@code left < value} comparison of a {@code int}-read left operand against the cached
 * {@code int} cursor scalar. All cold lifecycle behavior (sub-query execution, cardinality
 * enforcement, worker state donation, plan rendering) lives in {@link AbstractScalarCursorFunction}.
 */
class LtIntCursorFunction extends AbstractScalarCursorFunction {
    private final int cursorTag;
    private int value;

    LtIntCursorFunction(RecordCursorFactory factory, Function leftFunc, Function rightFunc, int cursorTag, int rightPos) {
    ...
    protected void donateValueTo(AbstractScalarCursorFunction that) {
        ((LtIntCursorFunction) that).value = value;
    }
    ...
    protected void readValue(Record record) {
        value = ScalarSubQueryUtils.readIntValue(record, cursorTag);
    }
