...
public class RecordToRowCopierUtils {
    ...
    public static void transferVarcharToStrCol(TableWriter.Row row, int col, Utf8Sequence seq) {
        ...
        if (seq instanceof DirectUtf8Sequence ds) {
            row.putStrUtf8(col, ds);
        } else if (seq.isAscii()) {
            row.putStr(col, seq.asAsciiCharSequence());
        } else {
            // For non-DirectUtf8Sequence (e.g., Utf8StringSink from cast functions),
            // convert to CharSequence first
