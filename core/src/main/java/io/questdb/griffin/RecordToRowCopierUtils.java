...
public class RecordToRowCopierUtils {
    ...
    public static void transferVarcharToStrCol(TableWriter.Row row, int col, Utf8Sequence seq) {
        // Every copier caller -- INSERT AS SELECT, CTAS, mat view refresh -- reads a column that
        // already holds these bytes, so malformed UTF-8 copies as null rather than failing the
        // whole statement. Row.putStrUtf8() rejects it instead, which is right only for client
        // writes, so the rejection is caught and downgraded here.
        // Catching keeps the one-pass direct decoder for every valid value; pre-decoding to
        // detect malformed input would cost a scan and a second copy on every row. The throw
        // leaves the column untouched, so putStr() below still writes a clean null.
        try {
    ...
    public static void transferVarcharToSymbolCol(TableWriter.Row row, int col, Utf8Sequence seq) {
        ...
        final CharSequence value = Utf8s.utf8ToUtf16OrView(seq, Misc.getThreadLocalSink());
        if (value != null) {
