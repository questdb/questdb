...
public class RecordToRowCopierUtils {
    ...
    private static final int BASE_BYTECODE_PER_COLUMN = 20;  // aload, iconst, aload, iconst, invokeInterface
    private static final int CONVERSION_BYTECODE_OVERHEAD = 15;
    private static final int COMPLEX_TYPE_OVERHEAD = 60;  // UUID, Decimal, Array types need more bytecode
    ...
    private static int estimateColumnBytecodeSize(int fromType, int toType) {
        ...
        if (fromTag == ColumnType.UUID || toTag == ColumnType.UUID ||
                fromTag == ColumnType.LONG128 || toTag == ColumnType.LONG128 ||
                ColumnType.isDecimal(fromTag) || ColumnType.isDecimal(toTag) ||
                fromTag == ColumnType.ARRAY || toTag == ColumnType.ARRAY) {
    ...
    private static int estimateTotalBytecodeSize(
        ...
        for (int i = 0; i < n; i++) {
            ...
            int fromType = from.getColumnType(i);
            int toType = to.getColumnType(toColumnIndex);
    ...
    public static RecordToRowCopier generateCopier(
        ...
        int estimatedSize = estimateTotalBytecodeSize(from, to, toColumnFilter, timestampIndex);

        // Path 1: Small schema - use single-method approach
        if (estimatedSize <= methodSizeLimit) {
            return generateSingleMethodCopier(asm, from, to, toColumnFilter, methodSizeLimit);
        }

        // Path 2: Large schema with chunking enabled
        if (chunkedEnabled) {
            RecordToRowCopier chunked = generateChunkedCopier(asm, from, to, toColumnFilter, methodSizeLimit);
            if (chunked != null) {
                return chunked;
            }
    ...
    // Backward-compatible overload that defaults to chunked enabled
    public static RecordToRowCopier generateCopier(
            BytecodeAssembler asm,
            ColumnTypes from,
            RecordMetadata to,
        ...
        return generateCopier(asm, from, to, toColumnFilter, methodSizeLimit, true);
    }
    ...
    private static RecordToRowCopier generateChunkedCopier(
        ...
        int timestampIndex = to.getTimestampIndex();
        IntList boundaries = calculateChunkBoundaries(from, to, toColumnFilter, timestampIndex);
        ...
        for (int chunk = 0; chunk < numChunks; chunk++) {
            ...
            for (int i = startCol; i < endCol; i++) {
                ...
                generateColumnCopyBytecode(asm, i, toColumnIndex, toColumnType, fromColumnType,
                        ...
                        rGetSym, rGetStrA, rGetBin, rGetVarchar, rGetArray, rGetDecimal8,
                        rGetDecimal16, rGetDecimal32, rGetDecimal64, rGetDecimal128, rGetDecimal256,
                        wPutInt, wPutIPv4, wPutLong, wPutLong256, wPutLong256Utf8, wPutLong128,
                        wPutUuidStr, wPutUuidUtf8, wPutDate, wPutTimestamp, wPutByte, wPutShort,
                        wPutBool, wPutFloat, wPutDouble, wPutSym, wPutSymChar, wPutStr,
                        wPutGeoStr, wPutGeoVarchar, wPutVarchar, wPutArray, wPutDecimalStr,
                        implicitCastCharAsByte, implicitCastCharAsGeoHash, implicitCastStrAsFloat,
                        implicitCastStrAsDouble, implicitCastStrAsByte, implicitCastStrAsShort,
                        implicitCastStrAsChar, implicitCastStrAsInt, implicitCastStrAsIPv4,
                        implicitCastUtf8StrAsIPv4, implicitCastStrAsLong, implicitCastStrAsLong256,
                        implicitCastStrAsDate, implicitCastStrAsTimestamp, implicitCastShortAsByte,
                        implicitCastIntAsByte, implicitCastLongAsByte, implicitCastFloatAsByte,
                        implicitCastDoubleAsByte, implicitCastIntAsDouble, implicitCastIntAsLong,
                        implicitCastIntAsFloat, implicitCastLongAsFloat, implicitCastLongAsDouble,
                        implicitCastFloatAsDouble, implicitCastVarcharAsLong, implicitCastVarcharAsShort,
                        implicitCastVarcharAsInt, implicitCastVarcharAsByte, implicitCastVarcharAsChar,
                        implicitCastVarcharAsFloat, implicitCastVarcharAsDouble, implicitCastIntAsShort,
                        implicitCastLongAsShort, implicitCastFloatAsShort, implicitCastDoubleAsShort,
                        implicitCastLongAsInt, implicitCastFloatAsInt, implicitCastDoubleAsInt,
                        implicitCastDateAsTimestamp, implicitCastTimestampAsDate, implicitCastTimestampAsTimestamp,
                        implicitCastFloatAsLong, implicitCastDoubleAsLong, implicitCastDoubleAsFloat,
                        wPutStrChar, wPutVarcharChar, wPutChar, wPutBin, implicitCastGeoHashAsGeoHash,
                        transferUuidToStrCol, transferUuidToVarcharCol, transferVarcharToStrCol,
                        transferVarcharToSymbolCol, implicitCastVarcharAsTimestamp, transferVarcharToDateCol,
                        transferStrToVarcharCol, getTimestampDriverRef, validateArrayDimensionsAndTransferColString,
                        validateArrayDimensionsAndTransferColVarchar, transferDecimal8, transferDecimal16,
                        transferDecimal32, transferDecimal64, transferDecimal128, transferDecimal256,
                        transferByteToDecimal, transferShortToDecimal, transferIntToDecimal, transferLongToDecimal);
            }
    ...
    private static void generateColumnCopyBytecode(
            ...
            int timestampTypeRef,
            int toColumnType_0,
            int fromColumnType_0,
            int parserFieldRef,
            int sGetDecimal128, int sGetDecimal256,
            ...
            int rGetSym, int rGetStrA, int rGetBin, int rGetVarchar, int rGetArray, int rGetDecimal8,
            int rGetDecimal16, int rGetDecimal32, int rGetDecimal64, int rGetDecimal128, int rGetDecimal256,
            int wPutInt, int wPutIPv4, int wPutLong, int wPutLong256, int wPutLong256Utf8, int wPutLong128,
            int wPutUuidStr, int wPutUuidUtf8, int wPutDate, int wPutTimestamp, int wPutByte, int wPutShort,
            int wPutBool, int wPutFloat, int wPutDouble, int wPutSym, int wPutSymChar, int wPutStr,
            int wPutGeoStr, int wPutGeoVarchar, int wPutVarchar, int wPutArray, int wPutDecimalStr,
            int implicitCastCharAsByte, int implicitCastCharAsGeoHash, int implicitCastStrAsFloat,
            int implicitCastStrAsDouble, int implicitCastStrAsByte, int implicitCastStrAsShort,
            int implicitCastStrAsChar, int implicitCastStrAsInt, int implicitCastStrAsIPv4,
            int implicitCastUtf8StrAsIPv4, int implicitCastStrAsLong, int implicitCastStrAsLong256,
            int implicitCastStrAsDate, int implicitCastStrAsTimestamp, int implicitCastShortAsByte,
            int implicitCastIntAsByte, int implicitCastLongAsByte, int implicitCastFloatAsByte,
            int implicitCastDoubleAsByte, int implicitCastIntAsDouble, int implicitCastIntAsLong,
            int implicitCastIntAsFloat, int implicitCastLongAsFloat, int implicitCastLongAsDouble,
            int implicitCastFloatAsDouble, int implicitCastVarcharAsLong, int implicitCastVarcharAsShort,
            int implicitCastVarcharAsInt, int implicitCastVarcharAsByte, int implicitCastVarcharAsChar,
            int implicitCastVarcharAsFloat, int implicitCastVarcharAsDouble, int implicitCastIntAsShort,
            int implicitCastLongAsShort, int implicitCastFloatAsShort, int implicitCastDoubleAsShort,
            int implicitCastLongAsInt, int implicitCastFloatAsInt, int implicitCastDoubleAsInt,
            int implicitCastDateAsTimestamp, int implicitCastTimestampAsDate, int implicitCastTimestampAsTimestamp,
            int implicitCastFloatAsLong, int implicitCastDoubleAsLong, int implicitCastDoubleAsFloat,
            int wPutStrChar, int wPutVarcharChar, int wPutChar, int wPutBin, int implicitCastGeoHashAsGeoHash,
            int transferUuidToStrCol, int transferUuidToVarcharCol, int transferVarcharToStrCol,
            int transferVarcharToSymbolCol, int implicitCastVarcharAsTimestamp, int transferVarcharToDateCol,
            int transferStrToVarcharCol, int getTimestampDriverRef, int validateArrayDimensionsAndTransferColString,
            int validateArrayDimensionsAndTransferColVarchar, int transferDecimal8, int transferDecimal16,
            int transferDecimal32, int transferDecimal64, int transferDecimal128, int transferDecimal256,
            int transferByteToDecimal, int transferShortToDecimal, int transferIntToDecimal, int transferLongToDecimal
    ) {
