...
public class RecordToRowCopierUtils {
    private static final int MAX_COLUMN_COUNT_PER_COPIER = 1_000;
    ...
    public static RecordToRowCopier generateCopier(
        ...
        if (n <= MAX_COLUMN_COUNT_PER_COPIER) {
            return generateCopier0(asm, from, to, toColumnFilter, 0, n);
        }

        RecordToRowCopier[] copiers = new RecordToRowCopier[(n + MAX_COLUMN_COUNT_PER_COPIER - 1) / MAX_COLUMN_COUNT_PER_COPIER];
        for (int i = 0, lo = 0; i < copiers.length; i++, lo += MAX_COLUMN_COUNT_PER_COPIER) {
            int hi = Math.min(lo + MAX_COLUMN_COUNT_PER_COPIER, n);
            copiers[i] = generateCopier0(asm, from, to, toColumnFilter, lo, hi);
        }
        return new ChainedRecordToRowCopier(copiers);
    }
    ...
    private static RecordToRowCopier generateCopier0(BytecodeAssembler asm, ColumnTypes from, RecordMetadata to, ColumnFilter toColumnFilter, int lo, int hi) {
        ...
        // if at least one of the index writer indexes is greater than Short.MAX_VALUE
        // then we need to use a constant pool to store the index value :(
        // first, check if a constant pool is required at all
        int toColumnWriterIndex_0 = -1;
        for (int i = lo; i < hi; i++) {
            int toColumnIndex = toColumnFilter.getColumnIndexFactored(i);
            int toColumnWriterIndex = to.getWriterIndex(toColumnIndex);
            if (toColumnWriterIndex > Short.MAX_VALUE) {
                toColumnWriterIndex_0 = asm.getPoolCount();
                break;
            }
        }
        if (toColumnWriterIndex_0 != -1) {
            // ok, we need to pool writer indexes
            for (int i = lo; i < hi; i++) {
                int toColumnIndex = toColumnFilter.getColumnIndexFactored(i);
                int toColumnWriterIndex = to.getWriterIndex(toColumnIndex);
                asm.poolIntConst(toColumnWriterIndex);
            }
        }
        ...
        boolean needsArrayParser = isArrayParserRequired(from, to, toColumnFilter, lo, hi);
        if (needsArrayParser) {
        ...
        for (int i = 0; i < hi - lo; i++) {
            final int toColumnIndex = toColumnFilter.getColumnIndexFactored(i + lo);
            ...
            final int fromColumnType = from.getColumnType(i + lo);
            ...
            if (toColumnTypeTag == ColumnType.ARRAY &&
                ...
                if (toColumnWriterIndex <= Short.MAX_VALUE) {
                    ...
                    assert toColumnWriterIndex_0 != -1;
                    asm.ldc(i + toColumnWriterIndex_0);
                }
                ...
                pushFromColumnIndexToStack(asm, lo, i);
    ...
    private static boolean isArrayParserRequired(ColumnTypes from, RecordMetadata to, ColumnFilter toColumnFilter, int lo, int hi) {
        for (int i = lo; i < hi; i++) {
            int toColumnIndex = toColumnFilter.getColumnIndexFactored(i);
    ...
    private static void pushFromColumnIndexToStack(BytecodeAssembler asm, int lo, int fromColumnIndex_base) {
        asm.iconst(fromColumnIndex_base);
        // stack: [rowWriter, toColumnIndex, record, fromColumnIndex_base]
        if (lo > 0) {
            // this is a chained copier. we need to add 'lo' to 'fromColumnIndex_base'
            if (lo <= Short.MAX_VALUE) {
                // the easy case: lo is less than Short.MAX_VALUE so we can use asm.iconst()
                asm.iconst(lo);
                // Stack: [rowWriter, toColumnIndex, record, fromColumnIndex_base, lo]
            } else {
                // the hard case: lo is greater than Short.MAX_VALUE so we cannot use asm.iconst()
                // we could use a constant pool, but it would be a waste of space
                // so let's use some math to get 'lo' to stack without using a constant pool
                int factor = lo / MAX_COLUMN_COUNT_PER_COPIER;
                assert factor <= Short.MAX_VALUE;
                assert lo % MAX_COLUMN_COUNT_PER_COPIER == 0;

                // this is a trick to get 'lo' to stack, without using a constant pool
                // lo can be greater than Short.MAX_VALUE so we cannot use asm.iconst()
                asm.iconst(factor);
                // Stack: [rowWriter, toColumnIndex, record, fromColumnIndex_base, factor]
                asm.iconst(MAX_COLUMN_COUNT_PER_COPIER);
                // Stack: [rowWriter, toColumnIndex, record, fromColumnIndex_base, factor, maxColumnCountPerCopier]
                asm.imul();
                // Stack: [rowWriter, toColumnIndex, record, fromColumnIndex_base, lo]
            }
            // Stack: [rowWriter, toColumnIndex, record, fromColumnIndex_base, lo]
            // let's add 'lo' to 'fromColumnIndex_base'
            asm.iadd();
            // Stack: [rowWriter, toColumnIndex, record, fromColumnIndex]
        }
    }
