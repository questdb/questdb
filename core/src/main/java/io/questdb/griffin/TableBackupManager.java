...
public class TableBackupManager {
	...
	public void backupTable(CairoSecurityContext securityContext, CharSequence tableName, Path path) {
		...
		try (TableReader reader = engine.getReader(securityContext, tableName)) {
			cloneMetaData(tableName, configuration.getFilesFacade(), path, configuration.getRoot(), configuration.getBackupRoot(), configuration.getMkDirMode(), (TableReaderMetadata) reader.getMetadata());
	...
	private static void cloneMetaData(CharSequence tableName, FilesFacade ff, Path path, CharSequence root, CharSequence backupRoot, int mkDirMode, TableReaderMetadata sourceMetaData) {
