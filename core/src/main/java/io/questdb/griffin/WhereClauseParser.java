...
public final class WhereClauseParser implements Mutable {
    ...
    public IntrinsicModel extract(
            ...
            boolean preventKeyColumnExtraction,
    ...
    private IntrinsicModel extract0(
        ...
        if (removeAndIntrinsics(
                ...
                metadata,
                executionContext,
                preventKeyColumnExtraction, reader)) {
        ...
        if (a.type == ExpressionNode.LITERAL && (b.type == ExpressionNode.CONSTANT || isFunc(b))) {
            if (isTimestamp(a)) {
                ...
                switch (ColumnType.tagOf(m.getColumnType(index))) {
                    ...
                    case ColumnType.INT:
                        if (columnCanBecomeKeyColumn(columnName, m, preventKeyColumnExtraction)) {
    ...
    private boolean analyzeIn(
        ...
        return analyzeInInterval(timestampDriver, model, col, node, false, functionParser, metadata, executionContext, false)
                || analyzeListOfValues(model, column, metadata, node, preventKeyColumnExtraction, reader, functionParser, executionContext)
                || analyzeInLambda(model, column, metadata, node, preventKeyColumnExtraction, reader);
    }
    ...
    private boolean analyzeListOfValues(
        ...
        if (columnCanBecomeKeyColumn(columnName, meta, preventKeyColumnExtraction)) {
            // check if we already have indexed column, and it is of worse selectivity
            if (model.keyColumn != null
    ...
    private boolean analyzeNotBetween(
        ...
        if (ok) {
            ...
            analyzeNotListOfValues(model, column, m, node, notNode, preventKeyColumnExtraction, reader, functionParser, executionContext);
        }
    ...
    private boolean columnCanBecomeKeyColumn(
            CharSequence columnName,
            RecordMetadata m,
            boolean preventKeyColumnExtraction
    ) {
        return !preventKeyColumnExtraction &&
                (
    ...
    private boolean removeAndIntrinsics(
        ...
        return switch (intrinsicOps.get(node.token)) {
            case INTRINSIC_OP_IN ->
                    analyzeIn(timestampDriver, translator, model, node, m, functionParser, executionContext, preventKeyColumnExtraction, reader);
            ...
            case INTRINSIC_OP_EQUAL ->
                    analyzeEquals(timestampDriver, translator, model, node, m, functionParser, executionContext, preventKeyColumnExtraction, reader);
            case INTRINSIC_OP_NOT_EQ ->
                    analyzeNotEquals(timestampDriver, translator, model, node, m, functionParser, executionContext, preventKeyColumnExtraction, reader);
            ...
            case INTRINSIC_OP_AND_OFFSET ->
                    analyzeAndOffset(timestampDriver, translator, model, node, m, functionParser, metadata, executionContext, preventKeyColumnExtraction, reader);
            default -> false;
        };
    }
