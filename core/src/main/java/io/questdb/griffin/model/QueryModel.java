...
public class QueryModel implements IQueryModel {
    ...
    /**
     * Recursively deep-clones this IQueryModel and its entire subtree (nestedModel,
     ...
     */
    @Override
    public IQueryModel deepClone(
            ObjectPool<QueryModel> queryModelPool,
            ObjectPool<QueryColumn> queryColumnPool,
            ObjectPool<ExpressionNode> expressionNodePool,
            ObjectPool<WindowExpression> windowExpressionPool
    ) {
        IQueryModel dst = queryModelPool.next();
        ...
        for (int i = 1, n = joinModels.size(); i < n; i++) {
            IQueryModel dstJm = joinModels.getQuick(i).deepClone(queryModelPool, queryColumnPool, expressionNodePool, windowExpressionPool);
            dst.getJoinModels().add(dstJm);
        ...
        for (int i = 0, n = aliasColKeys.size(); i < n; i++) {
            ...
            if (key != null && dst.getAliasToColumnMap().excludes(key)) {
        ...
        for (int i = 0, n = orderBy.size(); i < n; i++) {
            dst.getOrderBy().add(ExpressionNode.deepClone(expressionNodePool, orderBy.getQuick(i)));
            dst.getOrderByDirection().add(orderByDirection.getQuick(i));
        }
        ...
        for (int i = 0, n = sampleByFill.size(); i < n; i++) {
            dst.getSampleByFill().add(ExpressionNode.deepClone(expressionNodePool, sampleByFill.getQuick(i)));
        }
        ...
        for (int i = 0, n = latestBy.size(); i < n; i++) {
            dst.getLatestBy().add(ExpressionNode.deepClone(expressionNodePool, latestBy.getQuick(i)));
        }
