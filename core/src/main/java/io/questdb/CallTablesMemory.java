                ...
                for (int partitionIndex = 0; partitionIndex < partitionCount; partitionIndex++) {
                    ...
                    if (rowCount > 1){
                        for (int columnIndex = 0; columnIndex < reader.getColumnCount(); columnIndex++) {
                            ...
                            LOG.info().$("[EDIT]").$(columnData);
                            /* In case, we want to manipulate the tuples through another data structure / debugging purposes*/
                            /*LOG.info().$("[EDIT] Reading [table=").$(readEntireColumn(columnData, reader.getMetadata().getColumnType(columnIndex), rowCount)).I$();*/
                        }   
                    }   
                }
            }
