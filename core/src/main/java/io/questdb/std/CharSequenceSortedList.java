...
public class CharSequenceSortedList implements Mutable, Sinkable, ReadOnlyObjList<CharSequence> {
    ...
    public void add(CharSequence value) {
        ...
        if (size > 0 && p > 0 && Chars.compare(buffer[p - 1], value) == 0) {
            return;
        }
    ...
    public void remove(CharSequence val) {
        ...
        if (size > 0 && index > -1 && Chars.compare(buffer[index], val) == 0) {
            removeIndex(index);
        }
    }
    ...
    private int binSearch(CharSequence v) {
        ...
        while (high - low > 65) {
            ...
            int compareResult = Chars.compare(midVal, v);
            if (compareResult < 0)
                low = mid + 1;
            else if (compareResult > 0)
                high = mid;
            else {
                while (++mid < high && Chars.compare(buffer[mid], v) == 0) {
                }
                return mid;
            }
        }
