...
public final class Unsafe {
    ...
    private static final boolean enableJemalloc;
    ...
    public static long malloc(long size, int memoryTag) {
        try {
            ...
            long ptr = enableJemalloc ? Os.malloc(size) : Unsafe.getUnsafe().allocateMemory(size);
            recordMemAlloc(size, memoryTag);
            MALLOC_COUNT.incrementAndGet();
            return ptr;
    ...
    public static long realloc(long address, long oldSize, long newSize, int memoryTag) {
        try {
            ...
            long ptr = enableJemalloc ? Os.realloc(address, newSize) : Unsafe.getUnsafe().reallocateMemory(address, newSize);
    ...
    static {
        try {
            ...
            enableJemalloc = !Os.isWindows() && System.getenv("qdb_no_jemalloc") == null;
        } catch (ReflectiveOperationException e) {
            throw new ExceptionInInitializerError(e);
        }
        //#if jdk.version!=8
