...
public interface CharSinkBase<T extends CharSinkBase<?>> {
     ...
     */
    default T fillAscii(char c, int n) {
        for (int i = 0; i < n; i++) {
            putAscii(c);
        }
        return (T) this;
     ...
     * the implementation.
     */
    default T putAscii(@Nullable CharSequence cs) {
        return put(cs);
    }
