...
/**
 * A long list backed by anonymous mmap. Uses MADV_FREE on resetCapacity()
 * to allow the kernel to reclaim pages without actually unmapping the memory.
 * Linux-only.
 */
public class MMapedLongList implements Mutable, Closeable, Reopenable {
    private static final Log LOG = LogFactory.getLog(MMapedLongList.class);
    ...
    public MMapedLongList(long capacity) {
        this.capacity = (capacity * Long.BYTES);
        this.address = Files.mmapAnon(this.capacity);
        if (this.address == -1) {
            throw CairoException.critical(Os.errno()).put("could not mmap anonymous memory [size=").put(this.capacity).put(']');
        }
        this.pos = address;
    ...
    public final void addAll(MMapedLongList that) {
    ...
    public void close() {
        if (address != 0) {
            Files.munmap0(address, capacity);
            address = 0;
            limit = 0;
            pos = 0;
            capacity = 0;
    ...
    public void reopen() {
        if (address == 0) {
            ...
            this.pos = address;
            this.limit = address + this.capacity;
        }
    }

    /**
     * Resets the list to empty state while keeping the memory mapped.
     * Uses MADV_FREE to tell the kernel that the pages can be reclaimed
     * if memory pressure occurs, but data will be preserved if not reclaimed.
     */
    public void resetCapacity() {
        pos = address;
        // MADV_FREE tells the kernel these pages are no longer needed
        // and can be reclaimed under memory pressure
        Files.nativeMadvise(address, capacity, Files.POSIX_MADV_FREE);
    }
    ...
    private void setCapacityBytes(long capacity) {
        if (this.capacity != capacity) {
            ...
            final long newAddress = Files.mremapAnon(this.address, oldCapacity, capacity);
            if (newAddress == -1) {
                throw CairoException.critical(Os.errno()).put("could not mremap anonymous memory [oldSize=").put(oldCapacity).put(", newSize=").put(capacity).put(']');
            }
            this.capacity = capacity;
            this.address = newAddress;
            this.limit = newAddress + capacity;
            this.pos = Math.min(this.limit, newAddress + oldSize);
