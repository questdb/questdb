...
class ObjArrayDequeue<T> implements Mutable {
    ...
    public ObjArrayDequeue() {
        this(DEFAULT_INITIAL_CAPACITY);
    }

    public ObjArrayDequeue(int initialCapacity) {
    ...
    public T peekLast(int n) {
        return elements[(head + n) & mask];
    }

    public T popFirst() {
        int t = tail;
        final T result = elements[t = (t-1)&mask];
        if (result == null) {
            return null;
        }
        elements[t] = null;
