...
public class CheckWalTransactionsJob extends SynchronizedJob {
    ...
    public void checkNotifyOutstandingTxnInWal(TableToken tableToken, long seqTxn) {
        if (
            ...
            if (TableUtils.isInvisibleTempTableName(tableToken.getTableName(), engine.getConfiguration().getTempTablePrefix())) {
    ...
    private class RenameTrackingMetadataService implements MetadataServiceStub {
        ...
        @Override
        public void addColumn(CharSequence name, int type, int symbolCapacity, boolean symbolCacheFlag, boolean isIndexed, int indexValueBlockCapacity, boolean isSequential, SqlExecutionContext executionContext) {
        }
