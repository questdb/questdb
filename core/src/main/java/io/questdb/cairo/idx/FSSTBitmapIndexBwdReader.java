...
/**
 * Backward reader for FSST-compressed bitmap index.
 * <p>
 * Iterates generations in reverse order (newest first), decoding all values for
 * the requested key in each generation into a buffer, then yields them in
 * descending order. FSST encoding is a forward-only byte stream, so backward
 * iteration requires decoding the full key data per generation.
 */
public class FSSTBitmapIndexBwdReader implements BitmapIndexReader {
    private static final String INDEX_CORRUPT = "cursor could not consistently read index header [corrupt?]";
    private static final Log LOG = LogFactory.getLog(FSSTBitmapIndexBwdReader.class);
    ...
    public FSSTBitmapIndexBwdReader(
    ...
    private class Cursor implements RowCursor {
        ...
        private long[] valueBuffer = new long[256];
        private int valueBufferLen;

        @Override
        public boolean hasNext() {
            while (true) {
                // Serve from buffer in reverse
                while (bufferPos >= 0) {
                    long value = valueBuffer[bufferPos];
                    if (value < minValue) {
                        return false;
                    }
                    bufferPos--;
        ...
        private void loadGeneration() {
            ...
            int valueCount = Unsafe.getUnsafe().getInt(genAddr + (long) requestedKey * Integer.BYTES);
            if (valueCount == 0) {
                this.bufferPos = -1;
                this.valueBufferLen = 0;
                return;
            ...
            if (valueCount > valueBuffer.length) {
                valueBuffer = new long[Math.max(valueCount, valueBuffer.length * 2)];
            }

            // Decode all values for this key in this generation
            ...
            decodeAll(encodedAddr, encodedLen, valueCount);
            this.valueBufferLen = valueCount;
            this.bufferPos = valueCount - 1;
        }

        private void decodeAll(long encodedAddr, int encodedLen, int valueCount) {
            int encodedOff = 0;
