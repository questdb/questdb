...
public class CoveringCompressor {
    ...
    public static int compressLongsLinearPred(long srcAddr, int count, long destAddr, long longWorkspaceAddr) {
        ...
        if (lastValue < firstValue) {
            // A composite partition mixes pieces that a merge-append relocated to the tail of the shared column files,
            // so ascending physical row ids no longer mean ascending timestamps: one key's posting list steps.
            return compressLongs(srcAddr, count, destAddr);
        }
