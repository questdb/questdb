...
public class PageFrameSequence<T extends StatefulAtom> implements Closeable {
    ...
    public void await() {
        LOG.debug()
                ...
                .$(", dispatchStartFrameIndex=").$(dispatchStartFrameIndex)
                .$(", reduceCounter=").$(reduceCounter.get())
                .$(", doneLatch=").$(doneLatch.getCount())
                .I$();
    ...
    public PageFrameSequence<T> of(
        ...
        initWorkerRecord(executionContext.getCircuitBreaker());
    ...
    private void initWorkerRecord(SqlExecutionCircuitBreaker executionContextCircuitBreaker) {
        if (record == null) {
