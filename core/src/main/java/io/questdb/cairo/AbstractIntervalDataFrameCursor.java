...
public abstract class AbstractIntervalDataFrameCursor implements DataFrameCursor {
    ...
    protected static long search(ReadMemory column, long value, long low, long high, int increment) {
        while (low < high) {
    ...
    private long computeSize() {
        ...
        while (intervalsLo < intervalsHi && partitionLo < partitionHi) {
            ...
            if (rowCount > 0) {
                ...
                final ReadMemory column = reader.getColumn(TableReader.getPrimaryColumnIndex(reader.getColumnBase(partitionLo), timestampIndex));
