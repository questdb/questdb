...
public final class TableUtils {
	...
	public static void backupTable(CharSequence tableName, CairoConfiguration sourceConfig) {
		if (null == sourceConfig.getBackupRoot()) {
		...
		try (TableReader reader = new TableReader(sourceConfig, tableName)) {
			cloneMetaData(tableName, sourceConfig.getFilesFacade(), new Path(), sourceConfig.getRoot(), sourceConfig.getBackupRoot(), sourceConfig.getMkDirMode());
			RecordMetadata metaData = reader.getMetadata();
			int timestampCol = metaData.getTimestampIndex();

			try (TableWriter backupWriter = new TableWriter(sourceConfig, tableName,  sourceConfig.getBackupRoot()); RecordCursor cursor = reader.getCursor()) {
	...
	private static void cloneMetaData(CharSequence tableName, FilesFacade ff, Path path, CharSequence root, CharSequence backupRoot, int mkDirMode) {
		...
		try (TableReaderMetadata sourceMetaData = new TableReaderMetadata(ff, path)) {
			...
			try (AppendMemory backupMem = new AppendMemory()) {
				...
				backupMem.of(ff, path.trimTo(rootLen).concat(TXN_FILE_NAME).$(), ff.getPageSize());
				TableUtils.resetTxn(backupMem, symbolMapCount, 0L, INITIAL_TXN);
