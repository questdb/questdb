...
public interface LiveViewStateStore extends QuietCloseable {
    ...
    /**
     * Returns false when this store is a quiesced no-op stand-in -- e.g. when live views are
     * disabled. {@link LiveViewRefreshJob} consults it to skip its entire refresh pass
     * (both the notification-queue drain and the registry fallback scan) without touching the
     * registry, so the refresh workers stay idle until a real store is installed. Under symmetric
     * local refresh every role runs a refresh-enabled store, so this returns true on both a
     * primary and a read-only replica.
     */
