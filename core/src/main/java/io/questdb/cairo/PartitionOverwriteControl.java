...
public class PartitionOverwriteControl {
    ...
    public void acquirePartitions(TableReader reader) {
        if (enabled) {
            LOG.info().$("acquiring partitions [table=").utf8(reader.getTableToken().getTableName())
    ...
    public void releasePartitions(TableReader reader) {
        if (enabled) {
            LOG.info().$("releasing partitions [table=").utf8(reader.getTableToken().getTableName())
                    .$(", readerTxn=").$(reader.getTxn())
                    .I$();
            ...
            LOG.error().$("reader not found in partition usage map [table=").utf8(reader.getTableToken().getTableName())
                    .$(", readerTxn=").$(reader.getTxn())
                    .I$();
        }
    }
