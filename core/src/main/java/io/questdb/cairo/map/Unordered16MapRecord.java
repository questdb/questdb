...
/**
 * Provides Record access interface for Unordered16Map key-value pairs.
 * <p>
...
final class Unordered16MapRecord implements MapRecord {
    ...
    Unordered16MapRecord(
        ...
        assert keyTypes.getColumnCount() == 2;
        ...
        if (valueTypes != null) {
            ...
            nColumns = valueTypes.getColumnCount() + 2;
        } else {
            keyIndexOffset = 0;
            nColumns = 2;
        }
        ...
        columnOffsets[keyIndexOffset + 1] = Long.BYTES;
        long offset = Unordered16Map.KEY_SIZE;
        if (valueTypes != null) {
    ...
    public void copyToKey(MapKey destKey) {
        ...
        destBaseKey.copyFromRawKey(
                Unsafe.getLong(startAddress),
                Unsafe.getLong(startAddress + Long.BYTES)
        );
    }
    ...
    public void copyValue(MapValue destValue) {
        ...
        destPackedValue.copyRawValue(startAddress + Unordered16Map.KEY_SIZE);
    }
    ...
    public MapValue getValue() {
        return value.of(startAddress, startAddress + Unordered16Map.KEY_SIZE, false);
    }

    @Override
    public long keyHashCode() {
        return Hash.hashLong128_64(
                Unsafe.getLong(startAddress),
                Unsafe.getLong(startAddress + Long.BYTES)
        );
    }
