...
public final class Unordered16MapCursor implements MapRecordCursor {
    ...
    public void longTopK(DirectLongLongSortedList list, Function recordFunction) {
        ...
        for (
                long addr = memStart, control = controlStart;
                addr < memLimit;
                addr += entrySize, control++
        ) {
            if (Unsafe.getByte(control) != 0) {
                recordA.of(addr);
                long v = recordFunction.getLong(recordA);
                list.add(addr, v);
            }
    ...
    public void toTop() {
        ...
        remaining = count;
        if (count > 0 && (zeroKeyAddress == 0 || count > 1) && Unsafe.getByte(controlAddress) == 0) {
            skipToNonZeroKey();
        }
    }
    ...
    private void skipToNonZeroKey() {
        do {
            address += entrySize;
            controlAddress++;
        } while (controlAddress < controlLimit && Unsafe.getByte(controlAddress) == 0);
    }

    Unordered16MapCursor init(
            long controlStart,
            long controlLimit,
            long memStart,
            long memLimit,
