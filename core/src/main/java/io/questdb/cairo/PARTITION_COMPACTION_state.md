...
Same five steps as the reference's own pass: `PARTITION_COMPACTION.md` Sec.9 steps 0, 1, 2, 3, 5. Steps 4
(MOVE-TAIL, MAKE-PLAIN, TRIM-FILES) and 6 (turn on by default, delete the temporary squash budget) are not
attempted, matching the reference.
...
| 4 | **no** | MOVE-TAIL, MAKE-PLAIN, TRIM-FILES | not started, as in the reference |
...
- `TableWriter.java` - `runCompaction`, `compactPhysicalPartition`, `foldContiguousPieces` (JOIN),
  `foldFoldableFolders`, `rewritePhysicalPartition` (REWRITE), `avgRecordSize()`, `getCompactionWrittenRows()`,
  wired into `housekeep` right before `processPartitionRemoveCandidates()`, matching the reference's own
  ordering rationale (REWRITE is what puts a directory on the remove-candidate list).
...
- Five-file config pattern for eleven `cairo.partition.compaction.*` settings - `PropertyKey.java`,
  `PropServerConfiguration.java`, `CairoConfiguration.java`, `DefaultCairoConfiguration.java`,
  `CairoConfigurationWrapper.java`. Same eleven keys the reference actually implemented (not
  `prefix.min.percent`, `make.plain.enabled`, `queue.capacity` - those belong to step 4/6).
...
`O3PartitionCompactionTest`: **6 of 10 pass** - the same bar the reference reports, though not always for
the same reason.
...
| `testMoveTailCopiesTheTailNotTheWholePartition` | **fail - step 4 not implemented** |
| `testMakePlainWaitsForAReaderHoldingThePreMoveTailTransaction` | **fail - step 4 not implemented** |
| `testTrimFilesWaitsForAReaderThatMappedTheOldExtent` | **fail - step 4 not implemented** |
...
waste; the four failures are exactly the ones the reference predicts and for the reference's own reasons -
REWRITE runs regardless of a pinned reader (it never writes below `E`, so it needs no reader check), so it
always finishes the job REWRITE is capable of finishing, and the three step-4 tests fail on the one
assertion that specifically checks for the missing instalment plan:

- `testMoveTailCopiesTheTailNotTheWholePartition` fails because REWRITE copies the WHOLE partition
  (`rowsWritten=20600`), not just the messy tail (`< 5000`) - the waste is still fully reclaimed first.
...
`testMoveTailCopiesTheTailNotTheWholePartition` and `testTrimFilesWaitsForAReaderThatMappedTheOldExtent`
to push `dead` safely past `live`. This branch has no equivalent of the reference's cluster pre-split
kicking in unasked for a plain `insert`-sized batch against a 20k-row partition with no explicit
`cairo.o3.partition.split.min.size` override, so every backdate here rewrites the WHOLE partition rather
than just its tail - which is a fixture-tuning difference, not a functional one.
...
- **Compaction never touches `minSplitPartitionTimestamp` or any of the classic-split (`squashSplitPartitions`)
  ...
  ever touch one `attachedPartitions` entry's own fields and this branch's `PartitionGeometry`.
...
- MOVE-TAIL, MAKE-PLAIN, TRIM-FILES (step 4) and turning compaction on by default (step 6) - out of scope,
  matching the reference's own pass.
