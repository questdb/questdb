...
public class ReaderPool extends AbstractPool implements ResourcePool<TableReader> {
                    ...
                    if (r == null) {
                        try {
                            LOG.info()
                                    ...
                                    .$("' [at=").$(e.index).$(':').$(i)
                                    .$(']').$();
                            String tableName = engine.getTableNameBySystemName(name);
                            r = new R(this, e, i, name, tableName, messageBus);
    ...
    private boolean returnToPool(R reader) {
        CharSequence name = reader.getSystemTableName();
    ...
    public static class R extends TableReader {
        ...
        public R(ReaderPool pool, Entry entry, int index, CharSequence systemTableName, CharSequence tableName, MessageBus messageBus) {
            super(pool.getConfiguration(), tableName, systemTableName, messageBus);
            this.pool = pool;
            this.entry = entry;
            this.index = index;
        }
