...
public class UncompressedMetadataPool extends AbstractPool implements ResourcePool<TableRecordMetadata> {

    public static final int ENTRY_SIZE = 32;
    private static final Log LOG = LogFactory.getLog(UncompressedMetadataPool.class);
    ...
    public UncompressedMetadataPool(CairoConfiguration configuration, TableSequencerAPI tableSequencerAPI) {
    ...
    private R1 newMetadataInstance(CharSequence tableName, Entry e, int entryIndex) {
        if (tableSequencerAPI.hasSequencer(tableName)) {
            ...
            tableSequencerAPI.getTableMetadata(tableName, r, false);
            return r;
        }
    ...
    private boolean returnToPool(R1 r) {
        ...
        final UncompressedMetadataPool.Entry e = r.getEntry();
        if (e == null) {
            return false;
        }
    ...
    private interface R1 extends TableRecordMetadata {
        ...
        UncompressedMetadataPool getPool();
    ...
    private static class LegacyR extends DynamicTableReaderMetadata implements R1 {
        ...
        private UncompressedMetadataPool pool;
        private Entry entry;

        public LegacyR(UncompressedMetadataPool pool, Entry entry, int index, CharSequence name) {
        ...
        @Override
        public void close() {
            final UncompressedMetadataPool pool = getPool();
        ...
        @Override
        public UncompressedMetadataPool getPool() {
            return pool;
        }
    ...
    private static class R extends GenericTableRecordMetadata implements R1 {
        ...
        public R(UncompressedMetadataPool pool, Entry entry, int index) {
            this.pool = pool;
            this.entry = entry;
            this.index = index;
        }
        ...
        @Override
        public void tryReload() {
            pool.tableSequencerAPI.reloadMetadataConditionally(getTableName(), getStructureVersion(), this, false);
        }
    }
}