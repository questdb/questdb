        ...
        if (wallClockMillis > nextCommitTime) {
            ...
            for (int n = 0, sz = assignedTables.size(); n < sz; n++) {
                ...
                try {
                    ...
                    LOG.critical()
                            .$("commit failed [table=").$(assignedTables.getQuick(n).getUtf16TableName())
                            .$(",ex=").$(ex)
                            .I$();
                    ...
                    try {
                        if (tab.isWriterInError()) {
                            ...
                            if (!tab.isAssignedToJob()) {
                                ...
                                LOG.info()
                                        .$("assigned table to writer thread [tableName=").$(tab.getUtf16TableName())
                                        .$(", threadId=").$(workerId)
                                        .I$();
                            }
                            event.append();
                        ...
                        LOG.critical()
                                .$("closing writer because of error [table=").$(tab.getUtf16TableName())
                                .$(",ex=").$(ex)
                                .I$();
                        metrics.health().incrementUnhandledErrors();
                        closeWriter = true;
