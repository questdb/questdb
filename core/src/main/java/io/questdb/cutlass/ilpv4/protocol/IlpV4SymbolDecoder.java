 ...
 * </pre>
 */
public final class IlpV4SymbolDecoder implements IlpV4ColumnDecoder {

    public static final IlpV4SymbolDecoder INSTANCE = new IlpV4SymbolDecoder();
    ...
    public int decode(long sourceAddress, int sourceLength, int rowCount, boolean nullable, ColumnSink sink) throws IlpV4ParseException {
        ...
        if (dictSize < 0) {
            throw IlpV4ParseException.create(
                    IlpV4ParseException.ErrorCode.INVALID_DICTIONARY_INDEX,
    ...
    public static long encode(long destAddress, String[] values, boolean[] nulls) {
        ...
        pos = IlpV4Varint.encode(pos, dictList.size());

        // Write dictionary entries
        for (String entry : dictList) {
            byte[] bytes = entry.getBytes(StandardCharsets.UTF_8);
            pos = IlpV4Varint.encode(pos, bytes.length);
            for (byte b : bytes) {
    ...
    public static int encode(byte[] buf, int offset, String[] values, boolean[] nulls) {
        ...
        // Write dictionary size
        offset = IlpV4Varint.encode(buf, offset, dictList.size());

        // Write dictionary entries
        for (String entry : dictList) {
            byte[] bytes = entry.getBytes(StandardCharsets.UTF_8);
            offset = IlpV4Varint.encode(buf, offset, bytes.length);
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            if (dictIndices[i] < 0) {
                ...
                offset = IlpV4Varint.encode(buf, offset, dictIndices[i]);
            }
        }
