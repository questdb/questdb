        ...
        try {
            ...
            switch (atomicity) {
                case Atomicity.SKIP_ALL:
                    writer.rollback();
                    throw CairoException.instance(-1).put("bad syntax [line=").put(line).put(", col=").put(i).put(']');
            ...
            if (detectedType != columnType) {
                ...
                switch (ColumnType.tagOf(columnType)) {
                    ...
                    case ColumnType.BINARY:
                        writer.close();
                        throw CairoException.instance(-1).put("cannot import text into BINARY column [index=").put(i).put(']');
                    default:
