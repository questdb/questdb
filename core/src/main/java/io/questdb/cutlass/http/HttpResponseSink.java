...
public class HttpResponseSink implements Closeable, Mutable {
    ...
    private class ChunkBuffer extends AbstractCharSink implements Closeable {
        ...
        private ChunkBuffer(int sz) {
            bufStart = Unsafe.malloc(sz + MAX_CHUNK_HEADER_SIZE + EOF_CHUNK.length(), MemoryTag.NATIVE_DEFAULT);
        ...
        public void close() {
            if (0 != bufStart) {
                Unsafe.free(bufStart, bufEndOfData - bufStart + EOF_CHUNK.length(), MemoryTag.NATIVE_DEFAULT);
