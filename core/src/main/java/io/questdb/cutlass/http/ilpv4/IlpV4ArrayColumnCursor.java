...
import static io.questdb.cutlass.http.ilpv4.IlpV4Constants.TYPE_DOUBLE_ARRAY;
 ...
 */
public final class IlpV4ArrayColumnCursor implements IlpV4ColumnCursor {
    ...
    public int of(long dataAddress, int dataLength, int rowCount, byte typeCode, boolean nullable,
        ...
        for (int row = 0; row < rowCount; row++) {
            if (nullable && IlpV4NullBitmap.isNull(nullBitmapAddress, row)) {
    ...
    public boolean advanceRow() throws IlpV4ParseException {
        ...
        if (nullable && nullBitmapAddress != 0 && IlpV4NullBitmap.isNull(nullBitmapAddress, currentRow)) {
            currentIsNull = true;
            currentNDims = 0;
