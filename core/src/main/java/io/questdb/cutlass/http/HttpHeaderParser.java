...
public class HttpHeaderParser implements Mutable, Closeable, HttpRequestHeader {
    ...
    public HttpHeaderParser(int bufferLen, ObjectPool<DirectByteCharSequence> pool) {
        ...
        this.headerPtr = Unsafe.malloc(sz, MemoryTag.NATIVE_DEFAULT);
...
//        this.pool.clear();
    ...
    public void close() {
        if (this.headerPtr != 0) {
            Unsafe.free(this.headerPtr, this.hi - this.headerPtr, MemoryTag.NATIVE_DEFAULT);
            this.headerPtr = 0;
    ...
    public static class BoundaryAugmenter implements Closeable {
        ...
        public BoundaryAugmenter() {
            this.lim = 64;
            this.lo = this._wptr = Unsafe.malloc(this.lim, MemoryTag.NATIVE_DEFAULT);
            of0(BOUNDARY_PREFIX);
        }
        ...
        public void close() {
            if (lo > 0) {
                Unsafe.free(this.lo, this.lim, MemoryTag.NATIVE_DEFAULT);
                this.lo = 0;
            }
        }
        ...
        private void resize(int lim) {
            ...
            this.lo = _wptr = Unsafe.malloc(this.lim, MemoryTag.NATIVE_DEFAULT);
            of0(BOUNDARY_PREFIX);
        }
    }
