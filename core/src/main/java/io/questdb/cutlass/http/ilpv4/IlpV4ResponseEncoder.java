 ...
 * </pre>
 */
public class IlpV4ResponseEncoder {
     ...
     */
    public static int encode(IlpV4Response response, long address, int maxLen) {
        int offset = 0;
        ...
        if (statusCode == IlpV4StatusCode.PARTIAL) {
            ...
            for (int i = 0; i < errorCount; i++) {
                IlpV4Response.TableError error = errors.get(i);
     ...
     */
    public static byte[] encode(IlpV4Response response) {
     ...
     */
    public static int calculateSize(IlpV4Response response) {
     ...
     */
    public static IlpV4Response decode(byte[] buf, int offset, int length) throws IlpV4ParseException {
        if (length < 1) {
        ...
        if (statusCode == IlpV4StatusCode.PARTIAL) {
            ...
            ObjList<IlpV4Response.TableError> errors = new ObjList<>(errorCount);
            for (int i = 0; i < errorCount; i++) {
                // Table index
                ...
                errors.add(new IlpV4Response.TableError(tableIndex, errorCode, errorMessage));
            }

            return IlpV4Response.partial(errors);
        } else {
            // Decode error message
            String[] stringResult = decodeString(buf, pos, offset + length);
            String errorMessage = stringResult[0];

            return IlpV4Response.error(statusCode, errorMessage);
        }
    }
     ...
     * @throws IlpV4ParseException if parsing fails
     */
    public static IlpV4Response decode(long address, int length) throws IlpV4ParseException {
    ...
    private static int[] decodeVarint(byte[] buf, int offset, int limit) throws IlpV4ParseException {
        long result = 0;
        int shift = 0;
        int pos = offset;
    ...
    private static String[] decodeString(byte[] buf, int offset, int limit) throws IlpV4ParseException {
