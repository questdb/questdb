...
public class ExportQueryProcessor implements HttpRequestProcessor, HttpRequestHandler, Closeable, CopyExportRequestTask.WriteParquetCallBack {
    ...
    @Override
    public void onWrite(long dataPtr, long dataLen, byte format) throws Exception {
        if (dataLen <= 0) {
            return;
        }
        assert currentContext != null;
        ExportQueryProcessorState state = LV.get(currentContext);
        if (state.firstParquetWriteCall && !state.getExportModel().isNoDelay()) {
            header(currentContext.getChunkedResponse(), state, 200);
        }
    ...
    private void copyQueryToSend(ExportQueryProcessorState state) throws SqlException {
        if (state.copyID != -1) {
            CopyExportContext.ExportTaskEntry entry = null;
            try {
                var copyExportContext = engine.getCopyExportContext();
