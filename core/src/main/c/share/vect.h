...
typedef int64_t MatchGroupFuncType(int32_t*, double *, int64_t);

#define MATCH_GROUP_DISPATCHER(func) \
\
MatchGroupFuncType F_SSE2(func), F_SSE41(func), F_AVX2(func), F_AVX512(func), F_DISPATCH(func); \
\
extern "C" { \
JNIEXPORT jlong JNICALL Java_io_questdb_std_Vect_ ## func(JNIEnv *env, jclass cl, jlong pInt, jlong pDouble, jlong size) { \
    return F_AVX2(func)((int32_t*) pInt, (double *) pDouble, size); \
}\
\
}
...
MatchGroupFuncType F_SSE2(func), F_SSE41(func), F_AVX2(func), F_AVX512(func), F_DISPATCH(func); \
\
MatchGroupFuncType *POINTER_NAME(func) = &func ## _dispatch; \
\
int64_t F_DISPATCH(func) (int32_t* pi, double *d, int64_t count) { \
    const int iset = instrset_detect();  \
    if (iset >= 10) { \
        POINTER_NAME(func) = &F_AVX512(func); \
    } else if (iset >= 8) { \
    ...
    return (*POINTER_NAME(func))(pi, d, count); \
} \
\
inline int64_t func(int32_t* pi, double *d, int64_t count) { \
return (*POINTER_NAME(func))(pi, d, count); \
}\
\
extern "C" { \
