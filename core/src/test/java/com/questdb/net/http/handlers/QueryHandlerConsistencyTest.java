...
public class QueryHandlerConsistencyTest extends AbstractOptimiserTest {
    ...
    public void testCsvHandlerConsistency() throws Exception {
        try (ReaderFactoryPool pool = new ReaderFactoryPool(getWriterFactory().getConfiguration(), 1)) {
            testHandler(new CsvHandler(pool, new ServerConfiguration()));
        }
    }
    ...
    public void testCsvOutput() throws Exception {
        try (ReaderFactoryPool pool = new ReaderFactoryPool(getWriterFactory().getConfiguration(), 1)) {
            ContextHandler handler = new CsvHandler(pool, new ServerConfiguration());
            handler.setupThread();
    ...
    public void testQueryHandlerConsistency() throws Exception {
        try (ReaderFactoryPool pool = new ReaderFactoryPool(getWriterFactory().getConfiguration(), 1)) {
            testHandler(new QueryHandler(pool, new ServerConfiguration(), getWriterFactory()));
        }
    }
