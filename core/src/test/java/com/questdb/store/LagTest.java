...
public class LagTest extends AbstractTest {
    ...
    public void testLagAppendScenarios() throws JournalException {
        ...
        data1.add(new Quote().setSym("S1").setTimestamp(Dates.toMicros(2013, 1, 10, 10, 0)));
        data1.add(new Quote().setSym("S2").setTimestamp(Dates.toMicros(2013, 1, 10, 14, 0)));
        ...
        try (Journal<Quote> reader = getFactory().reader(Quote.class)) {
            ...
            data2.add(new Quote().setSym("S3").setTimestamp(Dates.toMicros(2013, 1, 10, 15, 0)));
            data2.add(new Quote().setSym("S4").setTimestamp(Dates.toMicros(2013, 1, 10, 16, 0)));
            rw.mergeAppend(data2);

            // simple append + lag split (30 days increment)
            List<Quote> data3 = new ArrayList<>();
            data3.add(new Quote().setSym("S8").setTimestamp(Dates.toMicros(2013, 2, 10, 15, 0)));
            data3.add(new Quote().setSym("S9").setTimestamp(Dates.toMicros(2013, 2, 10, 16, 0)));
            rw.mergeAppend(data3);

            // data on fully above lag
            List<Quote> data4 = new ArrayList<>();
            data4.add(new Quote().setSym("S6").setTimestamp(Dates.toMicros(2013, 2, 10, 10, 0)));
            data4.add(new Quote().setSym("S7").setTimestamp(Dates.toMicros(2013, 2, 10, 11, 0)));
            rw.mergeAppend(data4);

            // lag is fully inside data
            List<Quote> data5 = new ArrayList<>();
            data5.add(new Quote().setSym("S5").setTimestamp(Dates.toMicros(2013, 2, 10, 9, 0)));
            data5.add(new Quote().setSym("S10").setTimestamp(Dates.toMicros(2013, 2, 10, 17, 0)));
            rw.mergeAppend(data5);

            // lag and data have equal boundaries
            List<Quote> data6 = new ArrayList<>();
            data6.add(new Quote().setSym("near-S5").setTimestamp(Dates.toMicros(2013, 2, 10, 9, 0)));
            data6.add(new Quote().setSym("near-S10").setTimestamp(Dates.toMicros(2013, 2, 10, 17, 0)));
            rw.mergeAppend(data6);

            // bottom part of data overlaps top part of lag
            List<Quote> data7 = new ArrayList<>();
            data7.add(new Quote().setSym("after-S4").setTimestamp(Dates.toMicros(2013, 2, 9, 9, 0)));
            data7.add(new Quote().setSym("after-S9").setTimestamp(Dates.toMicros(2013, 2, 10, 16, 30)));
            rw.mergeAppend(data7);

            // top part of data overlaps bottom part of lag
            List<Quote> data8 = new ArrayList<>();
            data8.add(new Quote().setSym("after-S8").setTimestamp(Dates.toMicros(2013, 2, 10, 15, 30)));
            data8.add(new Quote().setSym("after-S10").setTimestamp(Dates.toMicros(2013, 2, 10, 18, 30)));
            rw.mergeAppend(data8);

            // data is fully inside of lag
            List<Quote> data9 = new ArrayList<>();
            data9.add(new Quote().setSym("after-S6").setTimestamp(Dates.toMicros(2013, 2, 10, 10, 30)));
            data9.add(new Quote().setSym("before-S10").setTimestamp(Dates.toMicros(2013, 2, 10, 16, 45)));
            rw.mergeAppend(data9);

            // full discard
            List<Quote> data10 = new ArrayList<>();
            data10.add(new Quote().setSym("discard-S1").setTimestamp(Dates.toMicros(2013, 1, 1, 10, 30)));
            data10.add(new Quote().setSym("discard-S2").setTimestamp(Dates.toMicros(2013, 1, 1, 16, 45)));
            rw.mergeAppend(data10);

            // full discard
            List<Quote> data11 = new ArrayList<>();
            data11.add(new Quote().setSym("discard-S3").setTimestamp(Dates.toMicros(2013, 1, 1, 10, 30)));
            data11.add(new Quote().setSym("before-S6").setTimestamp(Dates.toMicros(2013, 2, 10, 9, 45)));
