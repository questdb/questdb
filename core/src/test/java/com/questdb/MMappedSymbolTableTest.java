...
public class MMappedSymbolTableTest extends AbstractTest {
    ...
    private void createTestTable(String data[]) throws JournalException {
        if (tab == null) {
            tab = new MMappedSymbolTable(DATA_SIZE, 256, 1, getReaderFactory().getConfiguration().getJournalBase(), "test", JournalMode.APPEND, 0, 0, false, false);
        }
    ...
    private MMappedSymbolTable getReader() throws JournalException {
        return new MMappedSymbolTable(DATA_SIZE, 256, 1, getReaderFactory().getConfiguration().getJournalBase(), "test", JournalMode.READ, tab.size(), tab.getIndexTxAddress(), false, false);
    }

    private MMappedSymbolTable getWriter() throws JournalException {
        return new MMappedSymbolTable(DATA_SIZE, 256, 1, getReaderFactory().getConfiguration().getJournalBase(), "test", JournalMode.APPEND, tab.size(), tab.getIndexTxAddress(), false, false);
    }
}