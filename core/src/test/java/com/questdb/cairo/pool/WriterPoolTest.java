...
public class WriterPoolTest extends AbstractCairoTest {
    ...
    public void testAllocateAndClear() throws Exception {
        ...
        TestUtils.assertMemoryLeak(() -> {
            ...
            try (final WriterPool pool = new WriterPool(new DefaultCairoConfiguration(root))) {
                new Thread(() -> {
                    try {
    ...
    public void testFactoryCloseBeforeRelease() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (WriterPool pool = new WriterPool(new DefaultCairoConfiguration(root))) {
                TableWriter x;
                ...
                for (int k = 0; k < 1000; k++) {
                    ...
                    for (int i = 0; i < n; i++) {
                        new Thread(() -> {
                            try {
                                barrier.await();
                                getFromPoolPopulateAndLock(pool, writerCount);
                            } catch (Exception e) {
                                e.printStackTrace();
                                errors.incrementAndGet();
                            } finally {
    ...
    private void getFromPoolPopulateAndLock(WriterPool pool, AtomicInteger writerCount) {
        try (TableWriter w = pool.writer("z")) {
            writerCount.incrementAndGet();
            populate(w);
