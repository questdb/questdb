...
public class WriterPoolTest extends AbstractCairoTest {
            ...
            new Thread(() -> {
                try {
                    for (int i = 0; i < 1000; i++) {
                        try (TableWriter ignored = pool.getWriter("z")) {
    ...
    public void testFactoryCloseBeforeRelease() throws Exception {
        assertWithPool(pool -> {
            ...
            try {
                ...
                Assert.assertTrue(x == pool.getWriter("z"));
                pool.close();
            } finally {
                x.close();
            }
    ...
    public void testLockNonExisting() throws Exception {
        assertWithPool(pool -> {
            ...
            try (TableWriter wx = pool.getWriter("z")) {
    ...
    public void testLockUnlock() throws Exception {
        ...
        assertWithPool(pool -> {
            TableWriter wx = pool.getWriter("x");
            Assert.assertNotNull(wx);
            Assert.assertTrue(wx.isOpen());

            TableWriter wy = pool.getWriter("y");
            ...
            try {
                ...
                new Thread(() -> {
                    try (TableWriter ignored = pool.getWriter("x")) {
                        result.set(false);
                    } catch (EntryUnavailableException ignored) {
                        result.set(true);
            ...
            for (int k = 0; k < 1000; k++) {
                ...
                for (int i = 0; i < n; i++) {
                    new Thread(() -> {
                        try {
                            barrier.await();
                            try (TableWriter w = pool.getWriter("z")) {
                                writerCount.incrementAndGet();
                                populate(w);
                            } catch (EntryUnavailableException ignored) {
                            }
    ...
    public void testWriterDoubleClose() throws Exception {
        assertWithPool(pool -> {
            ...
            X x = new X();
            pool.setPoolListner(x);
            TableWriter w = pool.getWriter("z");
