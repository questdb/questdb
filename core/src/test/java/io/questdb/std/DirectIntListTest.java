...
public class DirectIntListTest {
    ...
    public void testAddList() {
        DirectIntList list = new DirectIntList(256, MemoryTag.NATIVE_INT_LIST);
        DirectIntList list2 = new DirectIntList(256, MemoryTag.NATIVE_INT_LIST);
        final int N = 100;
        for (int i = 0; i < N; ++i) {
            list.add(i);
            list2.add(N + i);
    ...
    public void testAddListExpand() {
        DirectIntList list = new DirectIntList(128, MemoryTag.NATIVE_INT_LIST);
        DirectIntList list2 = new DirectIntList(128, MemoryTag.NATIVE_INT_LIST);
        final int N = 100;
        for (int i = 0; i < N; ++i) {
            list.add(i);
            list2.add(N + i);
    ...
    public void testCapacityAndSize() {
        ...
        DirectIntList list = new DirectIntList(1024, MemoryTag.NATIVE_INT_LIST);
        ...
        Unsafe.getUnsafe().putInt(addr, 42);
    ...
    public void testResizeMemLeak() {
        ...
        long expected = Unsafe.getMemUsed();
        try (DirectIntList list = new DirectIntList(1024, MemoryTag.NATIVE_INT_LIST)) {
    ...
    public void testToString() {
        try (DirectIntList list = new DirectIntList(1001, MemoryTag.NATIVE_INT_LIST)) {
            final int N = 1000;
            for (int i = 0; i < N; ++i) {
                list.add(i);
            }
