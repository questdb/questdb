...
/**
 * The lower/upper-bounded forward page-frame cursor a localized out-of-order live-view
 * repair reads through. The repair proves a convergence boundary {@code H} above which
 * no output can change; the guarantee that buys nothing unless the source scan actually
 * stops there, which a record-level stop filter cannot deliver - it would still visit
 * every partition above {@code H} before rejecting its rows.
 * <p>
 * So these tests assert both halves: the exact row set of an inclusive
 * {@code [lo, hi]} range, and that the partitions outside it are never opened.
 * <p>
...
public class PageFrameTimestampRangeCursorTest extends AbstractCairoTest {
    ...
    private void assertTimestamps(
        ...
        final LongList expected = new LongList();
        for (long timestamp : expectedTimestamps) {
