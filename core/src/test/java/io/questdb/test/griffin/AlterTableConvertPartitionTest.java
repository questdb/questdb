...
public class AlterTableConvertPartitionTest extends AbstractCairoTest {
    ...
    public void testConvertLastPartition() throws Exception {
        final long rows = 10;
        assertMemoryLeak(TestFilesFacadeImpl.INSTANCE, () -> {
            execute("create table x as (select" + " x id," + " rnd_boolean() a_boolean," + " rnd_byte() a_byte," + " timestamp_sequence('2024-06', 500) designated_ts" + " from long_sequence(" + rows + ")) timestamp(designated_ts) partition by month");
            ...
            execute("insert into x(designated_ts) values('2024-06-20')");
    ...
    public void testConvertListZeroSizeVarcharData() throws Exception {
        assertMemoryLeak(TestFilesFacadeImpl.INSTANCE, () -> {
            execute("create table x as (select" + " case when x % 2 = 0 then rnd_varchar(1, 40, 1) end as a_varchar," + " to_timestamp('2024-07', 'yyyy-MM') as a_ts," + " from long_sequence(1)) timestamp (a_ts) partition by MONTH");
    ...
    public void testConvertPartitionBrokenSymbols() throws Exception {
        ...
        assertMemoryLeak(TestFilesFacadeImpl.INSTANCE, () -> {
            execute("create table " + tableName + " as (select" + " x id," + " rnd_symbol('a','b','c') a_symbol," + " timestamp_sequence(400000000000, 500) designated_ts" + " from long_sequence(" + rows + ")) timestamp(designated_ts) partition by month");
