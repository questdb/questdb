...
public class AmaxArrayGroupByFunctionFactoryTest extends AbstractCairoTest {
        ...
        assertMemoryLeak(() -> {
            ...
            assertQueryNoLeakCheck(
                    """
                            ...
                            2\t[50.0,51.0]
                            """,
                    "SELECT grp, amax(arr) arr FROM tab ORDER BY grp",
                    null,
                    true,
                    true
            );
        ...
        assertMemoryLeak(() -> {
            ...
            assertQueryNoLeakCheck(
                    """
                            ...
                            """,
                    "SELECT ts, amax(arr) arr FROM tab SAMPLE BY 1h",
                    "ts",
                    true,
                    true
            );
        ...
        try (WorkerPool pool = new WorkerPool(() -> 4)) {
            TestUtils.execute(
                    ...
                    (engine, compiler, sqlExecutionContext) -> {
                        String sql = "SELECT sym, amax(arr) FROM tab GROUP BY sym ORDER BY sym";
