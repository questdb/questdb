...
public class AvgDecimalRescaleGroupByFunctionFactoryTest extends AbstractCairoTest {
    ...
    public void testAvgDecimal256Rescale256ProducesCorrectAverage() throws Exception {
        assertQuery("select avg(d, 4) avg from d256_values")
                .ddl("create table d256_values as (" +
                        "select case x " +
    ...
    public void testAvgDecimal32Rescale256UsesCount() throws Exception {
        assertQuery("select avg(d, 30) avg from d32_values")
                .ddl("create table d32_values as (" +
    ...
    public void testAvgDecimal64Rescale256UsesCount() throws Exception {
        assertQuery("select avg(d, 25) avg from d64_values")
                .ddl("create table d64_values as (" +
    ...
    public void testAvgDecimalAllNull() throws Exception {
        assertQuery("select avg(x, 1) from (select cast(null as decimal(10,2)) x from long_sequence(1000))")
                .noRandomAccess()
                .expectSize()
                .returns("avg\n\n");
    }
    ...
    public void testAvgDecimalNegativeScale() throws Exception {
        assertQuery("select avg(d, -1) from x")
                .ddl("create table x (d decimal(4,0))")
                .fails(14, "non-negative scale required: -1");
    }
    ...
    public void testAvgDecimalOverflow() throws Exception {
        assertQuery("select avg(d, 0) from x")
                .ddl("create table x as (" +
                        "select cast('9999999999999999999999999999999999999999999999999999999999999999999999999999' as decimal(76,0)) d " +
                        "from long_sequence(10)" +
                        ")")
                .fails(7, "avg aggregation failed: Overflow in addition: result exceeds 256-bit capacity");
    }

    @Test
    public void testAvgDecimalTooLargePrecision() throws Exception {
        assertQuery("select avg(d, 2) from x")
                .ddl("create table x (d decimal(76,0))")
                .fails(14, "rescaled decimal has precision that exceeds maximum of 76: 78");
    }

    @Test
    public void testAvgDecimalTooLargeScale() throws Exception {
        assertQuery("select avg(d, 80) from x")
                .ddl("create table x (d decimal(4,0))")
                .fails(14, "scale exceeds maximum of 76: 80");
    }

    @Test
    public void testAvgDecimalWithRescale() throws Exception {
        assertQuery("select avg(d8,1) a8, avg(d16,2) a16, avg(d32,2) a32, avg(d64,3) a64, avg(d128,4) a128, avg(d256,8) a256 from x")
                .ddl("create table x as (" +
                        "select" +
