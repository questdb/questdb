...
public class CompiledFilterRegressionTest extends AbstractCairoTest {
    ...
    public void testLatestOnFiltersInAllModes() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try {
                execute("CREATE TABLE jit_latest AS (SELECT x id, (x / 3 * 1000000)::TIMESTAMP ts,"
                        ...
                        + " (CASE WHEN x % 7 = 0 THEN from_base64('') ELSE rnd_bin(1,32,3) END) binary_value, rnd_uuid4() u"
                        + ", rnd_boolean() flag, rnd_byte() b, rnd_short() sh, (CASE WHEN x % 2 = 0 THEN 'A' ELSE NULL END)::CHAR ch,"
                        ...
                        + " FROM long_sequence(8193)) TIMESTAMP(ts) PARTITION BY DAY");
    ...
    public void testLatestOnFloatRuntimePairTolerance() throws Exception {
        assertMemoryLeak(() -> {
            ...
            // The first batch retains the generic JIT's existing FLOAT tolerance boundary.
    ...
    public void testLatestOnFloatToleranceBoundConstant() throws Exception {
        assertMemoryLeak(() -> {
            // Both keys fit in the first two-row frame. LATEST must apply the same tolerance as
            // ordinary WHERE even when this first frame runs through the compiled filter.
