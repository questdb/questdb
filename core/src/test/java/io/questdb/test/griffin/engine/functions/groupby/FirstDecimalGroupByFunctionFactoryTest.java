...
public class FirstDecimalGroupByFunctionFactoryTest extends AbstractCairoTest {
    ...
    public void testFirst() throws Exception {
        assertQuery("select first(d8) f8, first(d16) f16, first(d32) f32, first(d64) f64, first(d128) f128, first(d256) f256 from x")
    ...
    public void testFirstAllNull() throws Exception {
        assertQuery("select first(x) from (select cast(null as decimal(10,2)) x from long_sequence(1000))")
                .noRandomAccess()
                .expectSize()
                .returns("first\n\n");
    }
