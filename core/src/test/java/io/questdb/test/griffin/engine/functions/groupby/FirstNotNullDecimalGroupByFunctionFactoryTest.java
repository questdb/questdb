...
public class FirstNotNullDecimalGroupByFunctionFactoryTest extends AbstractCairoTest {
    ...
    public void testFirstNotNull() throws Exception {
        assertQuery("select first_not_null(d8) f8, first_not_null(d16) f16, first_not_null(d32) f32, first_not_null(d64) f64, first_not_null(d128) f128, first_not_null(d256) f256 from x")
    ...
    public void testFirstNotNullAllNull() throws Exception {
        assertQuery("select first_not_null(x) from (select cast(null as decimal(10,2)) x from long_sequence(1000))")
                .noRandomAccess()
                .expectSize()
                .returns("first_not_null\n\n");
    }
