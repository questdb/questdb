...
public class DistinctSymbolTest extends AbstractCairoTest {
    ...
    public void testDistinctSymbolIndexed() throws Exception {
        assertQuery("select DISTINCT(sym) from tab order by 1 LIMIT 10")
                ...
                .expectSize()
                .mutateWith("alter table tab drop partition list '2020-01'")
                .returns(
                        "sym\n" +
                                "2020-01-01\n" +
                                "2020-01-02\n" +
    ...
    public void testDistinctSymbolIndexedWithUpdates() throws Exception {
        assertQuery("select DISTINCT(sym) from tab order by 1 LIMIT 5")
                .ddl("create table tab as (select timestamp_sequence('2020-01-01', 10 * 60 * 1000000L) ts, cast(" +
                        "to_str(timestamp_sequence('2020-01-01', 10 * 60 * 1000000L), 'yyyy-MM-dd')" +
                        " as symbol) sym from long_sequence(10000)), index(sym) timestamp(ts) PARTITION BY MONTH")
                .expectSize()
                .mutateWith("update tab set sym = 'abracadabra' where sym = '2020-01-01'")
                .returns(
                        "sym\n" +
                                "2020-01-01\n" +
                                "2020-01-02\n" +
    ...
    public void testDistinctSymbolNonIndexed() throws Exception {
        assertQuery("select DISTINCT sym from tab order by 1 LIMIT 3")
                .ddl("create table tab as (select timestamp_sequence('2020-01-01', 10 * 60 * 1000000L) ts, cast(" +
                        "to_str(timestamp_sequence('2020-01-01', 10 * 60 * 1000000L), 'yyyy-MM-dd')" +
    ...
    public void testDistinctSymbolNonIndexedFilteredFramed() throws Exception {
        assertQuery("select DISTINCT sym from tab WHERE ts in '2020-03' order by 1 LIMIT 4")
                .ddl("create table tab as (select timestamp_sequence('2020-01-01', 10 * 60 * 1000000L) ts, cast(" +
                        "to_str(timestamp_sequence('2020-01-01', 10 * 60 * 1000000L), 'yyyy-MM-dd')" +
                        " as symbol) sym from long_sequence(10000)) timestamp(ts) PARTITION BY MONTH")
                .expectSize()
    ...
    public void testDistinctSymbolNonIndexedFilteredNonFramed() throws Exception {
        ...
        assertQuery("select DISTINCT sym from tab WHERE ts > '2020-01-15' and left(sym, 7) = '2020-02' order by 1 LIMIT 4")
                .ddl("create table tab as (select timestamp_sequence('2020-01-01', 10 * 60 * 1000000L) ts, cast(" +
                        "to_str(timestamp_sequence('2020-01-01', 10 * 60 * 1000000L), 'yyyy-MM-dd')" +
                        " as symbol) sym from long_sequence(10000)) timestamp(ts) PARTITION BY MONTH")
                .expectSize()
    ...
    public void testDistinctSymbolNonIndexedWithNulls() throws Exception {
        assertQuery("select DISTINCT sym from tab order by 1")
                .ddl("create table tab as (select timestamp_sequence('2020-01-01', 10 * 60 * 1000000L) ts, rnd_symbol('a','b','c',null) sym from long_sequence(10000))" +
    ...
    public void testDistinctSymbolWithAnotherCol() throws Exception {
        assertQuery("select DISTINCT sym, month(ts) from tab order by 1 LIMIT 10")
                .ddl("create table tab as (select timestamp_sequence('2020-01-01', 10 * 60 * 1000000L) ts, cast(" +
                        "to_str(timestamp_sequence('2020-01-01', 10 * 60 * 1000000L), 'yyyy-MM-dd')" +
                        " as symbol) sym from long_sequence(10000)) timestamp(ts) PARTITION BY MONTH")
                .expectSize()
