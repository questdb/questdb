...
public class Base64FunctionFactoryTest extends AbstractFunctionFactoryTest {
    ...
    public void testInvalidLength() throws Exception {
        assertQuery("select base64(rnd_bin(6,6,0), 0)")
                .failsWith("maxLength has to be greater than 0");
    }
    ...
    public void testLengthHardLimit() throws Exception {
        ...
        assertQuery("select x, base64(x, 100) y from t")
                .ddl("create table t as (select rnd_bin(16,16,0) x from long_sequence(5))")
                .expectSize()
                .returns("""
                        x\ty
    ...
    public void testRandomBinSeq() throws Exception {
        assertQuery("select x, base64(x, 100) y from t")
                .ddl("create table t as (select rnd_bin(6,6,1) x from long_sequence(10))")
                .expectSize()
                .returns("""
                        x\ty
