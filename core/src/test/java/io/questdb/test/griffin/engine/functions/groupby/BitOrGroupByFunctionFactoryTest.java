        ...
        try (WorkerPool pool = new WorkerPool(() -> 4)) {
            TestUtils.execute(pool, (engine, compiler, sqlExecutionContext) -> {
                ...
                TestUtils.assertSql(compiler, sqlExecutionContext, "select count() from table_partitions('t')", sink, """
                        ...
                        8
                        """);
                TestUtils.assertSql(compiler, sqlExecutionContext, "select bit_or(v) from t", sink, """
                        bit_or
                        -9223372036854775807
                        """);
                TestUtils.assertSql(compiler, sqlExecutionContext, "select g, bit_or(v) from t order by g", sink, """
                        g\tbit_or
                        a\t-9223372036854775807
                        """);
            }, configuration, LOG);
