...
public class ModeVarcharGroupByFunctionFactoryTest extends AbstractCairoTest {
    ...
    public void testModeAllNull() throws Exception {
        assertQuery("select mode(f) from tab")
                .ddl("create table tab as (select null::varchar as f from long_sequence(5))")
                .noRandomAccess()
                .expectSize()
                .returns("""
                        mode
    ...
    public void testModeEmpty() throws Exception {
        assertQuery("select mode(f) from tab")
                .ddl("create table tab (f varchar)")
                .noRandomAccess()
                .expectSize()
                .returns("""
                        mode
    ...
    public void testModeSingleValue() throws Exception {
        assertQuery("select mode(f) from tab")
                .ddl("create table tab as (select cast('single_varchar' as varchar) as f from long_sequence(1))")
                .noRandomAccess()
                .expectSize()
                .returns("""
                        mode
