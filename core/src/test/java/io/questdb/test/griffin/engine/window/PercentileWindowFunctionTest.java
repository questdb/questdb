...
public class PercentileWindowFunctionTest extends AbstractCairoTest {
    ...
    public void testMultiPercentileContWithNulls() throws Exception {
        assertMemoryLeak(() -> {
            ...
            // 25th percentile: position = 0.25 * 6 = 1.5 → interpolate between index 1 and 2 → 1.5 * (4-2) + 2 = 3.0
            // 50th percentile: position = 0.50 * 6 = 3.0 → index 3 → 5.0
            // 75th percentile: position = 0.75 * 6 = 4.5 → interpolate between index 4 and 5 → 0.5 * (10-8) + 8 = 9.0
            assertSql(
    ...
    public void testMultiPercentileContWithNullsInPartition() throws Exception {
        assertMemoryLeak(() -> {
            ...
            //   25th: 0.25 * 2 = 0.5 → interpolate between 0 and 1 → 3.0
            //   50th: 0.50 * 2 = 1.0 → index 1 → 6.0
            //   75th: 0.75 * 2 = 1.5 → interpolate between 1 and 2 → 7.0
    ...
    public void testMultiPercentileContWithSingleValueInPartition() throws Exception {
        assertMemoryLeak(() -> {
            ...
            // cat=2: values 1,2,3,4,6,7,8,9,10 (9 values)
            assertSql(
