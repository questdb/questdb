...
public class AlterTableDetachPartitionTest extends AbstractAlterTableAttachPartitionTest {
    ...
    /**
     * Attaching a PARQUET partition whose {@code _pm} metadata file is absent must regenerate it,
     * durably, and the rows must come back.
     * <p>
     * This is the SUCCESS path of the block the sibling test below only enters to fail. It is
     * reached ONLY when the partition has {@code data.parquet} but NO {@code _pm} -- a normally
     * converted partition already carries {@code _pm}, so simply attaching one skips the block
     * entirely and covers nothing. Hence the deliberate eviction of {@code _pm} below; it is the
     * precondition of the test, not incidental setup.
     * <p>
     * Both of the block's durability gates -- the {@code _pm} fsync after
     * {@code ParquetMetadataWriter.generate}, and the partition-directory fsync -- are guarded by
     * {@code effectiveCommitMode != CommitMode.NOSYNC}, and CI's merged coverage showed them
     * unreached. They are adaptive-relevant: attach is one-shot structural work that no durable
     * epoch later re-derives, so it stays eager rather than lazy. Running under the suite default
     * (adaptive) makes both gates evaluate true, so the fsyncs are taken rather than skipped.
     */
    @Test
    public void testCanAttachParquetPartitionMissingMetadataFile() throws Exception {
        assertMemoryLeak(() -> {
            ...
            final long before = countRows(tableName);
            Assert.assertTrue("precondition: the table must hold rows to lose", before > 0);
            ...
            final long afterDetach = countRows(tableName);
            Assert.assertTrue("precondition: detaching must actually remove rows, otherwise the"
                    + " re-attach below proves nothing", afterDetach < before);

            renameDetachedToAttachable(tableName, "2022-06-01");

            // Evict _pm, keeping data.parquet: the inverse of the sibling test's eviction, and the
            // only shape that reaches the regenerate-and-fsync block.
            TableToken tableToken = engine.verifyTableName(tableName);
            path.of(configuration.getDbRoot()).concat(tableToken)
                    .concat("2022-06-01").put(configuration.getAttachPartitionSuffix())
                    .concat(PARQUET_METADATA_FILE_NAME).$();
            Assert.assertTrue("precondition: conversion must have produced _pm for us to evict",
                    Files.exists(path.$()));
            Assert.assertTrue(Files.remove(path.$()));

            execute("ALTER TABLE " + tableName + " ATTACH PARTITION LIST '2022-06-01'", sqlExecutionContext);

            // _pm must have been regenerated by the attach, not merely tolerated as absent. The
            // attached directory carries a name-txn suffix, so resolve the path the way the engine
            // does instead of assuming the bare partition name.
            try (TableReader reader = engine.getReader(tableToken)) {
                int parquetPartitions = 0;
                for (int i = 0, n = reader.getPartitionCount(); i < n; i++) {
                    ...
                    TableUtils.setPathForParquetPartitionMetadata(
                            ...
                            PartitionBy.DAY,
                            reader.getTxFile().getPartitionTimestampByIndex(i),
                            reader.getTxFile().getPartitionNameTxn(i)
                    );
                    Assert.assertTrue("attach must regenerate the missing _pm [path=" + path + "]",
                            Files.exists(path.$()));
                }
                Assert.assertEquals("the re-attached partition must still be parquet",
                        1, parquetPartitions);
            }

            Assert.assertEquals("every detached row must come back after re-attaching the parquet"
                    + " partition", before, countRows(tableName));
        });
    }
