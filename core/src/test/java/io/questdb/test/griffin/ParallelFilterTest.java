...
public class ParallelFilterTest extends AbstractCairoTest {
    ...
    private static final String symbolInCursorQueryLimit = "select v, s from x where s::string::symbol in (select rnd_varchar('C', 'B', null)::string from long_sequence(100)) order by v limit 10";
    private static final String symbolStaticInCursorQueryLimit = "select v, s from x where s in (select rnd_varchar('C', 'B')::string from long_sequence(100)) order by v limit 10";
