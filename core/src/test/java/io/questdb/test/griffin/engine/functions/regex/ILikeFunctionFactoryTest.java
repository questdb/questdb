...
public class ILikeFunctionFactoryTest extends AbstractGriffinTest {
    ...
    public void testBindVariableConcatIndexed() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = fact("select * from x where name ilike '%' || $1 || '%'")) {
    ...
    public void testBindVariableConcatNamed() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = fact("select * from x where name ilike '%' || :str || '%'")) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), true, sink);
                    Assert.assertNotEquals(sink.toString().indexOf('H'), -1);
    ...
    public void testEmptyLikeString() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = fact("select * from x where name ilike ''")) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
    ...
    public void testInvalidRegex() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = fact("select * from x where name ilike '[][n'")) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), false, sink);
                    Assert.assertEquals(sink.toString(), "");
    ...
    public void testLikeCharacterNoMatch() throws Exception {
        assertMemoryLeak(() -> {
            ddl("create table x as (select rnd_str() name from long_sequence(2000))");
            try (RecordCursorFactory factory = fact("select * from x where  name ilike 'H'")) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), true, sink);
    ...
    public void testLikeStringCaseInsensitive() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = fact("select * from x where name ilike 'aBcGe'")) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), false, sink);
    ...
    public void testLikeStringNoMatch() throws Exception {
        assertMemoryLeak(() -> {
                    ddl("create table x as (select rnd_str() name from long_sequence(2000))");
                    try (RecordCursorFactory factory = fact("select * from x where name ilike 'XJ'")) {
                        try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                            sink.clear();
                            printer.print(cursor, factory.getMetadata(), true, sink);
    ...
    public void testLikeStringPercentageAtEnd() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = fact("select * from x where name ilike 'AbC%'")) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), false, sink);
                    Assert.assertEquals(sink.toString().replace("\n", ""), "ABCGE");
    ...
    public void testLikeStringPercentageAtStart() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = fact("select * from x where name ilike '%GgG'")) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), false, sink);
                    Assert.assertEquals(sink.toString().replace("\n", ""), "BDGDGGG");
    ...
    public void testLikeStringPercentageAtStartAndEnd() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = fact("select * from x where name ilike '%BcG%'")) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), false, sink);
                    Assert.assertEquals(sink.toString().replace("\n", ""), "ABCGE");
    ...
    public void testLikeStringUnderscoreAndPercentage() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = fact("select * from x where name ilike '_B%'")) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), false, sink);
    ...
    public void testLikeStringUnderscoreAtStartAndEnd() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = fact("select * from x where name ilike '_BC__'")) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), false, sink);
                    Assert.assertEquals(sink.toString().replace("\n", ""), "ABCGE");
    ...
    public void testNonConstantExpression() throws Exception {
        assertMemoryLeak(() -> {
            ddl("create table x as (select rnd_str() name from long_sequence(2000))");
            try {
                fail("select * from x where name ilike rnd_str('foo','bar')");
    ...
    public void testNotLikeCharacterMatch() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = fact("select * from x where not name ilike 'H'")) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), true, sink);
                    Assert.assertNotEquals(sink.toString().indexOf('H'), -1);
    ...
    public void testNotLikeStringMatch() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = fact("select * from x where not name ilike 'XJ'")) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), true, sink);
