...
public class ModeStringGroupByFunctionFactoryTest extends AbstractCairoTest {
    ...
    public void testModeAllNull() throws Exception {
        assertQuery("select mode(f) from tab")
                .ddl("create table tab as (select null::string as f from long_sequence(5))")
                .noRandomAccess()
                .expectSize()
                .returns("""
                        mode
    ...
    public void testModeSingleValue() throws Exception {
        assertQuery("select mode(f) from tab")
                .ddl("create table tab as (select 'world' as f from long_sequence(1))")
                .noRandomAccess()
                .expectSize()
                .returns("""
                        mode
