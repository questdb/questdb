...
public class AsumArrayGroupByFunctionFactoryTest extends AbstractCairoTest {
        ...
        assertMemoryLeak(() -> {
            ...
            assertQueryNoLeakCheck(
                    """
                            ...
                            2\t[90.0,92.0]
                            """,
                    "SELECT grp, asum(arr) arr FROM tab ORDER BY grp",
                    null,
                    true,
                    true
            );
        ...
        assertMemoryLeak(() -> {
            ...
            assertQueryNoLeakCheck(
                    """
                            ...
                            """,
                    "SELECT ts, asum(arr) arr FROM tab SAMPLE BY 1h",
                    "ts",
                    true,
                    true
            );
        ...
        try (WorkerPool pool = new WorkerPool(() -> 4)) {
            TestUtils.execute(
                    ...
                    (engine, compiler, sqlExecutionContext) -> {
                        String sql = "SELECT sym, asum(arr) FROM tab GROUP BY sym ORDER BY sym";
                        ...
                        // Run to verify no crash; exact comparison skipped due to
                        // floating-point non-associativity of parallel summation.
