 ...
 *       failures for out-of-range literals, including a value that does not
 *       fit the target DECIMAL);</li>
 ...
 *   <li>anything else, including a bare {@link NumericException} and any
 *       other {@link CairoException} &rarr; reported as a failure. Queries
 *       generated by this fuzzer should never leak either; if one escapes,
 *       the engine is surfacing an internal error through the query API.
 *       See {@link #isAcceptedSkip}.</li>
 * </ul>
...
public final class QueryRunner {
    ...
    public Result run(GeneratedQuery query) {
        ...
        try {
            ...
            // A re-pass that surfaced a per-row cast error the first pass did
     ...
     * reduce worker. It relaxes oracle 1 for a query that terminates early: under
     * the eager parallel reduce, a fired
     * {@code test_fault()} can land on a page frame whose rows the query never
     * reads, so its throw is discarded on a worker and the query still returns a
    ...
    public Result runFault(GeneratedQuery query, FaultType type, Rnd rnd, boolean parallel) {
        ...
        if (outcome.failure == null) {
            ...
            // frame whose rows the query never reads: the eager reduce throws on a
            // worker, but the frame's result is discarded and the query completes.
    ...
    /**
     * The errors a generated query may legitimately raise. A bare
     * {@link NumericException} is not among them: every decimal operation the
     * generator can emit reports its arithmetic limits as a typed error
     * instead. A cast to DECIMAL rewraps the parser's or the rescaler's
     * failure as an {@link ImplicitCastException}, {@code sum} / {@code avg}
     * raise the {@link CairoException} {@link #isDecimalAggregationOverflow}
     * matches, and a comparison aligns scales without throwing. So a
     * {@code NumericException} that escapes means an operator leaked an
     * internal arithmetic error through the query API.
     */
     ...
     * oracle skips it rather than reporting it as a failure. It is the only
     * overflow {@link #isAcceptedSkip} tolerates.
     * <p>
    ...
    /**
     * One outcome threw an allowlisted cast error, the other succeeded.
    ...
    /**
     * Erroring side threw a per-row runtime cast error, succeeding side
     * reached the index-driven access path. An index probe filters rows out
     * before they reach the residual filter, so a row that would have
     * triggered the cast on the full-scan side never makes it to the
     * comparator on the indexed side. The asymmetry is a planner choice driven
     * by the table's index configuration, not a data divergence.
     */
    ...
    /**
     * Erroring side threw a per-row runtime cast error, succeeding side ran
     * against tables with parquet partitions. Parquet pushdown evaluates
     * predicates against row-group statistics and column dictionaries before
     * materializing rows, so an out-of-range cast that the full-scan side
     * would hit may simply not be reached on the parquet side. The asymmetry
     * is a planner choice driven by the table's storage format, not a data
     * divergence.
     */
    ...
    /**
     * The per-row runtime error a filter expression can raise mid-scan: an
     * out-of-range cast ({@link ImplicitCastException}). Whether it fires
     ...
     * can surface it from a frame past the cutoff on one cursor pass but not
     * another. That is the same access-path-dependent non-determinism the
     * differential oracle already tolerates (see {@link #isIndexFilteredAsymmetry}
     * / {@link #isParquetFilteredAsymmetry}, which key on the same error), so the
     * cursor self-checks treat it, and only it, as a tolerated re-pass throw after
     * a clean first pass.
     * <p>
    ...
    private static boolean isPerRowRuntimeError(Throwable t) {
        return t instanceof ImplicitCastException;
    }
     ...
     * row, so a per-row cast error on an out-of-range value may
     * legitimately not surface here even though the equivalent
    ...
    /**
     * Detects whether the consumption spine of {@code factory} (the root and its
     * base-factory chain) ends in an early-exit non-keyed group by, i.e. one that
     * stops scanning as soon as the aggregate value is final. The optimiser
     * routes {@code count_distinct} over a constant or a fully-enumerated symbol
     * column to {@code GroupByNotKeyedRecordCursorFactory}'s early-exit cursor.
     * <p>
     * Like a pushed limit (see {@link #planHasPushedLimit}), this is a legitimate
     * early termination the swallow oracle must tolerate: the early-exit consumer
     * stops after the first matching row, but the eager parallel reduce beneath it
     * keeps filtering later frames, so a fired {@code test_fault()} can land on a
     * frame whose result the consumer never reads and is discarded on a worker.
     * <p>
     * The check is deliberately narrow to avoid masking a real swallow (a false
     * negative). It matches only {@link GroupByNotKeyedRecordCursorFactory} with
     * its early-exit cursor, and only on the base-factory chain (the top-level
     * consumption spine), not anywhere in the plan as the limit check does, so
     * an early exit buried in a fully-drained subquery is not tolerated. It cannot
     * hide an error on a frame the query actually reads: had the fault landed on a
     * consumed frame the reduce error would surface when the group by pulls it, so
     * {@code outcome.failure} would be non-null and the swallow branch never runs.
     * The fuzzer only injects {@code test_fault()} into the scan a non-keyed group
     * by consumes (a count_distinct query is single-source; see GroupByClause), so
     * a detected early exit always governs the fault-bearing frames.
     */
    static boolean factoryHasEarlyExitGroupBy(RecordCursorFactory factory) {
        while (factory != null) {
            if (factory instanceof GroupByNotKeyedRecordCursorFactory groupBy && groupBy.isEarlyExitSupported()) {
                return true;
            }
            factory = factory.getBaseFactory();
        }
    ...
    private void checkCalculateSize(String sql, RecordCursor cursor, StringSink firstPass, int firstRowsRead) {
        ...
        try {
            ...
            // calculateSize() is normally a defect. A per-row cast error
    ...
    private void checkToTop(String sql, RecordCursor cursor, RecordMetadata metadata, int columnCount,
        ...
        try {
            ...
            // re-iterate). The one exception is a per-row cast error: the
    ...
    private Result reconcilePair(String sql, Outcome a, Outcome b, StringSink rowsA, StringSink rowsB,
        ...
        // cast exception fires on whichever row the iterator visits
        ...
        // One side threw a per-row runtime error (ImplicitCastException for
        // an out-of-range cast) and the other side succeeded with an
        // index-driven access path. An index probe filters rows out
        // before they reach the residual filter, so the offending row
        // simply never reaches the cast on the indexed side.
    ...
    private Outcome runOnce(String sql, StringSink rows, boolean usesParquet, boolean deterministic) {
        ...
        try (RecordCursorFactory factory = engine.select(sql, executionContext)) {
            ...
            // Snapshot the flyweight message now: the thread-local instance is
            // reused and cleared, so it is unreadable by the time the oracle
            // renders the failure.
            return Outcome.error(e, e.getFlyweightMessage().toString(), usesParquet);
        } catch (Exception t) {
    ...
    private static final class CursorCheckException extends RuntimeException {
        ...
        // surfaced a per-row cast error (see isPerRowRuntimeError)
