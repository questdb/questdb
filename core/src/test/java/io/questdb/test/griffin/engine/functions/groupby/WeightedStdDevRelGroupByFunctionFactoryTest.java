...
public class WeightedStdDevRelGroupByFunctionFactoryTest extends AbstractCairoTest {
    ...
    @Test
    public void testWeightedStddevRelSampleByFillNegativeWeight() throws Exception {
        assertMemoryLeak(() -> {
            execute("CREATE TABLE test_fill (value DOUBLE, weight DOUBLE, ts TIMESTAMP) TIMESTAMP(ts)");
            ...
            execute("""
                    ...
                    """);
            assertQuery("SELECT ts, round(weighted_stddev_rel(value, weight), 12) FROM test_fill SAMPLE BY 1h FILL(PREV)")
                    .noLeakCheck()
                    .timestamp("ts")
                    .noRandomAccess()
                    .returns("""
                            ...
                            """);
            assertQuery("SELECT ts, round(weighted_stddev_rel(value, weight), 12) FROM test_fill SAMPLE BY 1h FILL(LINEAR)")
                    .noLeakCheck()
                    .timestamp("ts")
                    .expectSize()
                    .returns("""
                            ...
                            1970-01-01T04:00:00.000000Z\t1.581138830084
                            """);
            assertQuery("SELECT ts, round(weighted_stddev_rel(value, weight), 12) FROM test_fill SAMPLE BY 1h FILL(NULL)")
                    .noLeakCheck()
                    .timestamp("ts")
                    .noRandomAccess()
                    .returns("""
    ...
    public void testWeightedStddevRelZeroAndNegativeWeight() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertQuery("SELECT weighted_stddev_rel(x, CASE WHEN x = 1 THEN -1 ELSE x END) FROM long_sequence(11)")
                    .noLeakCheck()
                    .noRandomAccess()
                    .expectSize()
                    .returns("""
                            ...
                            null
                            """);
            assertQuery("SELECT weighted_stddev(x, CASE WHEN x = 1 THEN -1 ELSE x END) FROM long_sequence(11)")
                    .noLeakCheck()
                    .noRandomAccess()
                    .expectSize()
                    .returns("""
            ...
            assertQuery("""
                    SELECT x % 2 k, weighted_stddev_rel(x, CASE WHEN x = 7 THEN -1 ELSE 1 END)
                    FROM long_sequence(10)
                    ORDER BY k
                    """)
                    .noLeakCheck()
