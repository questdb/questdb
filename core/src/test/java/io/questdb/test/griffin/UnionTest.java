...
public class UnionTest extends AbstractCairoTest {
    ...
    public void testNullConversionsFromFloat() throws Exception {
        assertMemoryLeak(() -> {
            execute("CREATE TABLE foo (ts TIMESTAMP, colA DOUBLE, colB TIMESTAMP, colC DATE, colD LONG, colE INT) timestamp(ts) PARTITION BY DAY WAL;");
            execute("INSERT INTO foo (ts, colA, colB, colC, colD, colE) " +
                    "SELECT '2025-04-09 17:20:00.000' AS ts, " +
    ...
    public void testNullConversionsFromTimestamp() throws Exception {
        assertMemoryLeak(() -> {
            execute("CREATE TABLE foo (ts TIMESTAMP, colA DOUBLE, colB FLOAT, colC DATE, colD LONG, colE INT) timestamp(ts) PARTITION BY DAY WAL;");
            execute("INSERT INTO foo (ts, colA, colB, colC, colD, colE) " +
                    "SELECT '2025-04-09 17:20:00.000' AS ts, " +
    ...
    public void testUnionAllOfAllSupportedTypes() throws Exception {
        assertMemoryLeak(() -> {
            final String expected = "a\tb\tc\td\te\tf\tg\ti\tj\tk\tl\tm\tn\tl256\tchr\n" +
                    "1569490116\tfalse\tZ\tnull\t0.7611\t428\t2015-05-16T20:27:48.158Z\tVTJW\t-8671107786057422727\t1970-01-01T00:00:00.000000Z\t26\t00000000 68 61 26 af 19 c4 95 94 36 53 49\tFOWLPD\t0x5f20a35e80e154f458dfd08eeb9cc39ecec82869edec121bc2593f82b430328d\tE\n" +
                    ...
                    "-2016176825\ttrue\tT\tnull\t0.2357\t813\t2015-12-27T00:19:42.415Z\tCPSW\t3464609208866088600\t1970-01-01T04:10:00.000000Z\t49\t\tFNUHNR\t0x8ca81bb363d7ac4585afb517c8aee023d107b1affff76a2c79189b578191a8f6\tW\n" +
    ...
    public void testUnionAllSupportedTypes() throws Exception {
        assertMemoryLeak(() -> {
            ...
            final String expected2 = "a\tb\tc\td\te\tf\tg\ti\tj\tk\tl\tm\tn\tl256\tchr\n" +
                    ...
                    "-1896190362\tfalse\tL\tnull\t0.3300\t442\t2015-02-06T14:17:40.815Z\tSGQF\t8892131787872519172\t1970-01-01T00:16:40.000000Z\t41\t\t\t0x49e5a0f99d31a104a75dd7280fc9b66b84debde2e574e061463243e5bb8a743d\tK\n" +
