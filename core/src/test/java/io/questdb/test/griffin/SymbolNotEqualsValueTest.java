...
public class SymbolNotEqualsValueTest extends AbstractCairoTest {
    ...
    public void testSymbolNotEqualsWhenNumberOfSymbolsExceedTheConfigMax() throws Exception {
        ...
        assertQuery("select sym k, price, ts from x where sym != 'HBC'")
                ...
                .mutateWith("""
                        insert into x select * from (select rnd_symbol(120, 3, 3, 0) sym,\s
                                rnd_double() price,\s
                                timestamp_sequence(172800000000, 360000000) ts\s
    ...
    public void testSymbolNotPresentAndThenInserted() throws Exception {
        ...
        assertQuery(query)
                .ddl("create table x as (" +
                        ...
                        ") timestamp(ts) partition by DAY")
                .mutateWith("""
                        insert into x select * from (select rnd_symbol('ABC') sym,\s
                                rnd_double() price,\s
                                timestamp_sequence(177800000000, 360000000) ts\s
