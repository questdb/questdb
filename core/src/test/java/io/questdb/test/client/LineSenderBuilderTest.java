...
public class LineSenderBuilderTest extends AbstractLineTcpReceiverTest {
    ...
    @Test
    public void testBinaryTransferWithProtocolVersionThrows() {
        try {
            Sender.builder(Sender.Transport.HTTP)
                    .address("localhost:9000")
                    .binaryTransfer()
                    .protocolVersion(Sender.PROTOCOL_VERSION_V1);
            fail("should not allow binaryTransfer() with protocolVersion()");
        } catch (LineSenderException e) {
            TestUtils.assertContains(e.getMessage(), "cannot set protocolVersion() when binary transfer is enabled");
        }
    }

    @Test
    public void testProtocolVersionWithBinaryTransferThrows() {
        try {
            Sender.builder(Sender.Transport.HTTP)
                    .address("localhost:9000")
                    .protocolVersion(Sender.PROTOCOL_VERSION_V1)
                    .binaryTransfer();
            fail("should not allow protocolVersion() with binaryTransfer()");
        } catch (LineSenderException e) {
            TestUtils.assertContains(e.getMessage(), "cannot use binary transfer with protocolVersion()");
        }
    }

    @Test
    public void testBinaryTransferWithTcpThrows() {
        try {
            Sender.builder(Sender.Transport.TCP)
                    .address("localhost:9009")
                    .binaryTransfer()
                    .build();
            fail("should not allow binaryTransfer() with TCP transport");
        } catch (LineSenderException e) {
            TestUtils.assertContains(e.getMessage(), "binary transfer is only supported for HTTP protocol");
        }
    }

    @Test
    public void testBinaryTransferDoubleSetThrows() {
        try {
            Sender.builder(Sender.Transport.HTTP)
                    .address("localhost:9000")
                    .binaryTransfer()
                    .binaryTransfer();
            fail("should not allow binaryTransfer() to be called twice");
        } catch (LineSenderException e) {
            TestUtils.assertContains(e.getMessage(), "binary transfer was already enabled");
        }
    }

    @Test
    public void testConfigStringBinaryTransferInvalidValue() {
        try {
            Sender.fromConfig("http::addr=localhost:9000;binary_transfer=yes;");
            fail("should not accept invalid binary_transfer value");
        } catch (LineSenderException e) {
            TestUtils.assertContains(e.getMessage(), "invalid binary_transfer");
            TestUtils.assertContains(e.getMessage(), "allowed-values=[on, off]");
        }
    }

    @Test
    public void testConfigStringBinaryTransferOff() {
        // binary_transfer=off should not throw during parsing
        // fromConfig() will try to build() which may fail for other reasons,
        // but it should not fail due to binary_transfer=off parsing
        try {
            Sender.fromConfig("http::addr=localhost:9000;binary_transfer=off;retry_timeout=0").close();
        } catch (LineSenderException e) {
            // May fail at connection time, but not at config parsing
            if (e.getMessage().contains("binary_transfer")) {
                fail("should accept binary_transfer=off: " + e.getMessage());
            }
            // Other failures (like connection refused) are expected
        }
    }
}
