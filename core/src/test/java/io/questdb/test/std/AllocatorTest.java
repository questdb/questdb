...
public class AllocatorTest extends AbstractCairoTest {
    private final AllocatorType allocatorType;

    public AllocatorTest(AllocatorType allocatorType) {
    ...
    public void testCanBeUsedAfterClose() throws Exception {
        ...
        assertMemoryLeak(() -> {
            try (Allocator allocator = createAllocator(config)) {
    ...
    private Allocator createAllocator(CairoConfiguration config) {
        switch (allocatorType) {
            case THREAD_SAFE:
                return AllocatorFactory.createThreadSafeAllocator(config);
            case THREAD_UNSAFE:
                return AllocatorFactory.createThreadUnsafeAllocator(config);
            default:
