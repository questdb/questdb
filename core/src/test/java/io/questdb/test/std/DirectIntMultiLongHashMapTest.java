...
public class DirectIntMultiLongHashMapTest {
    ...
    @Test
    public void testClearAndRestore() throws Exception {
        assertMemoryLeak(() -> {
    ...
    public void testCloseAfterRehashOom() throws Exception {
        assertMemoryLeak(() -> {
            try (DirectIntMultiLongHashMap map = new DirectIntMultiLongHashMap(4, 0.5, Integer.MIN_VALUE, 0, 2, MemoryTag.NATIVE_DEFAULT)) {
                for (int i = 0; i < 3; i++) {
                    long[] values = {(long) i * 10, (long) i * 20};
                    map.putAll(i, values);
                }
                ...
                Assert.assertEquals(40L, map.get(4, 0));
                Assert.assertEquals(80L, map.get(4, 1));
            } finally {
                Unsafe.setRssMemLimit(0);
            }
        });
    ...
    public void testCloseAfterRestoreInitialCapacityOom() throws Exception {
        assertMemoryLeak(() -> {
            DirectIntMultiLongHashMap map = new DirectIntMultiLongHashMap(4, 0.5, Integer.MIN_VALUE, 0, 2, MemoryTag.NATIVE_DEFAULT);
            map.close();
