...
public class NanosFormatCompilerTest {
    ...
    public void testBasicParserCompiler() throws Exception {
        ...
        long millis = fmt.parse("Mon, 08 Apr 2017 PM 11:11:10.123 UTC", CommonUtils.EN_LOCALE);
        sink.clear();
        utc.format(millis, CommonUtils.EN_LOCALE, "Z", sink);
    ...
    private static void assertException(String pattern, String input) {
        ...
        try {
            format.parse(pattern, CommonUtils.EN_LOCALE);
            Assert.fail();
        ...
        try {
            compiled.parse(input, CommonUtils.EN_LOCALE);
            Assert.fail();
        } catch (NumericException ignored) {
        }
    }
    ...
    private void assertFormat(String expected, String pattern, String utcInput) throws NumericException {
        ...
        sink.clear();
        get(pattern).format(nanos, CommonUtils.EN_LOCALE, "GMT", sink);
        TestUtils.assertEqualsIgnoreCase(expected, sink);

        sink.clear();
        compiler.compile(pattern, false).format(nanos, CommonUtils.EN_LOCALE, "GMT", sink);
        TestUtils.assertEqualsIgnoreCase(expected, sink);
    }

    private void assertNanos(String pattern, String expected, String input) throws NumericException {
        sink.clear();
        REFERENCE.format(get(pattern).parse(input, CommonUtils.EN_LOCALE), CommonUtils.EN_LOCALE, "Z", sink);
        TestUtils.assertEquals(expected, sink);

        sink.clear();
        REFERENCE.format(compiler.compile(pattern).parse(input, CommonUtils.EN_LOCALE), CommonUtils.EN_LOCALE, "Z", sink);
    }
    ...
    private void testAgainstJavaReferenceImpl(String pattern) throws Exception {
        ...
        for (long nanos = 0, n = TimeUnit.DAYS.toNanos(1); nanos < n; nanos += step) {
            ...
            genericQuestFmt.format(nanos, CommonUtils.EN_LOCALE, "UTC", sink);
            TestUtils.assertEqualsIgnoreCase(javaFormatted, sink);

            sink.clear();
            compiledQuestFmt.format(nanos, CommonUtils.EN_LOCALE, "UTC", sink);
            TestUtils.assertEqualsIgnoreCase(javaFormatted, sink);

            // now we know both Java and QuestDB format the same way.
            // let's try to parse it back.
            Assert.assertEquals(nanos, genericQuestFmt.parse(sink, CommonUtils.EN_LOCALE));
            Assert.assertEquals(nanos, compiledQuestFmt.parse(sink, CommonUtils.EN_LOCALE));
