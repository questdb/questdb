...
public class NanosFormatCompilerTest {
    ...
    public void testParseUtc() throws Exception {
        assertThat(CommonFormatUtils.UTC_PATTERN, "2011-10-03T00:00:00.000000000Z", "2011-10-03T00:00:00.000Z");
    }
    ...
    private static void assertException(String pattern, String input) {
        ...
        try {
            format.parse(pattern, CommonFormatUtils.EN_LOCALE);
            Assert.fail();
        ...
        try {
            compiled.parse(input, CommonFormatUtils.EN_LOCALE);
            Assert.fail();
        } catch (NumericException ignored) {
        }
    }
    ...
    private void assertFormat(String expected, String pattern, String utcInput) throws NumericException {
        ...
        sink.clear();
        get(pattern).format(nanos, CommonFormatUtils.EN_LOCALE, "GMT", sink);
        TestUtils.assertEqualsIgnoreCase(expected, sink);

        sink.clear();
        compiler.compile(pattern, false).format(nanos, CommonFormatUtils.EN_LOCALE, "GMT", sink);
        TestUtils.assertEqualsIgnoreCase(expected, sink);
    }
    ...
    private void testAgainstJavaReferenceImpl(String pattern) throws Exception {
        ...
        for (long nanos = 0, n = TimeUnit.DAYS.toNanos(1); nanos < n; nanos += step) {
            ...
            genericQuestFmt.format(nanos, CommonFormatUtils.EN_LOCALE, "UTC", sink);
            TestUtils.assertEqualsIgnoreCase(javaFormatted, sink);

            sink.clear();
            compiledQuestFmt.format(nanos, CommonFormatUtils.EN_LOCALE, "UTC", sink);
            TestUtils.assertEqualsIgnoreCase(javaFormatted, sink);

            // now we know both Java and QuestDB format the same way.
            // let's try to parse it back.
            Assert.assertEquals(nanos, genericQuestFmt.parse(sink, CommonFormatUtils.EN_LOCALE));
            Assert.assertEquals(nanos, compiledQuestFmt.parse(sink, CommonFormatUtils.EN_LOCALE));
