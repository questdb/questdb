...
public class DirectCharSequenceIntHashMapTest {
    ...
    public void testCloseAfterRehashOom() throws Exception {
        assertMemoryLeak(() -> {
            // mapCapacity = 16 (MIN_INITIAL_CAPACITY), free = 16
            try (DirectCharSequenceIntHashMap map = new DirectCharSequenceIntHashMap(16, 0.5, -1)) {
                // Insert 15 entries, leaving free = 1.
                for (int i = 0; i < 15; i++) {
                    map.put("key" + i, i);
                }
    ...
    @Test
    public void testCloseAfterWriteKeyOom() throws Exception {
        assertMemoryLeak(() -> {
            // Use avgKeySize=1 so the kv buffer is small (128 bytes after constructor clear).
            try (DirectCharSequenceIntHashMap map = new DirectCharSequenceIntHashMap(16, 0.5, -1, 1)) {
                // Insert 10 short keys ("k0"-"k9", 2 chars each, 12 bytes per entry).
                // After 10 entries: currentOffset=30, kv usage = 120 bytes, kvCapacity = 128.
                for (int i = 0; i < 10; i++) {
                    map.put("k" + i, i);
                }

                // Force OOM so the writeKey realloc fails.
                Unsafe.setRssMemLimit(Unsafe.getRssMemUsed());
                try {
                    // This 3-char key needs 16 bytes. 120 + 16 = 136 > 128, triggers kv realloc.
                    map.put("k10", 10);
                    Assert.fail("Expected CairoException");
                } catch (CairoException e) {
                    Assert.assertTrue(e.isOutOfMemory());
                ...
                // Existing entries must still be accessible.
                for (int i = 0; i < 10; i++) {
    ...
    @Test
    public void testCopyToFuzz() throws Exception {
        assertMemoryLeak(() -> {
    ...
    @Test
    public void testInvalidLoadFactor() throws Exception {
        assertMemoryLeak(() -> {
    ...
    @Test
    public void testKeyValueBufferReallocation() throws Exception {
        assertMemoryLeak(() -> {
    ...
    @Test
    public void testOffsetsIterateInInsertionOrder() throws Exception {
        assertMemoryLeak(() -> {
            try (DirectCharSequenceIntHashMap map = new DirectCharSequenceIntHashMap()) {
                map.put("a", 10);
                map.put("bc", 20);
    ...
    @Test
    public void testOverwriteValueKeepsSize() throws Exception {
        assertMemoryLeak(() -> {
            try (DirectCharSequenceIntHashMap map = new DirectCharSequenceIntHashMap()) {
                map.put("dup", 1);
                Assert.assertEquals(1, map.size());
    ...
    @Test
    public void testPutAtUsesRawIndex() throws Exception {
        assertMemoryLeak(() -> {
            try (DirectCharSequenceIntHashMap map = new DirectCharSequenceIntHashMap()) {
    ...
    @Test
    public void testRandomisedInsertUpdateAndDeleteFuzz() throws Exception {
        assertMemoryLeak(() -> {
            try (DirectCharSequenceIntHashMap map = new DirectCharSequenceIntHashMap(64, 0.5, DirectCharSequenceIntHashMap.NO_ENTRY_VALUE)) {
    ...
    @Test
    public void testRehashPreservesEntries() throws Exception {
        assertMemoryLeak(() -> {
    ...
    @Test
    public void testValueAtMatchesStoredValues() throws Exception {
        assertMemoryLeak(() -> {
            try (DirectCharSequenceIntHashMap map = new DirectCharSequenceIntHashMap()) {
                map.put("alpha", 10);
                map.put("beta", 20);
