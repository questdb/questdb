...
public class WalPurgeJobTest extends AbstractCairoTest {
    ...
    public void testDirectorySequencerRace() throws Exception {
        ...
        assertMemoryLeak(() -> {
            ...
            try (WalWriter walWriter1 = getWalWriter(tableName)) {
                ...
                TableWriter.Row row2 = walWriter1.newRow(TimestampUtils.parseFloorPartialTimestamp("2022-02-25"));
                row2.putLong(0, 2);
                row2.putInt(2, 2);
                row2.append();
                ...
                try (WalWriter walWriter2 = getWalWriter(tableName)) {
                    ...
                    TableWriter.Row row3 = walWriter2.newRow(TimestampUtils.parseFloorPartialTimestamp("2022-02-26"));
                    row3.putLong(0, 3);
                    row3.putInt(2, 3);
                    row3.append();
                    walWriter2.commit();
                    ...
                    TableWriter.Row row4 = walWriter2.newRow(TimestampUtils.parseFloorPartialTimestamp("2022-02-27"));
                    row4.putLong(0, 4);
                    row4.putInt(2, 4);
                    row4.putInt(3, 4);
                    row4.append();
    ...
    public void testRollback() throws Exception {
        ...
        try (WalWriter walWriter1 = getWalWriter(tableName)) {
            ...
            TableWriter.Row row = walWriter1.newRow(TimestampUtils.parseFloorPartialTimestamp("2022-02-25"));
            row.putLong(0, 2);
            row.putLong(2, 2);
            row.append();
            walWriter1.rollback();
    ...
    public void testSegmentLockedWhenSweeping() throws Exception {
        ...
        assertMemoryLeak(testFF, () -> {
            ...
            try (WalWriter walWriter1 = getWalWriter(tableName)) {
                TableWriter.Row row = walWriter1.newRow(TimestampUtils.parseFloorPartialTimestamp("2022-02-24"));
                row.putLong(0, 11);
                row.append();
    ...
    private static void addColumnAndRow(WalWriter writer, String columnName) {
        TestUtils.unchecked(() -> {
            ...
            TableWriter.Row row = writer.newRow(TimestampUtils.parseFloorPartialTimestamp("2022-02-25"));
            row.putLong(0, 2);
            row.putInt(2, 2);
            row.append();
            writer.commit();
