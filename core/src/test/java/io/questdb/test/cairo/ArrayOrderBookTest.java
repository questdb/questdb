...
public class ArrayOrderBookTest extends AbstractCairoTest {
    ...
    public void testBidAskSpread() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertQuery("SELECT second(ts), round(asks[1][1] - bids[1][1], 2) spread FROM order_book")
                    .noLeakCheck()
                    .expectSize()
                    .returns("""
                            second\tspread
    ...
    public void testImbalanceAtTopLevel() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertQuery("SELECT round(bids[2, 1] / asks[2, 1], 2) imbalance FROM order_book")
                    .noLeakCheck()
                    .expectSize()
                    .returns("imbalance\n2.0\n0.93\n");
        });
    ...
    public void testVolumeAvailableUpToGivenPrice() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertQuery("SELECT asks[1,1] + 0.1 inflated_top FROM order_book")
                    .noLeakCheck()
                    .expectSize()
                    .returns("inflated_top\n10.1\n10.1\n");
            assertQuery("SELECT insertion_point(asks[1], asks[1,1] + 0.1) FROM order_book")
                    .noLeakCheck()
                    .expectSize()
    ...
    public void testVolumeAvailableWithin1Percent() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertQuery("SELECT 1.01 * asks[1,1] inflated_top FROM order_book")
                    .noLeakCheck()
                    .expectSize()
                    .returns("inflated_top\n10.1\n20.2\n");
            assertQuery("SELECT insertion_point(asks[1], 1.01 * asks[1,1]) FROM order_book")
                    .noLeakCheck()
                    .expectSize()
                    .returns("insertion_point\n5\n7\n");
