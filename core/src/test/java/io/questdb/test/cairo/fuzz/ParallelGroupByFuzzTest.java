...
public class ParallelGroupByFuzzTest extends AbstractCairoTest {
    ...
    @Test
    public void testParallelRegressionSlopeSparseNulls() throws Exception {
        // Reproducer for NaN-poisoning in the Welford parallel merge of regression
        // aggregates. When two worker partials both have count = 0 and isNew = false
        // (which happens when a worker's frames contain only non-finite (y, x) pairs),
        // the Chan merge divides 0/0 and poisons every double slot of the destination
        // value with NaN. A subsequent merge with a real-data worker propagates the NaN
        // even though the merged count is now > 0, so getDouble returns NULL instead of
        // the correct slope computed from the non-NULL subset.
        //
        // Data: 4000 rows; rows 1500..1509 carry a perfect y = 2x + 5 (slope = 2.0),
        // every other row is NULL. With pageFrameMaxRows ~ 100 the real rows fall into
        // one frame and the other 39 frames are all-NULL. With 4 parallel workers only
        // one slot accumulates real data; worker assignment is non-deterministic but the
        // bug fires whenever the real-data worker lands at slot >= 2 (about 50% of runs).
        // The loop runs the query multiple times to drive the catch probability above 99%.
        //
        // The same merge pattern (and the same bug) exists on master in corr/covar/
        // stddev/var; this test pins the regression family because the new shared base
        // makes the fix local.
        //
        // The second query in the loop guards the inverse case: with ZERO real rows the
        // result must still be NULL, confirming the merge guards do not over-suppress.
        Assume.assumeTrue(enableParallelGroupBy);
        Assume.assumeFalse(convertToParquet);
        ...
        assertMemoryLeak(() -> {
            ...
            TestUtils.execute(
                    ...
                    (engine, compiler, sqlExecutionContext) -> {
                        ...
                        execute(
                                compiler,
                                "create table tbl_all_null as (" +
                                        "select cast(null as double) x, cast(null as double) y from long_sequence(4000))",
                                sqlExecutionContext
                        );
                        for (int run = 0; run < 8; run++) {
                            TestUtils.assertSql(
                                    engine,
                                    sqlExecutionContext,
                                    "select regr_slope(y, x) from tbl",
                                    sink,
                                    """
                                            regr_slope
                                            2.0
                            ...
                            TestUtils.assertSql(
                                    engine,
                                    sqlExecutionContext,
                                    "select regr_slope(y, x) from tbl_all_null",
                                    sink,
                                    """
                                            regr_slope
                                            null
