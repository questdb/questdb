...
public class CompositeColumnDdlSurveyTest extends AbstractCompositeTwinTest {
     ...
     */
    @Ignore("SP2 SURVEY -- run by temporarily lifting the writer-side gate, then restoring it."
            + " Results 2026-08-18: ADD INDEX is a SILENT NO-OP, DROP INDEX unverified, RENAME COLUMN"
            + " and ALTER COLUMN TYPE fail. DROP COLUMN's per-cell file leak is FIXED by SP2A and this"
            + " test now passes with its on-disk assertion; its gate stays until the rest of the"
            + " sub-project lands.")
    @Test(timeout = 60_000)
