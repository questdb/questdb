...
public class CheckWalTransactionsJobTest extends AbstractGriffinTest {
    ...
    public void testRespectsGivenTablesToCheck() throws Exception {
        assertMemoryLeak(() -> {
            ...
            compile("create table " + table1 + " (x long, ts timestamp) timestamp(ts) partition by DAY WAL");
            compile("create table " + table2 + " (x long, ts timestamp) timestamp(ts) partition by DAY WAL");

            executeInsert("insert into " + table1 + " values (1, 1000)");
            executeInsert("insert into " + table2 + " values (2, 2000)");
