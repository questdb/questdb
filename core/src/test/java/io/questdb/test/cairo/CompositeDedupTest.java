...
public class CompositeDedupTest extends AbstractCompositeTwinTest {
    ...
    @Ignore("ROOT CAUSE FOUND 2026-08-20 -- the dedup MERGE INDEX references a row that does not exist."
            ...
            + " ENCODING CONFIRMED from core/src/main/c/share/dedup.cpp (mergeDedupTimestampWithLongIndexAsc):"
            + "   (*dest).i = src_pos | (1ull << 63)  -- high bit SET means existing COLUMN data"
            + "   *dest++   = index[index_pos]        -- high bit CLEAR means O3 data, copied VERBATIM"
            + " e0row=1 has the high bit CLEAR, so it is an O3 reference to row 1 -- and this commit has"
            + " ONE o3 row, index 0. The entry is copied verbatim from sortedTimestampsAddr, whose"
            + " entries carry each row's ABSOLUTE position in the original O3 batch."
            + " THIS IS THE SAME BUG CLASS ALREADY FIXED FOR THE MULTI-CELL SCRATCH PATH -- see the"
            + " comment in TableWriter#processO3BlockComposite about the scratch ts-index storing"
            + " absolute batch positions that the native merge read back as scratch-column positions,"
            + " landing on a later or out-of-bounds row. The single-cell dedup dispatch has the same"
            + " absolute-vs-relative mismatch: it passes the ORIGINAL sortedTimestampsAddr with"
            + " srcOooLo/srcOooHi selecting a range, so the indices inside are batch-absolute while the"
            + " copy treats them as relative. FIX THERE."
