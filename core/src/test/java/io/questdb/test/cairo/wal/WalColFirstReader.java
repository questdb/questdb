...
public class WalColFirstReader implements Closeable {
    private static final Log LOG = LogFactory.getLog(WalColFirstReader.class);
    private final int columnCount;
    private final ObjList<MemoryMR> columns;
    private final WalColFirstCursor dataCursor = new WalColFirstCursor();
    ...
    public WalColFirstReader(CairoConfiguration configuration, TableToken tableToken, CharSequence walName, int segmentId, long rowCount) {
    ...
    public WalColFirstCursor getCursor() {
        dataCursor.toTop();
        return dataCursor;
    }
    ...
    private void openSymbolMaps(WalEventCursor eventCursor, CairoConfiguration configuration) {
        while (eventCursor.hasNext()) {
            if (eventCursor.getType() == COL_FIRST_DATA) {
