...
public class CreateMatViewTest extends AbstractCairoTest {
    ...
    public void testCreateDropConcurrent() throws Exception {
        assertMemoryLeak(() -> {
            ...
            final Thread creator = new Thread(() -> {
                try (MatViewRefreshJob refreshJob = createMatViewRefreshJob(engine); SqlExecutionContext executionContext = TestUtils.createSqlExecutionCtx(engine)) {
                    barrier.await();
                    for (int i = 0; i < iterations; i++) {
                        // Use test alloc() function to make sure that we always free the factory.
                        execute("create materialized view if not exists price_1h as (" + "  select sym, alloc(42), last(price) as price, ts from base_price sample by 1h" + ") partition by DAY", executionContext);
    ...
    public void testCreateImmediateMatViewModelToSink() throws Exception {
        ...
        final DdlSerializationTest[] tests = new DdlSerializationTest[]{new DdlSerializationTest("create materialized view test as (" + query + "), index (k capacity 1024) partition by day ttl 3 days" + (Os.isWindows() ? "" : " in volume vol1"), "create materialized view test with base " + TABLE1 + " refresh immediate as (" + "select-choose ts, k, avg(v) avg from (table1 sample by 45s align to calendar with offset '00:00')" + "), index(k capacity 1024) partition by DAY TTL 3 DAYS" + (Os.isWindows() ? "" : " in volume 'vol1'")), new DdlSerializationTest("create materialized view test refresh immediate deferred as (" + query + "), index (k capacity 1024) partition by day ttl 3 days" + (Os.isWindows() ? "" : " in volume vol1"), "create materialized view test with base " + TABLE1 + " refresh immediate deferred as (" + "select-choose ts, k, avg(v) avg from (table1 sample by 45s align to calendar with offset '00:00')" + "), index(k capacity 1024) partition by DAY TTL 3 DAYS" + (Os.isWindows() ? "" : " in volume 'vol1'")),};
    ...
    public void testCreateManualMatViewModelToSink() throws Exception {
        ...
        final DdlSerializationTest[] tests = new DdlSerializationTest[]{new DdlSerializationTest("create materialized view test refresh manual as " + query, "create materialized view test with base " + TABLE1 + " refresh manual as (" + "select-choose ts, k, max(v) max from (table1 sample by 30s align to calendar with offset '00:00')" + ")"), new DdlSerializationTest("create materialized view test refresh manual deferred as " + query, "create materialized view test with base " + TABLE1 + " refresh manual deferred as (" + "select-choose ts, k, max(v) max from (table1 sample by 30s align to calendar with offset '00:00')" + ")"),};
        testModelToSink(tests);
    }
    ...
    public void testCreateMatViewAsExpectedPosition() throws Exception {
        assertMemoryLeak(() -> {
            assertExceptionNoLeakCheck("create materialized view test select as sym", 30, "'refresh' or 'as' expected");
    ...
    public void testCreateMatViewBaseTableDoesNotExist() throws Exception {
        assertException("create materialized view testView as (select ts, avg(v) from " + TABLE1 + " sample by 30s) partition by day", 61, "table does not exist [table=" + TABLE1 + "]");
    ...
    public void testCreateMatViewBaseTableNoReference() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test with base " + TABLE1 + " as (" + sql + ") partition by day", 40, "base table is not referenced in materialized view query");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewBasedOnView() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test with base " + VIEW1 + " as (" + sql + ") partition by day", 40, "base table should be a physical table, cannot be a view: view1");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewBasedOnViewReferencesMultipleTables() throws Exception {
        assertMemoryLeak(() -> {
            ...
            // error expected
            assertExceptionNoLeakCheck("create materialized view test as (select ts, min(v) from " + VIEW1 + " sample by 1h) partition by day", 34, "query references multiple tables (views are expanded to their underlying physical tables), use 'WITH BASE' to explicitly select the base table");
            assertNull(getMatViewDefinition("test"));
            ...
            assertExceptionNoLeakCheck("create materialized view test with base " + VIEW1 + " as (select ts, min(v) from " + VIEW1 + " sample by 1h) partition by day", 40, "base table should be a physical table, cannot be a view: view1");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewDisabled() throws Exception {
        assertMemoryLeak(() -> {
            ...
            createTable(TABLE1);
            assertExceptionNoLeakCheck("create materialized view test as (select ts, avg(v) from " + TABLE1 + " sample by 30s) partition by day", 0, "materialized views are disabled");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewFailPeriodIntervalCombinedWithOffset() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test refresh period(sample by interval) as (" + query + ") partition by day", 40, "PERIOD (SAMPLE BY INTERVAL) can't be used with WITH OFFSET");
        });
    }
    ...
    public void testCreateMatViewFailsOnUnionWithNonBaseTable() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test with base " + TABLE1 + " as (" + "  with b as (" + "    select ts, v from " + TABLE2 + "    union all" + "    select ts, v from " + TABLE1 + "  )" + "  select a.ts, avg(a.v)" + "  from " + TABLE1 + " a " + "  left outer join b on a.ts = b.ts" + "  sample by 1d" + ") partition by day", 106, "union on base table is not supported for materialized views: " + TABLE1);
        });
    }
    ...
    public void testCreateMatViewGroupByNoTimestamp() throws Exception {
        assertMemoryLeak(() -> {
            createTable(TABLE1);
            assertExceptionNoLeakCheck("create materialized view test as (select avg(v) from " + TABLE1 + ") partition by day", 34, "TIMESTAMP column is not present in select list");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewGroupByTimestamp3() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test as (" + query + ") partition by day", 34, "TIMESTAMP column does not exist or not present in select list [name=ts]");
            assertNull(getMatViewDefinition("testView"));
        });
    }
    ...
    public void testCreateMatViewInvalidColumn() throws Exception {
        assertMemoryLeak(() -> {
            createTable(TABLE1);
            assertExceptionNoLeakCheck("create materialized view test as (select ts, avg(no_col_like_this) from " + TABLE1 + " sample by 30s) partition by DAY", 49, "Invalid column: no_col_like_this");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewInvalidPartitionBy() throws Exception {
        assertMemoryLeak(() -> {
            createTable(TABLE1);
            assertExceptionNoLeakCheck("create materialized view test as (select ts, avg(v) from " + TABLE1 + " sample by 30s) partition by 3d", 92, "'HOUR', 'DAY', 'WEEK', 'MONTH' or 'YEAR' expected");
            assertNull(getMatViewDefinition("test"));

            assertExceptionNoLeakCheck("create materialized view test as (select ts, avg(v) from " + TABLE1 + " sample by 30s) partition by NONE", 92, "materialized view has to be partitioned");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewInvalidTimestamp() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view testView as (" + query + ") timestamp(k) partition by week", 96, "TIMESTAMP column expected [actual=SYMBOL]");
            assertNull(getMatViewDefinition("testView"));
        });
    }
    ...
    public void testCreateMatViewInvalidTtl() throws Exception {
        assertMemoryLeak(() -> {
            createTable(TABLE1);
            assertExceptionNoLeakCheck("create materialized view test as (select ts, avg(v) from " + TABLE1 + " sample by 30s) partition by day ttl 12 hours", 100, "TTL value must be an integer multiple of partition size");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewMultipleTables() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test as (select t1.ts, avg(t1.v) from " + TABLE1 + " as t1 " + "join " + TABLE2 + " as t2 on v sample by 30s) partition by day", 34, "query references multiple tables (views are expanded to their underlying physical tables), use 'WITH BASE' to explicitly select the base table");
            assertNull(getMatViewDefinition("test"));

            assertExceptionNoLeakCheck("create materialized view test as (select ts, avg(v) from " + TABLE3 + " sample by 30s " + "union select ts, avg(v) from " + TABLE1 + " sample by 30s) partition by day", 34, "query references multiple tables (views are expanded to their underlying physical tables), use 'WITH BASE' to explicitly select the base table");
            assertNull(getMatViewDefinition("test"));

            assertExceptionNoLeakCheck("create materialized view test as (select ts, avg(v) from " + TABLE3 + " sample by 30s " + "union select t1.ts, avg(t1.v) from " + TABLE1 + " as t1 join " + TABLE2 + " as t2 on v sample by 30s) partition by day", 34, "query references multiple tables (views are expanded to their underlying physical tables), use 'WITH BASE' to explicitly select the base table");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewNoAggrFunction() throws Exception {
        assertMemoryLeak(() -> {
            createTable(TABLE1);
            assertExceptionNoLeakCheck("create materialized view test as (select ts from " + TABLE1 + " sample by 10s) partition by day", 66, "at least one aggregation function must be present in 'select' clause");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewNoDesignatedTimestamp() throws Exception {
        assertMemoryLeak(() -> {
            execute("create table stocks_d1_ohlcv (" + "ts timestamp, ticker symbol, close double" + ") timestamp(ts) partition by day WAL");
            ...
            final String query = """
                      ...
                      ORDER BY ticker, ts
                    """;
            assertExceptionNoLeakCheck("create materialized view test_view as (" + query + ") partition by month", 39, "materialized view query is required to have designated timestamp");
            assertNull(getMatViewDefinition("testView"));
        });
    }
    ...
    public void testCreateMatViewNoPartitionByInvalidTtl() throws Exception {
        assertMemoryLeak(() -> {
            createTable(TABLE1);
            assertExceptionNoLeakCheck("create materialized view test as (select ts, avg(v) from " + TABLE1 + " sample by 30s) ttl 12 hours", 83, "TTL value must be an integer multiple of partition size");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewNoSampleBy() throws Exception {
        assertMemoryLeak(() -> {
            createTable(TABLE1);
            assertExceptionNoLeakCheck("create materialized view test as (select * from " + TABLE1 + " where v % 2 = 0) partition by day", 34, "TIMESTAMP column is not present in select list");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewNoTables() throws Exception {
        assertMemoryLeak(() -> {
            assertExceptionNoLeakCheck("create materialized view test as (select 1 from long_sequence(1)) partition by day", 34, "missing base table, materialized views have to be based on a table");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewNoTimestampInSelect() throws Exception {
        assertMemoryLeak(() -> {
            createTable(TABLE1);
            assertExceptionNoLeakCheck("create materialized view test as (select k, max(v) as v_max from " + TABLE1 + " sample by 30s) partition by day", 34, "TIMESTAMP column does not exist or not present in select list [name=ts]");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewNonDeterministicFunction() throws Exception {
        assertMemoryLeak(() -> {
            ...
            // nested in SELECT
            assertExceptionNoLeakCheck("create materialized view test as select coalesce(ts, now()) ts, avg(v) from " + TABLE1 + " sample by 30s", 53, "non-deterministic function cannot be used in materialized view: now");
            assertNull(getMatViewDefinition("test"));

            // WHERE clause
            assertExceptionNoLeakCheck("create materialized view test as (select ts, avg(v) from " + TABLE1 + " where ts in today() sample by 30s) partition by month", 76, "non-deterministic function cannot be used in materialized view: today");
            assertNull(getMatViewDefinition("test"));

            assertExceptionNoLeakCheck("create materialized view test as select ts, avg(v) from " + TABLE1 + " where k is not null and now() = '2020-01-01' sample by 30s", 87, "non-deterministic function cannot be used in materialized view: now");
            assertNull(getMatViewDefinition("test"));

            // IN (cursor)
            assertExceptionNoLeakCheck("create materialized view test as select ts, avg(v) from " + TABLE1 + " where ts in (select now()) sample by 30s", 83, "non-deterministic function cannot be used in materialized view: now");
            assertNull(getMatViewDefinition("test"));

            // JOIN
            assertExceptionNoLeakCheck("create materialized view test with base " + TABLE1 + " as select t1.ts, max(t2.dt) from " + TABLE1 + " t1 " + "left outer join (select k, sysdate() dt from " + TABLE2 + ") t2 on (k) sample by 30s", 117, "non-deterministic function cannot be used in materialized view: sysdate");
            assertNull(getMatViewDefinition("test"));

            // UNION
            assertExceptionNoLeakCheck("create materialized view test with base " + TABLE1 + " as select t1.ts, max(t2.ts) from " + TABLE1 + " t1 " + "left outer join (select 'a' k, '2020-01-01' ts union all select 'b' k, systimestamp() ts) t2 on (k) sample by 30s", 161, "non-deterministic function cannot be used in materialized view: systimestamp");
            assertNull(getMatViewDefinition("test"));

            // ORDER BY
            assertExceptionNoLeakCheck("create materialized view test as (select ts, avg(v) from " + TABLE1 + " sample by 30s order by rnd_int(), ts)", 87, "non-deterministic function cannot be used in materialized view: rnd_int");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewNonDeterministicFunctionInSelect() throws Exception {
        final String[][] functions = new String[][]{{"sysdate()", "sysdate"}, {"systimestamp()", "systimestamp"}, {"today()", "today"}, {"yesterday()", "yesterday"}, {"tomorrow()", "tomorrow"}, {"rnd_bin()", "rnd_bin"}, {"rnd_bin(4,4,4)", "rnd_bin"}, {"rnd_byte()", "rnd_byte"}, {"rnd_byte(1,4)", "rnd_byte"}, {"rnd_boolean()", "rnd_boolean"}, {"rnd_char()", "rnd_char"}, {"rnd_date()", "rnd_date"}, {"rnd_date(1,4,5)", "rnd_date"}, {"rnd_double()", "rnd_double"}, {"rnd_double(5)", "rnd_double"}, {"rnd_float()", "rnd_float"}, {"rnd_float(5)", "rnd_float"}, {"rnd_int()", "rnd_int"}, {"rnd_int(1,4,5)", "rnd_int"}, {"rnd_short()", "rnd_short"}, {"rnd_short(1,5)", "rnd_short"}, {"rnd_long()", "rnd_long"}, {"rnd_long(1,4,5)", "rnd_long"}, {"rnd_long256()", "rnd_long256"}, {"rnd_long256(3)", "rnd_long256"}, {"rnd_ipv4()", "rnd_ipv4"}, {"rnd_ipv4('2.2.2.2/16', 2)", "rnd_ipv4"}, {"rnd_str(1,4,5)", "rnd_str"}, {"rnd_str(1,4,5,6)", "rnd_str"}, {"rnd_str('abc','def','hij')", "rnd_str"}, {"rnd_varchar(1,4,5)", "rnd_varchar"}, {"rnd_varchar('abc','def','hij')", "rnd_varchar"}, {"rnd_symbol(1,4,5,6)", "rnd_symbol"}, {"rnd_symbol('abc','def','hij')", "rnd_symbol"}, {"rnd_timestamp(to_timestamp('2024-03-01', 'yyyy-mm-dd'), to_timestamp('2024-04-01', 'yyyy-mm-dd'), 0)", "rnd_timestamp"}, {"rnd_uuid4()", "rnd_uuid4"}, {"rnd_uuid4(5)", "rnd_uuid4"}, {"rnd_geohash(5)", "rnd_geohash"}};
    ...
    public void testCreateMatViewNonOptimizedSampleBy() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertQuery("""
                    ...
                    """, "select id,table_name,designatedTimestamp,partitionBy,maxUncommittedRows,o3MaxLag,walEnabled,directoryName,dedup,ttlValue,ttlUnit,table_type from tables()", false, true);
        });
    }
    ...
    public void testCreateMatViewNonWalBaseTable() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test as (select ts, avg(v) from " + TABLE1 + " sample by 30s) partition by day", 57, "base table has to be WAL enabled");
            assertNull(getMatViewDefinition("test"));
        });
    }
    ...
    public void testCreateMatViewSampleByAlignToFirstObservation() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test as (" + query + ") partition by day", 77, "ALIGN TO FIRST OBSERVATION on base table is not supported for materialized views: " + TABLE1);
        });
    }
    ...
    public void testCreateMatViewSampleByFill() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test as (" + "  select ts, avg(v) from " + TABLE1 + " where ts in '2024' sample by 1d fill(null)" + ") partition by day", 103, "FILL on base table is not supported for materialized views: " + TABLE1);
            assertExceptionNoLeakCheck("create materialized view test as (" + "  with b as (" + "    select ts, avg(v) from " + TABLE1 + " sample by 1d fill(null)" + "  )" + "  select a.ts, avg(a.v)" + "  from " + TABLE1 + " a " + "  left outer join b on a.ts = b.ts" + "  sample by 1d" + ") partition by day", 99, "FILL on base table is not supported for materialized views: " + TABLE1);
        });
    }
    ...
    public void testCreateMatViewSampleByFromTo() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test as (" + from + ") partition by day", 101, "FROM-TO on base table is not supported for materialized views: " + TABLE2);
            assertExceptionNoLeakCheck("create materialized view test as (" + to + ") partition by day", 99, "FROM-TO on base table is not supported for materialized views: " + TABLE2);

            assertExceptionNoLeakCheck("create materialized view test as (" + "  with b as (" + "    select ts, avg(v) from " + TABLE2 + " sample by 1d from '2024-03-01' to '2024-06-30'" + "  )" + "  select a.ts, avg(a.v)" + "  from " + TABLE2 + " a " + "  left outer join b on a.ts = b.ts" + "  sample by 1d" + ") partition by day", 99, "FROM-TO on base table is not supported for materialized views: " + TABLE2);
        });
    }
    ...
    public void testCreateMatViewSampleByNestedFill() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test as (" + fill + ") partition by day", 94, "FILL on base table is not supported for materialized views: " + TABLE2);
        });
    }
    ...
    public void testCreateMatViewSampleByNestedFromTo() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test as (" + from + ") partition by day", 93, "FROM-TO on base table is not supported for materialized views: " + TABLE2);
            assertExceptionNoLeakCheck("create materialized view test as (" + to + ") partition by day", 91, "FROM-TO on base table is not supported for materialized views: " + TABLE2);
        });
    }
    ...
    public void testCreateMatViewWindowFunctions() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view test as (" + query + ") partition by week", 45, "window function on base table is not supported for materialized views: " + TABLE1);
            assertExceptionNoLeakCheck("create materialized view test as (" + "  with b as (" + "    select ts, avg(v) over (order by ts) as v_max from " + TABLE1 + " sample by 30s" + "  )" + "  select a.ts, avg(a.v)" + "  from " + TABLE1 + " a " + "  left outer join b on a.ts = b.ts" + "  sample by 1d" + ") partition by day", 62, "window function on base table is not supported for materialized views: " + TABLE1);
        });
    }
    ...
    public void testCreateMatViewWithExistingTableName() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("create materialized view " + TABLE2 + " as (select ts, avg(v) from " + TABLE1 + " sample by 30s) partition by day", 25, "table or view with the requested name already exists");

            assertExceptionNoLeakCheck("create materialized view if not exists " + TABLE2 + " as (select ts, avg(v) from " + TABLE1 + " sample by 30s) partition by day", 39, "table or view with the requested name already exists");
            ...
            assertExceptionNoLeakCheck("create materialized view test as (select ts, avg(v) from " + TABLE1 + " sample by 30s) partition by day", 25, "view already exists");
            ...
            assertExceptionNoLeakCheck("create table test(ts timestamp, col varchar) timestamp(ts) partition by day wal", 13, "materialized view with the requested name already exists");
        });
    }
    ...
    public void testCreateMatViewWithInvalidVolume() throws Exception {
        ...
        assertMemoryLeak(() -> {
            createTable(TABLE1);
            try {
                execute("CREATE MATERIALIZED VIEW testView AS (SELECT ts, avg(v) FROM " + TABLE1 + " SAMPLE BY 30s) PARTITION BY DAY IN VOLUME aaa");
    ...
    public void testCreatePeriodMatView1() throws Exception {
        assertMemoryLeak(() -> {
            ...
            final String query = "select ts, k, max(v) as v_max from " + TABLE1 + " sample by 30s";
            execute("CREATE MATERIALIZED VIEW test REFRESH EVERY 6h PERIOD (LENGTH 1d TIME ZONE 'Europe/Berlin' DELAY 3h) AS (" + query + ") PARTITION BY MONTH;");
    ...
    public void testCreatePeriodMatView2() throws Exception {
        assertMemoryLeak(() -> {
            ...
            execute("CREATE MATERIALIZED VIEW test REFRESH MANUAL PERIOD (LENGTH 12h) AS (" + query + ") PARTITION BY MONTH;");
    ...
    public void testCreatePeriodMatView3() throws Exception {
        assertMemoryLeak(() -> {
            ...
            execute("CREATE MATERIALIZED VIEW test REFRESH PERIOD (SAMPLE BY INTERVAL) AS (" + query + ") PARTITION BY MONTH;");
    ...
    public void testCreatePeriodMatViewModelToSink() throws Exception {
        final String query = "select ts, k, avg(v) from " + TABLE1 + " sample by 30s";
        final DdlSerializationTest[] tests = new DdlSerializationTest[]{new DdlSerializationTest("create materialized view test1 refresh immediate period (length 12h time zone 'Europe/Sofia' delay 1h) as " + query, "create materialized view test1 with base " + TABLE1 + " refresh immediate period (length 12h time zone 'Europe/Sofia' delay 1h) " + "as (select-choose ts, k, avg(v) avg from (table1 sample by 30s align to calendar with offset '00:00'))"), new DdlSerializationTest("create materialized view test2 refresh manual period (length 10h time zone 'Europe/Sofia' delay 2h) as " + query, "create materialized view test2 with base " + TABLE1 + " refresh manual period (length 10h time zone 'Europe/Sofia' delay 2h) " + "as (select-choose ts, k, avg(v) avg from (table1 sample by 30s align to calendar with offset '00:00'))"), new DdlSerializationTest("create materialized view test3 refresh every 2d period (length 8h time zone 'Europe/Sofia' delay 3h) as " + query, "create materialized view test3 with base " + TABLE1 + " refresh every 2d period (length 8h time zone 'Europe/Sofia' delay 3h) " + "as (select-choose ts, k, avg(v) avg from (table1 sample by 30s align to calendar with offset '00:00'))"), new DdlSerializationTest("CREATE MATERIALIZED VIEW 'test4' REFRESH MANUAL PERIOD (LENGTH 60m) AS (" + query + ")", "create materialized view test4 with base " + TABLE1 + " refresh manual period (length 60m) " + "as (select-choose ts, k, avg(v) avg from (table1 sample by 30s align to calendar with offset '00:00'))"), new DdlSerializationTest("CREATE MATERIALIZED VIEW 'test4' REFRESH MANUAL DEFERRED PERIOD (LENGTH 60m) AS (" + query + ")", "create materialized view test4 with base " + TABLE1 + " refresh manual deferred period (length 60m) " + "as (select-choose ts, k, avg(v) avg from (table1 sample by 30s align to calendar with offset '00:00'))"), new DdlSerializationTest("CREATE MATERIALIZED VIEW 'test5' REFRESH PERIOD (SAMPLE BY INTERVAL) as (" + query + ")", "create materialized view test5 with base " + TABLE1 + " refresh immediate period (sample by interval) " + "as (select-choose ts, k, avg(v) avg from (table1 sample by 30s align to calendar with offset '00:00'))"), new DdlSerializationTest("CREATE MATERIALIZED VIEW 'test5' REFRESH PERIOD (SAMPLE BY INTERVAL) as (" + query + " ALIGN TO CALENDAR TIME ZONE 'Europe/Berlin')", "create materialized view test5 with base " + TABLE1 + " refresh immediate period (sample by interval) " + "as (select-choose ts, k, avg(v) avg from (table1 sample by 30s align to calendar time zone 'Europe/Berlin' with offset '00:00'))"),};
        testModelToSink(tests);
    }
    ...
    public void testCreateTimerMatView1() throws Exception {
        assertMemoryLeak(() -> {
            ...
            final String query = "select ts, k, max(v) as v_max from " + TABLE1 + " sample by 30s";
            execute("CREATE MATERIALIZED VIEW test REFRESH EVERY 5m START '" + start + "' TIME ZONE 'Europe/Berlin' AS (" + query + ") PARTITION BY YEAR;");
    ...
    public void testCreateTimerMatView2() throws Exception {
        assertMemoryLeak(() -> {
            ...
            execute("CREATE MATERIALIZED VIEW test REFRESH EVERY 5m AS (" + query + ") PARTITION BY YEAR;");
    ...
    public void testCreateTimerMatViewModelToSink() throws Exception {
        final String query = "select ts, k, avg(v) from " + TABLE1 + " sample by 30s";
        final DdlSerializationTest[] tests = new DdlSerializationTest[]{new DdlSerializationTest("create materialized view test refresh every 42m start '2021-01-01T01:01:00.000000Z' time zone 'Europe/Berlin' as " + query, "create materialized view test with base " + TABLE1 + " refresh " + "every 42m start '2021-01-01T01:01:00.000000Z' time zone 'Europe/Berlin' as (" + "select-choose ts, k, avg(v) avg from (table1 sample by 30s align to calendar with offset '00:00')" + ")"), new DdlSerializationTest("create materialized view test refresh every 42m deferred start '2021-01-01T01:01:00.000000Z' time zone 'Europe/Berlin' as " + query, "create materialized view test with base " + TABLE1 + " refresh " + "every 42m deferred start '2021-01-01T01:01:00.000000Z' time zone 'Europe/Berlin' as (" + "select-choose ts, k, avg(v) avg from (table1 sample by 30s align to calendar with offset '00:00')" + ")"),};
        testModelToSink(tests);
    }
        ...
        assertMemoryLeak(() -> {
            try (Path path = new Path()) {
                ...
                try (BlockFileReader reader = new BlockFileReader(configuration)) {
                    ...
                    MatViewDefinition.readFrom(engine, actualDefinition, reader, path, rootLen, matViewToken);
        ...
        assertMemoryLeak(() -> {
            try (Path path = new Path()) {
                ...
                try (BlockFileWriter writer = new BlockFileWriter(configuration.getFilesFacade(), configuration.getCommitMode())) {
                    ...
                    block = writer.append();
                    MatViewState.appendState(matViewState.getLastRefreshBaseTxn(), matViewState.isInvalid(), invalidationReason, block);
    ...
    public void testMatViewDefinitionInvalidTz() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try {
                def.init(MatViewDefinition.REFRESH_TYPE_IMMEDIATE, false, ColumnType.TIMESTAMP_MICRO, matViewToken, query, TABLE1, 30, 'K', "Europe/Berlin", "00:00", 0, 0, (char) 0, Numbers.LONG_NULL, null, 0, (char) 0, 0, (char) 0);
                Assert.fail("exception expected");
            } catch (CairoException e) {
            ...
            try {
                def.init(MatViewDefinition.REFRESH_TYPE_IMMEDIATE, false, ColumnType.TIMESTAMP_MICRO, matViewToken, query, TABLE1, 30, 's', "Oceania", "00:00", 0, 0, (char) 0, Numbers.LONG_NULL, "Europe/Berlin", 0, (char) 0, 0, (char) 0);
                Assert.fail("exception expected");
            } catch (CairoException e) {
            ...
            try {
                def.init(MatViewDefinition.REFRESH_TYPE_IMMEDIATE, false, ColumnType.TIMESTAMP_MICRO, matViewToken, query, TABLE1, 30, 's', "Europe/Berlin", "T00:00", 0, 0, (char) 0, Numbers.LONG_NULL, null, 0, (char) 0, 0, (char) 0);
                Assert.fail("exception expected");
            } catch (CairoException e) {
        ...
        assertMemoryLeak(() -> {
            try (Path path = new Path()) {
                ...
                try (BlockFileWriter writer = new BlockFileWriter(configuration.getFilesFacade(), configuration.getCommitMode())) {
                    ...
                    unknownDefinition.init(MatViewDefinition.REFRESH_TYPE_IMMEDIATE + 42, false, ColumnType.TIMESTAMP_MICRO, matViewToken, matViewDefinition.getMatViewSql(), TABLE1, matViewDefinition.getSamplingInterval(), matViewDefinition.getSamplingIntervalUnit(), matViewDefinition.getTimeZone(), matViewDefinition.getTimeZoneOffset(), matViewDefinition.getRefreshLimitHoursOrMonths(), matViewDefinition.getTimerInterval(), matViewDefinition.getTimerUnit(), matViewDefinition.getTimerStartUs(), matViewDefinition.getTimerTimeZone(), matViewDefinition.getPeriodDelay(), matViewDefinition.getPeriodDelayUnit(), matViewDefinition.getPeriodLength(), matViewDefinition.getPeriodLengthUnit());
                    AppendableBlock block = writer.append();
                ...
                try (BlockFileReader reader = new BlockFileReader(configuration)) {
                    ...
                    final int rootLen = path.size();
                    try {
                        MatViewDefinition.readFrom(engine, new MatViewDefinition(), reader, path, rootLen, matViewToken);
                        Assert.fail();
                    } catch (CairoException e) {
    ...
    public void testShowCreateImmediateMatView() throws Exception {
        final String query = "select ts, v+v doubleV, avg(v) from " + TABLE1 + " sample by 30s";
        final DdlSerializationTest[] tests = new DdlSerializationTest[]{new DdlSerializationTest("test1", "create materialized view test1 as (" + query + ") partition by day TTL 3 WEEKS", """
                ddl
                ...
                """), new DdlSerializationTest("test2", "create materialized view test2 refresh immediate deferred as (" + query + ") partition by day TTL 3 WEEKS", """
                ddl
    ...
    public void testShowCreateManualMatView() throws Exception {
        final String query = "select ts, v+v doubleV, avg(v) from " + TABLE1 + " sample by 30s";
        final DdlSerializationTest[] tests = new DdlSerializationTest[]{new DdlSerializationTest("test1", "create materialized view test1 refresh manual as " + query, """
                ddl
                CREATE MATERIALIZED VIEW 'test1' WITH BASE 'table1' REFRESH MANUAL AS (
                select ts, v+v doubleV, avg(v) from table1 sample by 30s
                ) PARTITION BY DAY;
                """), new DdlSerializationTest("test2", "create materialized view test2 refresh manual deferred as " + query, """
                ddl
    ...
    public void testShowCreateMatViewFail() throws Exception {
        assertMemoryLeak(() -> {
            ...
            assertExceptionNoLeakCheck("show create materialized test", 25, "'view' expected");
        });
    }
    ...
    public void testShowCreateMatViewFail2() throws Exception {
        assertMemoryLeak(() -> {
            createTable(TABLE1);
            assertExceptionNoLeakCheck("show create materialized view " + TABLE1, 30, "materialized view name expected, got table name");
        });
    }

    @Test
    public void testShowCreateMatViewFail3() throws Exception {
        assertException("show create materialized view 'test';", 30, "materialized view does not exist [view=test]");
    }

    @Test
    public void testShowCreatePeriodMatView() throws Exception {
        final String query = "select ts, v+v doubleV, avg(v) from " + TABLE1 + " sample by 30s";
        final DdlSerializationTest[] tests = new DdlSerializationTest[]{new DdlSerializationTest("test1", "create materialized view test1 refresh immediate period (length 12h time zone 'Europe/Sofia' delay 1h) as " + query, "ddl\n" + "CREATE MATERIALIZED VIEW 'test1' WITH BASE '" + TABLE1 + "' REFRESH IMMEDIATE PERIOD (LENGTH 12h TIME ZONE 'Europe/Sofia' DELAY 1h) AS (\n" + "select ts, v+v doubleV, avg(v) from table1 sample by 30s\n" + ") PARTITION BY DAY;\n"), new DdlSerializationTest("test2", "create materialized view test2 refresh manual period (length 10h time zone 'Europe/Sofia' delay 2h) as " + query, "ddl\n" + "CREATE MATERIALIZED VIEW 'test2' WITH BASE '" + TABLE1 + "' REFRESH MANUAL PERIOD (LENGTH 10h TIME ZONE 'Europe/Sofia' DELAY 2h) AS (\n" + "select ts, v+v doubleV, avg(v) from table1 sample by 30s\n" + ") PARTITION BY DAY;\n"), new DdlSerializationTest("test3", "create materialized view test3 refresh every 2d period(length 8h time zone 'Europe/Sofia' delay 3h) as " + query, "ddl\n" + "CREATE MATERIALIZED VIEW 'test3' WITH BASE '" + TABLE1 + "' REFRESH EVERY 2d PERIOD (LENGTH 8h TIME ZONE 'Europe/Sofia' DELAY 3h) AS (\n" + "select ts, v+v doubleV, avg(v) from table1 sample by 30s\n" + ") PARTITION BY DAY;\n"), new DdlSerializationTest("test4", "CREATE MATERIALIZED VIEW 'test4' REFRESH MANUAL PERIOD (LENGTH 60m) AS (" + query + ")", "ddl\n" + "CREATE MATERIALIZED VIEW 'test4' WITH BASE '" + TABLE1 + "' REFRESH MANUAL PERIOD (LENGTH 60m) AS (\n" + "select ts, v+v doubleV, avg(v) from table1 sample by 30s\n" + ") PARTITION BY DAY;\n"), new DdlSerializationTest("test5", "CREATE MATERIALIZED VIEW 'test5' REFRESH MANUAL DEFERRED PERIOD (LENGTH 60m) AS (" + query + ")", "ddl\n" + "CREATE MATERIALIZED VIEW 'test5' WITH BASE '" + TABLE1 + "' REFRESH MANUAL DEFERRED PERIOD (LENGTH 60m) AS (\n" + "select ts, v+v doubleV, avg(v) from table1 sample by 30s\n" + ") PARTITION BY DAY;\n"),};
        testShowCreateMaterializedView(tests, "timestamp");
    }
    ...
    public void testShowCreateTimerMatView() throws Exception {
        final String query = "select ts, v+v doubleV, avg(v) from " + TABLE1 + " sample by 30s";
        final DdlSerializationTest[] tests = new DdlSerializationTest[]{new DdlSerializationTest("test1", "create materialized view test1 refresh every 7d start '2020-01-01T02:23:59.900000Z' time zone 'Europe/Paris' as " + query, """
                ddl
                ...
                """), new DdlSerializationTest("test2", "create materialized view 'test2' refresh every 7d deferred start '2020-01-01T02:23:59.900000Z' time zone 'Europe/Paris' as " + query, """
                ddl
    ...
    public void testShowCreateTimerMatViewWithNanoTs() throws Exception {
        ...
        final DdlSerializationTest[] tests = new DdlSerializationTest[]{new DdlSerializationTest("test1", "create materialized view test1 refresh every 7d start '2020-01-01T02:23:59.900000Z' time zone 'Europe/Paris' as " + query, """
                ...
                """), new DdlSerializationTest("test2", "create materialized view 'test2' refresh every 7d deferred start '2020-01-01T02:23:59.900000000Z' time zone 'Europe/Paris' as " + query, """
                ddl
                CREATE MATERIALIZED VIEW 'test2' WITH BASE 'table1' REFRESH EVERY 7d DEFERRED START '2020-01-01T02:23:59.900000Z' TIME ZONE 'Europe/Paris' AS (
                select ts, v+v doubleV, avg(v) from table1 sample by 30s
                ) PARTITION BY DAY;
    ...
    private static void assertMatViewDefinition(int refreshType, String name, String query, String baseTableName, long samplingInterval, char samplingIntervalUnit, String timeZone, String timeZoneOffset, int refreshLimitHoursOrMonths, int timerInterval, char timerUnit, long timerStart, String timerTimeZone, int periodLength, char periodLengthUnit, int periodDelay, char periodDelayUnit) {
    ...
    private static void assertMatViewDefinitionFile(int refreshType, String name, String query, String baseTableName, long samplingInterval, char samplingIntervalUnit, String timeZone, String timeZoneOffset, int refreshLimitHoursOrMonths, int timerInterval, char timerUnit, long timerStart, String timerTimeZone, int periodLength, char periodLengthUnit, int periodDelay, char periodDelayUnit) {
        ...
        try (BlockFileReader reader = new BlockFileReader(configuration); Path path = new Path()) {
            ...
            MatViewDefinition.readFrom(engine, matViewDefinition, reader, path, rootLen, matViewToken);
    ...
    private void createTable(String tableName, boolean walEnabled, String timestampType) throws SqlException {
        execute("create table if not exists " + tableName + " (ts " + timestampType + ", k symbol capacity 2048, k2 symbol capacity 512, v long)" + " timestamp(ts) partition by day" + (walEnabled ? "" : " bypass") + " wal");
        for (int i = 0; i < 9; i++) {
    ...
    private void testCreateMatViewBaseTableSelfUnion(String timestampType) throws Exception {
        assertMemoryLeak(() -> {
            ...
            final String query = "with cte1 as (" + "  select ts, k, sum(v) sum_v from " + TABLE1 + " where k = 'k0' sample by 1d" + "), " + "cte2 as (" + "  select ts, k, sum(v) sum_v from " + TABLE1 + " where k = 'k1' sample by 1d" + ") " + "select ts, k, last(sum_v) as sum_v " + "from (" + "  select *" + "  from (" + "    select c1.ts, c1.k, c1.sum_v, c2.sum_v " + "    from cte1 c1 left join cte2 c2 on c1.ts = c2.ts" + "    union" + "    select c2.ts, c2.k, c2.sum_v, c1.sum_v " + "    from cte2 c2 left join cte1 c1 on c2.ts = c1.ts" + "  )" + "  order by ts asc" + ") timestamp(ts) " + "sample by 1d";
    ...
    private void testCreateMatViewNonDeterministicFunctionInSelect(String func, String columnName) throws Exception {
        ...
        assertExceptionNoLeakCheck("create materialized view test as (select ts, " + func + ", avg(v) from " + TABLE1 + " sample by 30s) partition by month", 45, "non-deterministic function cannot be used in materialized view: " + columnName);
        assertNull(getMatViewDefinition("test"));

        assertExceptionNoLeakCheck("create materialized view test as (select ts, f, a from (select ts, " + func + " f, avg(v) a from " + TABLE1 + " sample by 30s)) partition by month", 67, "non-deterministic function cannot be used in materialized view: " + columnName);
        assertNull(getMatViewDefinition("test"));
    }

    private void testCreateMatViewWithNonBaseTableKeys(String timestampType) throws Exception {
        final String[] queries = new String[]{"create materialized view test1 with base x as (select t1.ts, t2.k1, avg(t1.v) from x as t1 join y as t2 on v sample by 1m) partition by day", "create materialized view test2 with base x as (select \"t1\".\"ts\", \"t2\".\"k1\", avg(\"t1\".\"v\") from \"x\" as \"t1\" join \"y\" as \"t2\" on \"v\" sample by 1m) partition by day",
                // test table alias case-insensitivity
                "create materialized view test3 with base x as (select \"t1\".\"ts\", \"t2\".\"k1\", avg(\"t1\".\"v\") from \"x\" as \"T1\" join \"y\" as \"T2\" on \"v\" sample by 1m) partition by day", "create materialized view test4 with base x as (select t1.ts, t2.k1, avg(t1.v) from x as t1 join y as t2 on v sample by 1m) partition by day",
    ...
    private void testCreateMatViewWithNonDedupBaseKeys(String timestampType) throws Exception {
        final String[] queries = new String[]{"create materialized view x_hourly1 as (select ts, k2, avg(v) from x sample by 1h) partition by day;", "create materialized view x_hourly2 as (select xx.ts, xx.k2, avg(xx.v) from x as xx sample by 1h) partition by day;", "create materialized view x_hourly3 as (select ts, k1, k2, avg(v) from x sample by 1h) partition by day;", "create materialized view x_hourly4 as (select ts, concat(k1, k2) k, avg(v) from x sample by 1h) partition by day;", "create materialized view x_hourly5 as (select ts, k, avg(v) from (select concat(k1, k2) k, v, ts from x) sample by 1h) partition by day;", "create materialized view x_hourly6 as (select ts, k, avg(v) from (select concat(k2, 'foobar') k, v, ts from x) sample by 1h) partition by day;", "create materialized view x_hourly7 as (select ts, k, avg(v) from (select concat('foobar', k2) k, v, ts from x) sample by 1h) partition by day;", "create materialized view x_hourly8 as (select ts, k, avg(v) from (select ts, k2 as k, v from x) sample by 1h) partition by day;", "create materialized view test with base x as (select t1.ts, t1.k2, avg(t1.v) from x as t1 join y as t2 on v sample by 1m) partition by day"};
    ...
    private void testCreateMatViewWithNonDedupBaseKeys(String[] queries, String timestampType) throws Exception {
        assertMemoryLeak(() -> {
            execute("create table x " + " (ts " + timestampType + ", k1 symbol, k2 symbol, v long)" + " timestamp(ts) partition by day wal dedup upsert keys(ts, k1);");
            execute("create table y " + " (ts " + timestampType + ", k1 symbol, k2 symbol, v long)" + " timestamp(ts) partition by day wal;");
        ...
        assertMemoryLeak(() -> {
            ...
            try (SqlCompiler compiler = engine.getSqlCompiler()) {
                for (DdlSerializationTest test : tests) {
                    ...
                    TestUtils.assertEquals("ddl: " + test.ddl, test.expected, sink);
                }
            }
        });
    }
    ...
    private void testShowCreateMaterializedView(DdlSerializationTest[] tests, String timestamp) throws Exception {
        assertMemoryLeak(() -> {
            ...
            for (DdlSerializationTest test : tests) {
                execute(test.ddl);
                assertQueryNoLeakCheck(test.expected, "show create materialized view " + test.viewName, null, false);
            }
        });
    }
