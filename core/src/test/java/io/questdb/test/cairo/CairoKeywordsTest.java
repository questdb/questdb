...
public class CairoKeywordsTest extends AbstractTest {
    ...
    @Test
    public void testIsLiveViewCheckpointsMatchesOnlyExactDirName() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            ...
            // One same-length negative per character position. The length guard rejects every name
            // above, so without these only positions 0 and 1 are ever discriminated by a byte
            // compare, and a matcher that stopped comparing the tail would still pass.
            assertLiveViewCheckpoints("xcheckpoints", false);
            assertLiveViewCheckpoints("_Checkpoints", false);
            assertLiveViewCheckpoints("_cxeckpoints", false);
            assertLiveViewCheckpoints("_chxckpoints", false);
            assertLiveViewCheckpoints("_chexkpoints", false);
            assertLiveViewCheckpoints("_checxpoints", false);
            assertLiveViewCheckpoints("_checkxoints", false);
            assertLiveViewCheckpoints("_checkpxints", false);
            assertLiveViewCheckpoints("_checkpoxnts", false);
            assertLiveViewCheckpoints("_checkpoixts", false);
            assertLiveViewCheckpoints("_checkpoinxs", false);
            assertLiveViewCheckpoints("_checkpointx", false);
