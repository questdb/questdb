...
public class FuzzSetDefaultPartitionFormatOperation implements FuzzTransactionOperation {
    ...
    public boolean apply(Rnd tempRnd, CairoEngine engine, TableWriterAPI wApi, int virtualTimestampIndex, LongList excludedTsIntervals) {
        try (SqlExecutionContextImpl context = new SqlExecutionContextImpl(engine, 1);
            ...
            String sql = String.format("ALTER TABLE %s SET FORMAT %s", wApi.getTableToken().getTableName(), parquet ? "PARQUET" : "NATIVE");
            ...
            try {
                ...
                // FORMAT PARQUET is rejected on non-WAL tables and mat views;
                // also ignore if a concurrent fuzz step changed the table to a
                // shape that no longer satisfies the SQL-level guards.
