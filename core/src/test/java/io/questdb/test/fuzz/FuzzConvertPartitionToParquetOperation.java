...
public class FuzzConvertPartitionToParquetOperation implements FuzzTransactionOperation {

    private final long cutoffTimestamp;

    public FuzzConvertPartitionToParquetOperation(long cutoffTimestamp) {
    ...
    public boolean apply(Rnd tempRnd, CairoEngine engine, TableWriterAPI wApi, int virtualTimestampIndex, LongList excludedTsIntervals) {
        ...
        try (SqlExecutionContextImpl context = new SqlExecutionContextImpl(engine, 1);
            ...
            String sql = String.format("ALTER TABLE %s CONVERT PARTITION TO PARQUET WHERE %s > '%s' AND %s < '%s'",
            ...
            try {
                ...
                if (Chars.contains(e.getFlyweightMessage(), "table does not exist") || e.isTableDropped()) {
                    ...
                    return true;
                } else if (Chars.contains(e.getFlyweightMessage(), "could not convert partition")) {
                    return true;
                } else if (Chars.contains(e.getFlyweightMessage(), "already a parquet partition")) {
                    return true;
                } else {
                    throw e;
                }
