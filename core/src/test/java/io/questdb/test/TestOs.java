    ...
    static {
        ...
        if (resource != null) {
            ...
            if (absolutePathPreCompiled == null) {
                // The resource is packaged inside a jar (e.g. running with
                // mvn -pl <other-module> where this module's test-classes
                // are pulled in via questdb-tests.jar from the local m2
                // cache). URI.getPath() returns null for the opaque
                // {@code jar:file:/...!/...} URL, so we cannot compute a
                // filesystem-relative dev-build path. Stream the lib
                // straight out of the jar instead and let
                // {@code Os.loadLib} unpack it to a temp file.
