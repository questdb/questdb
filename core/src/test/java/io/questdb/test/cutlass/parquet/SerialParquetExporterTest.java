...
import static io.questdb.test.griffin.CopyExportTest.createJobThread;
import static org.junit.Assert.assertTrue;
import static org.junit.Assert.fail;

@RunWith(Parameterized.class)
public class SerialParquetExporterTest extends AbstractCairoTest {

    private final boolean walEnabled;

    public SerialParquetExporterTest(boolean walEnabled) {
    ...
    public static void setUpStatic() throws Exception {
        ...
        inputWorkRoot = TestUtils.unchecked(() -> temp.newFolder("exports" + System.nanoTime()).getAbsolutePath());
    ...
    @Test
    public void testCopyParquetBoundaryValues_MaxRowGroupSize() throws Exception {
        assertMemoryLeak(() -> {
            execute("create table test_table (x int)");
            execute("insert into test_table values (1)");

            execute("copy test_table to 'output_max_rows' with format parquet row_group_size 2147483647");

            assertTrue(exportDirectoryExists("output_max_rows"));

            // Verify data with max row group size
            assertSql("x\n1\n",
                    "select * from read_parquet('" + inputWorkRoot + "/output_max_rows')");
        });
    }

    @Test
    public void testCopyParquetBoundaryValues_MinRowGroupSize() throws Exception {
        assertMemoryLeak(() -> {
            execute("create table test_table (x int)");
            execute("insert into test_table values (1)");

            execute("copy test_table to 'output_min_rows' with format parquet row_group_size 1");

            assertTrue(exportDirectoryExists("output_min_rows"));

            // Verify data with min row group size
            assertSql("x\n1\n",
                    "select * from read_parquet('" + inputWorkRoot + "/output_min_rows')");
        });
    }

    @Test
    public void testCopyParquetBoundaryValues_NegativeRowGroupSize() throws Exception {
        assertException(
                "copy test_table to 'output' with format parquet row_group_size -1",
                64,
    ...
    public void testCopyParquetSyntaxError_InvalidOptionName() throws Exception {
        assertException(
                ...
                50,
                "unknown copy option: invalid_option"
        );
    }
    ...
    public void testCopyParquetSyntaxError_InvalidParquetVersion() throws Exception {
        assertException(
                ...
                66,
                "invalid parquet version, expected integer"
        );
    }
