...
public class IlpV4HttpSenderReceiverTest extends AbstractBootstrapTest {
    ...
    // ==================== Async Flush Integration Tests ====================

    @Test
    public void testAsyncFlushBasic() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (final TestServerMain serverMain = startWithEnvVariables(
                    PropertyKey.HTTP_RECEIVE_BUFFER_SIZE.getEnvVarName(), "65536"
            )) {
                ...
                try (IlpV4HttpSender sender = IlpV4HttpSender.connect("localhost", httpPort)) {
                    ...
                    // Send some data
                    for (int i = 0; i < 10; i++) {
                        sender.table("async_basic")
                                .symbol("tag", "value")
                                .longColumn("counter", i)
                                .at(1000000000L + i * 1000000L, ChronoUnit.MICROS);
                    }

                    // Flush should complete all async operations
                    sender.flush();
                ...
                serverMain.awaitTable("async_basic");
                serverMain.assertSql("select count() from async_basic", "count\n10\n");
            }
        });
    }

    @Test
    public void testAsyncFlushWithAutoFlush() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (final TestServerMain serverMain = startWithEnvVariables(
                    PropertyKey.HTTP_RECEIVE_BUFFER_SIZE.getEnvVarName(), "1048576"
            )) {
                ...
                try (IlpV4HttpSender sender = IlpV4HttpSender.connect("localhost", httpPort)) {
                    // Use maxInFlightRequests(1) to serialize requests for now
                    // This ensures only one flush at a time while still testing async infrastructure
                    sender.maxInFlightRequests(1)
                            .autoFlushRows(50);

                    Assert.assertTrue(sender.isAsyncMode());

                    // Send enough rows to trigger multiple auto-flushes
                    for (int i = 0; i < 200; i++) {
                        sender.table("async_autoflush")
                                .symbol("partition", "p" + (i % 5))
                                .longColumn("value", i)
                                .at(1000000000L + i * 1000000L, ChronoUnit.MICROS);
                    }
                ...
                serverMain.awaitTable("async_autoflush");
                serverMain.assertSql("select count() from async_autoflush", "count\n200\n");
            }
        });
    }

    @Test
    public void testAsyncFlushMultipleTables() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (final TestServerMain serverMain = startWithEnvVariables(
                    PropertyKey.HTTP_RECEIVE_BUFFER_SIZE.getEnvVarName(), "1048576"
            )) {
                ...
                try (IlpV4HttpSender sender = IlpV4HttpSender.connect("localhost", httpPort)) {
                    ...
                    // Write to multiple tables, triggering async flushes
                    for (int i = 0; i < 100; i++) {
                        sender.table("async_multi_t1")
                                .longColumn("id", i)
                                .at(1000000000L + i * 1000000L, ChronoUnit.MICROS);

                        sender.table("async_multi_t2")
                                .stringColumn("data", "row_" + i)
                                .at(1000000000L + i * 1000000L, ChronoUnit.MICROS);
                    }
                ...
                serverMain.awaitTable("async_multi_t1");
                serverMain.awaitTable("async_multi_t2");
                serverMain.assertSql("select count() from async_multi_t1", "count\n100\n");
                serverMain.assertSql("select count() from async_multi_t2", "count\n100\n");
            }
        });
    }

    @Test
    public void testAsyncFlushHighVolume() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (final TestServerMain serverMain = startWithEnvVariables(
                    PropertyKey.HTTP_RECEIVE_BUFFER_SIZE.getEnvVarName(), "2097152"
            )) {
                int httpPort = serverMain.getHttpServerPort();
                int rowCount = 10000;
                ...
                try (IlpV4HttpSender sender = IlpV4HttpSender.connect("localhost", httpPort)) {
                    ...
                    for (int i = 0; i < rowCount; i++) {
                        sender.table("async_high_volume")
                                .symbol("partition", "p" + (i % 10))
                                .longColumn("id", i)
                                .doubleColumn("value", i * 0.1)
                                .at(1000000000L + i * 1000000L, ChronoUnit.MICROS);
                ...
                serverMain.awaitTable("async_high_volume");
                serverMain.assertSql("select count() from async_high_volume", "count\n" + rowCount + "\n");
            }
        });
    }

    @Test
    public void testAsyncFlushCloseWaitsForInFlight() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (final TestServerMain serverMain = startWithEnvVariables(
                    PropertyKey.HTTP_RECEIVE_BUFFER_SIZE.getEnvVarName(), "1048576"
            )) {
                ...
                // Don't use try-with-resources so we can control close timing
                IlpV4HttpSender sender = IlpV4HttpSender.connect("localhost", httpPort);
                sender.maxInFlightRequests(1)
                        .autoFlushRows(50);

                // Send data that will trigger auto-flushes
                for (int i = 0; i < 150; i++) {
                    sender.table("async_close_wait")
                            .longColumn("id", i)
                            .at(1000000000L + i * 1000000L, ChronoUnit.MICROS);
                }
                ...
                serverMain.awaitTable("async_close_wait");
                serverMain.assertSql("select count() from async_close_wait", "count\n150\n");
            }
        });
    }

    @Test
    public void testAsyncFlushResetClearsErrorState() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (final TestServerMain serverMain = startWithEnvVariables(
                    PropertyKey.HTTP_RECEIVE_BUFFER_SIZE.getEnvVarName(), "65536"
            )) {
                ...
                try (IlpV4HttpSender sender = IlpV4HttpSender.connect("localhost", httpPort)) {
                    ...
                    // Normal operation
                    sender.table("async_reset")
                            .longColumn("id", 1)
                            .at(1000000000L, ChronoUnit.MICROS);
                    sender.flush();

                    // Reset and send more data
                    sender.reset();
                    Assert.assertFalse(sender.isInErrorState());
                ...
                serverMain.awaitTable("async_reset");
                serverMain.assertSql("select count() from async_reset", "count\n2\n");
            }
        });
    }

    @Test
    public void testAsyncFlushMaxInFlightOne() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (final TestServerMain serverMain = startWithEnvVariables(
                    PropertyKey.HTTP_RECEIVE_BUFFER_SIZE.getEnvVarName(), "1048576"
            )) {
                ...
                try (IlpV4HttpSender sender = IlpV4HttpSender.connect("localhost", httpPort)) {
                    // maxInFlight=1 should serialize requests
                    sender.maxInFlightRequests(1)
                            .autoFlushRows(25);
    ...
    @Test
    public void testAsyncFlushWithLargeMaxInFlight() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (final TestServerMain serverMain = startWithEnvVariables(
                    PropertyKey.HTTP_RECEIVE_BUFFER_SIZE.getEnvVarName(), "2097152"
            )) {
                ...
                try (IlpV4HttpSender sender = IlpV4HttpSender.connect("localhost", httpPort)) {
                    // Note: using maxInFlightRequests(1) for now until concurrency issues are resolved
                    sender.maxInFlightRequests(1)
                            .autoFlushRows(100);

                    for (int i = 0; i < 1000; i++) {
                        sender.table("async_large_inflight")
                                .symbol("part", "p" + (i % 5))
                                .longColumn("id", i)
                                .at(1000000000L + i * 1000000L, ChronoUnit.MICROS);
                    }
                ...
                serverMain.awaitTable("async_large_inflight");
                serverMain.assertSql("select count() from async_large_inflight", "count\n1000\n");
            }
        });
    }

    @Test
    public void testAsyncFlushMultipleFlushCalls() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (final TestServerMain serverMain = startWithEnvVariables(
                    PropertyKey.HTTP_RECEIVE_BUFFER_SIZE.getEnvVarName(), "1048576"
            )) {
                ...
                try (IlpV4HttpSender sender = IlpV4HttpSender.connect("localhost", httpPort)) {
                    ...
                    for (int batch = 0; batch < 5; batch++) {
                        for (int i = 0; i < 20; i++) {
                            sender.table("async_multi_flush")
                                    .longColumn("batch", batch)
                                    .longColumn("row", i)
                                    .at(1000000000L + batch * 100000000L + i * 1000000L, ChronoUnit.MICROS);
                        }
                        sender.flush();
                        Assert.assertEquals(0, sender.getInFlightCount());
                    }
                ...
                serverMain.awaitTable("async_multi_flush");
                serverMain.assertSql("select count() from async_multi_flush", "count\n100\n");
            }
        });
    }

    @Test
    public void testAsyncFlushEmptyFlush() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (final TestServerMain serverMain = startWithEnvVariables(
                    PropertyKey.HTTP_RECEIVE_BUFFER_SIZE.getEnvVarName(), "65536"
            )) {
                ...
                try (IlpV4HttpSender sender = IlpV4HttpSender.connect("localhost", httpPort)) {
                    ...
                    // Empty flush should not throw
                    sender.flush();
                    Assert.assertEquals(0, sender.getInFlightCount());

                    // Send some data
                    sender.table("async_empty_flush")
                            .longColumn("id", 1)
                            .at(1000000000L, ChronoUnit.MICROS);
                    sender.flush();
                ...
                serverMain.awaitTable("async_empty_flush");
                serverMain.assertSql("select count() from async_empty_flush", "count\n1\n");
            }
        });
    }

    @Test
    public void testAsyncFlushPreservesDataIntegrity() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (final TestServerMain serverMain = startWithEnvVariables(
                    PropertyKey.HTTP_RECEIVE_BUFFER_SIZE.getEnvVarName(), "2097152"
            )) {
                ...
                try (IlpV4HttpSender sender = IlpV4HttpSender.connect("localhost", httpPort)) {
                    ...
                    // Send data with verifiable pattern
                    for (int i = 0; i < rowCount; i++) {
                        sender.table("async_integrity")
                                .symbol("part", "p" + (i % 10))
                                .longColumn("id", i)
                                .longColumn("square", (long) i * i)
                                .doubleColumn("half", i / 2.0)
                                .at(1000000000L + i * 1000000L, ChronoUnit.MICROS);
                    }
                ...
                serverMain.awaitTable("async_integrity");
                serverMain.assertSql("select count() from async_integrity", "count\n" + rowCount + "\n");

                // Verify data integrity by checking sum of ids
                long expectedSum = (long) (rowCount - 1) * rowCount / 2;
                serverMain.assertSql("select sum(id) from async_integrity", "sum\n" + expectedSum + "\n");
            }
        });
    }
}
