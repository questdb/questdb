...
public class PgBootstrapLegacyTest extends AbstractBootstrapTest {
    ...
    public void testClientWithEnabledTlsGetsRejected() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (ServerMain serverMain = startWithEnvVariablesLegacy()) {
    ...
    public void testPgWireLoginDisabled() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (ServerMain serverMain = startWithEnvVariablesLegacy("QDB_PG_USER", "", // disables the default user
    ...
    public void testReadOnlyPgWireContext() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            try (ServerMain serverMain = startWithEnvVariablesLegacy("QDB_PG_SECURITY_READONLY", "true")) {
                int port = serverMain.getConfiguration().getPGWireConfiguration().getDispatcherConfiguration().getBindPort();
                assertQueryFails(
                        "admin",
                        "quest",
    ...
    private static TestServerMain startWithEnvVariablesLegacy(String... kvPairs) {
        kvPairs = Arrays.copyOf(kvPairs, kvPairs.length + 2);
        kvPairs[kvPairs.length - 2] = "QDB_PG_LEGACY_MODE_ENABLED";
        kvPairs[kvPairs.length - 1] = "true";
        return startWithEnvVariables(kvPairs);
    }
}
