...
public class HTTPSerialParquetExporterTest extends AbstractCairoTest {
    ...
    @Test
    public void testTempTableDroppedOnceOnDisconnectAfterBackpressure() throws Exception {
        assertMemoryLeak(() -> assertTempTableOwnerLifecycle(true));
    }

    @Test
    public void testTempTableReusesProtocolQueryOwner() throws Exception {
        assertMemoryLeak(() -> assertTempTableOwnerLifecycle(false));
    }
    ...
    private void assertTempTableOwnerLifecycle(boolean disconnectAfterBackpressure) throws Exception {
        final String sourceTable = "temp_export_source";
        final String tempTable = "temp_export_materialized";
        final String selectSql = "SELECT x FROM " + sourceTable;
        execute("CREATE TABLE " + sourceTable + " AS (SELECT x FROM long_sequence(3))");

        final CopyExportContext copyExportContext = engine.getCopyExportContext();
        final CopyExportContext.ExportTaskEntry entry = copyExportContext.assignExportEntry(
                sqlExecutionContext.getSecurityContext(),
                "COPY (" + selectSql + ") TO 'ignored'",
                "",
                null,
                CopyExportContext.CopyTrigger.HTTP
        );
        ...
        final SqlExecutionContextImpl parentExecutionContext = (SqlExecutionContextImpl) sqlExecutionContext;
        final TempTableProbeExporter exporter = new TempTableProbeExporter(parentExecutionContext);
        final AtomicInteger callbackCount = new AtomicInteger();
        long ownerId = -1;
        try (RecordCursorFactory selectFactory = select(selectSql)) {
            final CreateTableOperationImpl createOp = new CreateTableOperationImpl(
                    selectSql,
                    tempTable,
                    PartitionBy.NONE,
                    false,
                    configuration.getDefaultSymbolCapacity(),
                    entry.getSqlText().toString(),
                    false
            );
            createOp.setBatchSize(configuration.getParquetExportBatchSize());
            createOp.setTableKind(TableUtils.TABLE_KIND_TEMP_PARQUET_EXPORT);
            createOp.validateAndUpdateMetadataFromSelect(
                    selectFactory.getMetadata(),
                    selectFactory.getScanDirection()
            );
            task.of(
                    entry,
                    createOp,
                    ...
                    configuration.getParquetExportVersion(),
                    configuration.isParquetExportRawArrayEncoding(),
                    parentExecutionContext.getNowTimestampType(),
                    parentExecutionContext.getNow(parentExecutionContext.getNowTimestampType()),
                    false,
                    null,
                    selectFactory.getMetadata(),
                    (dataPtr, dataLen) -> {
            ...
            Assert.assertThrows(PeerIsSlowToReadException.class, exporter::process);
            if (disconnectAfterBackpressure) {
                exporter.clearExportResources();
                exporter.clearExportResources();
                Assert.assertEquals("disconnect must not resume the response callback", 1, callbackCount.get());
            } else {
                Assert.assertEquals(CopyExportRequestTask.Phase.SUCCESS, exporter.process());
                Assert.assertTrue("export must resume after client backpressure", callbackCount.get() > 1);
            }
            Assert.assertEquals("all nested SELECT work must retain the single protocol owner", 1, registrationCount.get());
            Assert.assertEquals("materialized temporary table must be dropped exactly once", 1, exporter.dropCount);
            Assert.assertNull("temporary table cursor must be released", task.getPageFrameCursor());
            Assert.assertNull(engine.getTableTokenIfExists(tempTable));
            Assert.assertEquals(ownerId, parentExecutionContext.getQueryRegistryOwnerId());
            Assert.assertSame(ownerMemoryTracker, parentExecutionContext.getMemoryTracker());
            Assert.assertSame(ownerSecurityContext, parentExecutionContext.getSecurityContext());
        } finally {
    ...
    private static final class TempTableProbeExporter extends HTTPSerialParquetExporter {
        private int dropCount;

        private TempTableProbeExporter(SqlExecutionContextImpl parentExecutionContext) {
            super(engine, parentExecutionContext);
        }
    ...
    private static final class TestExporter extends HTTPSerialParquetExporter {
        private TestExporter() {
            super(engine, (SqlExecutionContextImpl) HTTPSerialParquetExporterTest.sqlExecutionContext);
        }
