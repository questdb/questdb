...
public class QwpEgressRequestDecoderTest {
    ...
    public void testCancelBody() throws Exception {
        runWithBuf(32, (buf, _, decoder) -> {
            long p = buf;
            Unsafe.putByte(p++, QwpEgressMsgKind.CANCEL);
            Unsafe.putLong(p, 0xCAFEBABE_DEADBEEFL);
    ...
    public void testCancelRejectsTruncated() throws Exception {
        runWithBuf(32, (buf, _, decoder) -> {
            Unsafe.putByte(buf, QwpEgressMsgKind.CANCEL);
            try {
    ...
    public void testDecodeDecimal128RoundTrip() throws Exception {
        runWithBuf(128, (buf, bindVars, decoder) -> {
            ...
            Unsafe.putByte(p++, (byte) 6); // scale
            Unsafe.putLong(p, 0x0123456789ABCDEFL); // lo
            p += 8;
            Unsafe.putLong(p, 0x0011223344556677L); // hi
            p += 8;
            decoder.decodeQueryRequest(buf, (int) (p - buf), bindVars);
    ...
    public void testDecodeDecimal128ScaleOutOfRange() throws Exception {
        runWithBuf(64, (buf, bindVars, decoder) -> {
            ...
            long p = buf + totalLen;
            p = writeNullBind(p, QwpConstants.TYPE_DECIMAL128);
            Unsafe.putByte(p++, (byte) 200); // scale -- outside 0..76
            try {
                decoder.decodeQueryRequest(buf, (int) (p - buf), bindVars);
    ...
    public void testDecodeDecimal256NullBind() throws Exception {
        runWithBuf(64, (buf, bindVars, decoder) -> {
            ...
            Unsafe.putByte(p++, (byte) 10); // scale
            decoder.decodeQueryRequest(buf, (int) (p - buf), bindVars);
    ...
    public void testDecodeFloatAndDoubleBind() throws Exception {
        runWithBuf(128, (buf, bindVars, decoder) -> {
            ...
            Unsafe.putInt(p, Float.floatToRawIntBits(3.14f));
            p += 4;
            p = writeNonNullBind(p, QwpConstants.TYPE_DOUBLE);
            Unsafe.putLong(p, Double.doubleToRawLongBits(2.718281828));
            p += 8;
            decoder.decodeQueryRequest(buf, (int) (p - buf), bindVars);
    ...
    public void testDecodeGeohashBindRoundTripAtMaxPrecision() throws Exception {
        runWithBuf(64, (buf, bindVars, decoder) -> {
            ...
            Unsafe.putLong(p, 0x0FFF_FFFF_FFFF_FFFFL);
            p += 8;
            decoder.decodeQueryRequest(buf, (int) (p - buf), bindVars);
    ...
    public void testDecodeLong256Bind() throws Exception {
        runWithBuf(128, (buf, bindVars, decoder) -> {
            ...
            Unsafe.putLong(p, 0x11L);
            p += 8;
            Unsafe.putLong(p, 0x22L);
            p += 8;
            Unsafe.putLong(p, 0x33L);
            p += 8;
            Unsafe.putLong(p, 0x44L);
            p += 8;
            decoder.decodeQueryRequest(buf, (int) (p - buf), bindVars);
    ...
    public void testDecodeMixedBinds() throws Exception {
        runWithBuf(256, (buf, bindVars, decoder) -> {
            ...
            p += 4;
            for (byte b : s) Unsafe.putByte(p++, b);
    ...
    public void testDecodeStringBind() throws Exception {
        runWithBuf(256, (buf, bindVars, decoder) -> {
            ...
            Unsafe.putInt(p, 0);
            p += 4;
            Unsafe.putInt(p, bindStr.length);
            p += 4;
            for (byte b : bindStr) Unsafe.putByte(p++, b);
    ...
    public void testDecodeTimestampAndDateBind() throws Exception {
        runWithBuf(128, (buf, bindVars, decoder) -> {
            ...
            Unsafe.putLong(p, 1_700_000_000_000_000L);
            p += 8;
            p = writeNonNullBind(p, QwpConstants.TYPE_DATE);
            Unsafe.putLong(p, 1_700_000_000_000L);
            p += 8;
            decoder.decodeQueryRequest(buf, (int) (p - buf), bindVars);
    ...
    public void testDecodeTimestampNanosBindIsTaggedAsNanos() throws Exception {
        runWithBuf(128, (buf, bindVars, decoder) -> {
            ...
            Unsafe.putLong(p, 1_700_000_000_000_000_000L);
            p += 8;
            decoder.decodeQueryRequest(buf, (int) (p - buf), bindVars);
    ...
    public void testDecodeUuidBind() throws Exception {
        runWithBuf(128, (buf, bindVars, decoder) -> {
            ...
            Unsafe.putLong(p, 0xA0EE_0000_0000_0001L);
            p += 8;
            Unsafe.putLong(p, 0xFFFF_FFFF_FFFF_FFFEL);
            p += 8;
            decoder.decodeQueryRequest(buf, (int) (p - buf), bindVars);
            Function f = bindVars.getFunction(0);
    ...
    public void testRejectsArrayBind() throws Exception {
        runWithBuf(128, (buf, bindVars, decoder) -> {
            int totalLen = writeBindScaffold(buf, 1);
            long p = buf + totalLen;
            Unsafe.putByte(p++, QwpConstants.TYPE_DOUBLE_ARRAY);
            Unsafe.putByte(p++, (byte) 0); // null_flag = 0
            try {
                decoder.decodeQueryRequest(buf, (int) (p - buf), bindVars);
    ...
    public void testRejectsNegativeInitialCredit() throws Exception {
        runWithBuf(64, (buf, bindVars, decoder) -> {
            ...
            for (byte b : sql) Unsafe.putByte(p++, b);
            for (int i = 0; i < 9; i++) Unsafe.putByte(p++, (byte) 0xFF);
            Unsafe.putByte(p++, (byte) 0x01);
            try {
                decoder.decodeQueryRequest(buf, (int) (p - buf), bindVars);
    ...
    public void testRejectsTruncatedBindBody() throws Exception {
        runWithBuf(64, (buf, bindVars, decoder) -> {
            int totalLen = writeBindScaffold(buf, 1);
            long p = buf + totalLen;
            Unsafe.putByte(p++, QwpConstants.TYPE_LONG);
            Unsafe.putByte(p++, (byte) 0); // null_flag = 0 (non-null)
    ...
    public void testRejectsUnknownBindType() throws Exception {
        runWithBuf(64, (buf, bindVars, decoder) -> {
            int totalLen = writeBindScaffold(buf, 1);
            long p = buf + totalLen;
            Unsafe.putByte(p++, (byte) 0x7F); // unassigned type code
    ...
    private static long writeNullBind(long p, byte type) {
        Unsafe.putByte(p++, type);
        Unsafe.putByte(p++, (byte) 1); // null_flag
        Unsafe.putByte(p++, (byte) 1); // bitmap bit 0 set
        return p;
    }
