 ...
 */
public class IlpV4WebSocketUpgradeProcessorTest extends AbstractWebSocketTest {

    // ==================== PROCESSOR CREATION TESTS ====================
    // Note: Processor instantiation requires CairoEngine and configuration
    // Full processor tests are covered by integration tests (IlpV4WebSocketHandshakeTest, etc.)
    ...
    public void testWriteHandshakeResponse() {
        ...
        try {
            int written = IlpV4WebSocketUpgradeProcessor.writeHandshakeResponse(buffer, (int) bufferSize, key);
    ...
    public void testWriteBadRequestResponse() {
        ...
        try {
            ...
            int written = IlpV4WebSocketUpgradeProcessor.writeBadRequestResponse(buffer, (int) bufferSize, reason);
    ...
    public void testWriteUpgradeRequiredResponse() {
        ...
        long buffer = Unsafe.malloc(bufferSize, MemoryTag.NATIVE_DEFAULT);
        try {
            int written = IlpV4WebSocketUpgradeProcessor.writeUpgradeRequiredResponse(buffer, (int) bufferSize);
    ...
    public void testHandshakeResponseSize() {
        Utf8String key = new Utf8String("dGhlIHNhbXBsZSBub25jZQ==");
        int expectedSize = IlpV4WebSocketUpgradeProcessor.handshakeResponseSize(key);
        ...
        long buffer = Unsafe.malloc(bufferSize, MemoryTag.NATIVE_DEFAULT);
        try {
            int actualSize = IlpV4WebSocketUpgradeProcessor.writeHandshakeResponse(buffer, (int) bufferSize, key);
