                ...
                .run((engine, sqlExecutionContext) -> {
                    ...
                    try (HttpClient httpClient = HttpClientFactory.newPlainTextInstance(new DefaultHttpClientConfiguration())) {
                        ...
                        try (var headers = request.send()) {
                            ...
                            // Scan the raw body for U+FFFD UTF-8 = 0xEF 0xBF 0xBD.
                            // NOTE: this raw scan works only because the single
                            // 3-byte value page is below the compression threshold
                            // and is stored uncompressed. If this fixture ever
                            // grows enough for LZ4 to kick in, switch to decoding
                            // the parquet body and asserting the value equals
