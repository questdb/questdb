...
import io.questdb.cutlass.line.tcp.v4.IlpV4GorillaDecoder;
 ...
 */
public class IlpV4GorillaEncoderTest {
    ...
    public void testGetBucketZero() {
        ...
        Assert.assertEquals(0, IlpV4GorillaDecoder.getBucket(0));
    }

    @Test
    public void testGetBucket7Bit() {
        // DoD in [-63, 64] should use bucket 1 (9 bits)
        Assert.assertEquals(1, IlpV4GorillaDecoder.getBucket(1));
        Assert.assertEquals(1, IlpV4GorillaDecoder.getBucket(-1));
        Assert.assertEquals(1, IlpV4GorillaDecoder.getBucket(63));
        Assert.assertEquals(1, IlpV4GorillaDecoder.getBucket(-63));
        Assert.assertEquals(1, IlpV4GorillaDecoder.getBucket(64));
    }

    @Test
    public void testGetBucket9Bit() {
        // DoD in [-255, 256] but outside [-63, 64] should use bucket 2 (12 bits)
        Assert.assertEquals(2, IlpV4GorillaDecoder.getBucket(65));
        Assert.assertEquals(2, IlpV4GorillaDecoder.getBucket(-64));
        Assert.assertEquals(2, IlpV4GorillaDecoder.getBucket(255));
        Assert.assertEquals(2, IlpV4GorillaDecoder.getBucket(-255));
        Assert.assertEquals(2, IlpV4GorillaDecoder.getBucket(256));
    }

    @Test
    public void testGetBucket12Bit() {
        // DoD in [-2047, 2048] but outside [-255, 256] should use bucket 3 (16 bits)
        Assert.assertEquals(3, IlpV4GorillaDecoder.getBucket(257));
        Assert.assertEquals(3, IlpV4GorillaDecoder.getBucket(-256));
        Assert.assertEquals(3, IlpV4GorillaDecoder.getBucket(2047));
        Assert.assertEquals(3, IlpV4GorillaDecoder.getBucket(-2047));
        Assert.assertEquals(3, IlpV4GorillaDecoder.getBucket(2048));
    }

    @Test
    public void testGetBucket32Bit() {
        // DoD outside [-2047, 2048] should use bucket 4 (36 bits)
        Assert.assertEquals(4, IlpV4GorillaDecoder.getBucket(2049));
        Assert.assertEquals(4, IlpV4GorillaDecoder.getBucket(-2048));
        Assert.assertEquals(4, IlpV4GorillaDecoder.getBucket(100000));
        Assert.assertEquals(4, IlpV4GorillaDecoder.getBucket(-100000));
        Assert.assertEquals(4, IlpV4GorillaDecoder.getBucket(Integer.MAX_VALUE));
        Assert.assertEquals(4, IlpV4GorillaDecoder.getBucket(Integer.MIN_VALUE));
    }
    ...
    public void testGetBitsRequired() {
        // Bucket 0: 1 bit
        Assert.assertEquals(1, IlpV4GorillaDecoder.getBitsRequired(0));

        // Bucket 1: 9 bits (2 prefix + 7 value)
        Assert.assertEquals(9, IlpV4GorillaDecoder.getBitsRequired(1));
        Assert.assertEquals(9, IlpV4GorillaDecoder.getBitsRequired(-1));
        Assert.assertEquals(9, IlpV4GorillaDecoder.getBitsRequired(64));

        // Bucket 2: 12 bits (3 prefix + 9 value)
        Assert.assertEquals(12, IlpV4GorillaDecoder.getBitsRequired(65));
        Assert.assertEquals(12, IlpV4GorillaDecoder.getBitsRequired(256));

        // Bucket 3: 16 bits (4 prefix + 12 value)
        Assert.assertEquals(16, IlpV4GorillaDecoder.getBitsRequired(257));
        Assert.assertEquals(16, IlpV4GorillaDecoder.getBitsRequired(2048));

        // Bucket 4: 36 bits (4 prefix + 32 value)
        Assert.assertEquals(36, IlpV4GorillaDecoder.getBitsRequired(2049));
        Assert.assertEquals(36, IlpV4GorillaDecoder.getBitsRequired(-2048));
    }

    // ==================== Constant Delta Encoding Tests ====================

    @Test
    public void testEncodeConstantDelta() throws Exception {
        ...
        try {
            ...
            int expectedBits = (timestamps.length - 2) * 1; // 1 bit per DoD=0
    ...
    @Test
    public void testEncodeVaryingDelta() throws Exception {
    ...
    @Test
    public void testEncodeAllBucketSizes() throws Exception {
    ...
    @Test
    public void testEncodeLargeDataset() throws Exception {
    ...
    @Test
    public void testEncodeMatchesSpec() throws Exception {
        ...
        int size1 = IlpV4TimestampDecoder.calculateGorillaSize(ts1, false);
        ...
        int size2 = IlpV4TimestampDecoder.calculateGorillaSize(ts2, false);
    ...
    @Test
    public void testEncodeSingleTimestamp() throws Exception {
    ...
    @Test
    public void testEncodeWithNullBitmap() throws Exception {
        long[] timestamps = {1000L, 2000L, 0L, 4000L, 0L};
        boolean[] nulls = {false, false, true, false, true};
        int rowCount = timestamps.length;

        int size = IlpV4TimestampDecoder.calculateGorillaSize(timestamps, true);
    ...
    public void testCompressionRatioConstantInterval() {
        ...
        int gorillaSize = IlpV4TimestampDecoder.calculateGorillaSize(timestamps, false);
