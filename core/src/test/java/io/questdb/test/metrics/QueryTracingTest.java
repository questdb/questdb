...
public class QueryTracingTest extends AbstractCairoTest {
    ...
    public void testMigrationAddsTimingColumns() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (QueryTracingJob ignore = new QueryTracingJob(engine)) {
                assertQuery("SELECT \"column\" FROM (SHOW COLUMNS FROM '_query_trace') WHERE \"column\" IN ('client_wait_micros', 'first_row_micros', 'resource_group_cpu_wait_micros')")
                        .noLeakCheck()
                        .noRandomAccess()
            ...
            try (QueryTracingJob ignore = new QueryTracingJob(engine)) {
                assertQuery("SELECT \"column\" FROM (SHOW COLUMNS FROM '_query_trace') WHERE \"column\" IN ('first_row_micros', 'resource_group_cpu_wait_micros')")
                        .noLeakCheck()
                        .noRandomAccess()
                        .returns("column\nfirst_row_micros\nresource_group_cpu_wait_micros\n");
            }
            ...
            try (QueryTracingJob ignore = new QueryTracingJob(engine)) {
                assertQuery("SELECT \"column\" FROM (SHOW COLUMNS FROM '_query_trace') WHERE \"column\" IN ('client_wait_micros', 'resource_group_cpu_wait_micros')")
                        .noLeakCheck()
                        .noRandomAccess()
                        .returns("column\nclient_wait_micros\nresource_group_cpu_wait_micros\n");
            }
            ...
            try (QueryTracingJob job = new QueryTracingJob(engine)) {
                ...
                assertQuery("""
                        SELECT
                            ...
                            resource_group_cpu_wait_micros IS NULL AS rg_wait_null
                        FROM _query_trace
                        WHERE query_text = 'SELECT 42 AS answer'
                        """)
                        .noLeakCheck()
                        .returns("""
                                query_text\tprincipal\twall_nonnegative\tclient_wait_micros\tttfr_nonnegative\trg_wait_null
                                SELECT 42 AS answer\tadmin\ttrue\t0\ttrue\ttrue
                                """);
            }
        });
    }
