...
public class IntervalFwdDataFrameCursorTest extends AbstractCairoTest {
    ...
    public void testClose() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            ...
            IntervalFwdDataFrameCursor cursor = new IntervalFwdDataFrameCursor(intervals, reader.getMetadata().getTimestampIndex());
            cursor.of(reader);
            cursor.close();
    ...
    public void testIntervalCursorNoTimestamp() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            ...
            try (TableReader reader = new TableReader(configuration, "x")) {
                IntervalFwdDataFrameCursor cursor = new IntervalFwdDataFrameCursor(new LongList(), reader.getMetadata().getTimestampIndex());
                try {
                    cursor.of(reader);
                    Assert.fail();
    ...
    public void testReload(int partitionBy, long increment, LongList intervals, int rowCount, CharSequence expected1, CharSequence expected2) throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            ...
            try (CairoEngine engine = new CairoEngine(configuration)) {
                ...
                final IntervalFwdDataFrameCursorFactory factory = new IntervalFwdDataFrameCursorFactory(engine, "x", 0, intervals, timestampIndex);
    ...
    private void testIntervals(int partitionBy, long increment, int rowCount, CharSequence expected, long expectedCount) throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            ...
            try (TableReader reader = new TableReader(configuration, "x")) {
                final TableReaderRecord record = new TableReaderRecord();
                IntervalFwdDataFrameCursor cursor = new IntervalFwdDataFrameCursor(IntervalFwdDataFrameCursorTest.intervals, reader.getMetadata().getTimestampIndex());
                cursor.of(reader);
                record.of(reader);
