...
import static io.questdb.std.Os.sleep;
...
public class TableWriterTest extends AbstractCairoTest {
    ...
    @Test
    public void testAddColumnAndReadCrash01() throws Exception {
        ...
        try (TableModel model = new TableModel(configuration, tableName, PartitionBy.DAY)
            ...
            List<Short> newColTypes = new ArrayList<>();
            CyclicBarrier barrier = new CyclicBarrier(1);

            try (TableReader reader = new TableReader(configuration, tableName);
                 TableWriter writer = new TableWriter(configuration, model.getName())) {
                Thread writerT = new Thread(() -> {
                    try {
                        int i = 0;
                        List<CharSequence> newCols = new ArrayList<>();
                        while (i < 200) {
                            LOG.info().$("writer:").$(i).$();
                            ++i;
                            long ts = 0;
                            ...
                            Rnd rnd = new Rnd();
                            appendNProducts(ts, rnd, writer, 1000);
                            ...
                            appendNWithNewColumn(rnd, writer, newCols, newColTypes, 1000);
...
//                readerT.join();
    ...
    @Test
    public void testAddColumnAndReadCrash02() throws Exception {
        String tableName = "ABC";
        try (TableModel model = new TableModel(configuration, tableName, PartitionBy.DAY)
                .col("productId", ColumnType.INT)
                .col("productName", ColumnType.STRING)
            ...
            Short[] columnTypes = new Short[]{ColumnType.BOOLEAN, ColumnType.BYTE,
                    ...
                    ColumnType.SYMBOL, ColumnType.LONG256,
                    ColumnType.GEOBYTE, ColumnType.GEOSHORT,
                    ColumnType.GEOINT, ColumnType.GEOLONG,
                    ColumnType.BINARY, //ColumnType.PARAMETER, ColumnType.RECORD, ColumnType.CURSOR
                    //ColumnType.VAR_ARG,
                    };
            List<Short> newColTypes = new ArrayList<>();
                ...
                Thread writerT = new Thread(() -> {
                    try {
                        ...
                        while (i < 70) {
                            ...
                            Rnd rnd = new Rnd();
                            appendNProducts2(ts, rnd, writer, 1000);
                            writer.commit();

                            CharSequence newCol = "price" + i;
                            short colType = columnTypes[rnd.nextInt() % columnTypes.length];
                            writer.addColumn(newCol, colType);
                            newCols.add(newCol);
                            newColTypes.add(colType);

                            appendNWithNewColumn2(rnd, writer, newCols, newColTypes, 1000, ts);
                            writer.commit();
                            LOG.info().$("writer:").$(i).$("put once with ColumnType:").$(colType).$();
                        }
                        isWriteDone = true;
                ...
                Thread readerT = new Thread(() -> {
                    try {
                        ...
                        while (!isWriteDone) {
                            ...
                            if (reader.reload()) {
                                LOG.info().$("reader:").$(i).$("reloaded!").$();
                                RecordCursor cursor = reader.getCursor();
                                cursor.toTop();
                                int j = 0;
                                while (cursor.hasNext()) {
                                    cursor.getRecord().getByte(0);
                                    ++j;
                                }
                            }
                        }
    ...
    // TODO replace List to NoGC container
    private void appendNWithNewColumn(Rnd rnd, TableWriter writer, List<CharSequence> newCols, List<Short> colTypes,
                                      int n) {
        ...
        for (CharSequence cs : newCols) {
            int i = writer.getColumnIndex(cs);
            indexs.add(i);
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < indexs.size(); ++j) {
                Short t = colTypes.get(j);
                switch (t) {
                    case ColumnType.BOOLEAN:
                        r.putBool(indexs.get(j), rnd.nextBoolean());
                        break;
                    case ColumnType.BYTE:
                        r.putByte(indexs.get(j), rnd.nextByte());
                        break;
                    case ColumnType.SHORT:
                        r.putShort(indexs.get(j), rnd.nextShort());
                        break;
                    case ColumnType.CHAR:
                        r.putChar(indexs.get(j), rnd.nextChar());
                        break;
                    case ColumnType.INT:
                        r.putInt(indexs.get(j), rnd.nextInt());
                        break;
                    case ColumnType.LONG:
                        r.putLong(indexs.get(j), rnd.nextLong());
                        break;
                    case ColumnType.DATE:
                        r.putDate(indexs.get(j), rnd.nextLong());
                        break;
                    case ColumnType.TIMESTAMP:
                        r.putTimestamp(indexs.get(j), rnd.nextLong());
                        break;
                    case ColumnType.FLOAT:
                        r.putFloat(indexs.get(j), rnd.nextFloat());
                        break;
                    case ColumnType.DOUBLE:
                        r.putDouble(indexs.get(j), rnd.nextDouble());
                        break;
                    case ColumnType.STRING:
                        r.putStr(indexs.get(j), rnd.nextString(10));
                        break;
                    case ColumnType.SYMBOL:
                        r.putSym(indexs.get(j), rnd.nextString(5));
                        break;
                    case ColumnType.LONG256:
                        r.putLong256(indexs.get(j), rnd.nextLong(), rnd.nextLong(), rnd.nextLong(), rnd.nextLong());
                        break;
                    case ColumnType.GEOBYTE:
                        r.putGeoHash(indexs.get(j), rnd.nextLong()); // TODO check why there is no putGeoByte
                        break;
                    case ColumnType.GEOSHORT:
                        r.putGeoHash(indexs.get(j), rnd.nextLong()); // TODO check why there is no putGeoSHORT
                        break;
                    case ColumnType.GEOINT:
                        r.putGeoHash(indexs.get(j), rnd.nextLong()); // TODO check why there is no putGEOINT
                        break;
                    case ColumnType.GEOLONG:
                        r.putGeoHash(indexs.get(j), rnd.nextLong()); // TODO check why there is no putGEOLONG
                        break;
                    case ColumnType.BINARY:
                        r.putBin(indexs.get(j), (new TestRecord.ArrayBinarySequence()).of(rnd.nextBytes(25)));
                        break;
...
//                        break;
                    case ColumnType.VAR_ARG:
                        r.putStr(indexs.get(j), rnd.nextString(20));
                        break;
...
//                        break;
    ...
    // TODO replace List to NoGC container
    private void appendNWithNewColumn2(Rnd rnd, TableWriter writer, List<CharSequence> newCols, List<Short> colTypes,
                                      int n, long ts) {
        ...
        List<Integer> indexs = new ArrayList<>();
        for (CharSequence cs : newCols) {
            indexs.add(writer.getColumnIndex(cs));
        }

        for (int i = 0; i < n; i++) {
            TableWriter.Row r = writer.newRow(ts += 60000L * 1000L * 100L);
            r.putInt(productId, rnd.nextPositiveInt());
            r.putStr(productName, rnd.nextString(10));
            r.putSym(supplier, rnd.nextString(4));
            r.putSym(category, rnd.nextString(11));
...
//                        break;
    ...
    private long appendNProducts(long ts, Rnd rnd, TableWriter writer, int n) {
        int productId = writer.getColumnIndex("productId");
        int productName = writer.getColumnIndex("productName");
        int supplier = writer.getColumnIndex("supplier");
        int category = writer.getColumnIndex("category");
    ...
    private long appendNProducts2(long ts, Rnd rnd, TableWriter writer, int n) {
        int productId = writer.getColumnIndex("productId");
        int productName = writer.getColumnIndex("productName");
        int supplier = writer.getColumnIndex("supplier");
        int category = writer.getColumnIndex("category");
