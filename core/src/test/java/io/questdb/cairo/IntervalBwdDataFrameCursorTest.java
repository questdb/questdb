...
public class IntervalBwdDataFrameCursorTest extends AbstractCairoTest {
    ...
    public void testClose() throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            ...
            IntervalBwdDataFrameCursor cursor = new IntervalBwdDataFrameCursor(intervals, reader.getMetadata().getTimestampIndex());
            cursor.of(reader);
            cursor.close();
    ...
    public void testReload(int partitionBy, long increment, LongList intervals, int rowCount, CharSequence expected1, CharSequence expected2) throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            ...
            try (CairoEngine engine = new CairoEngine(configuration)) {
                ...
                final IntervalBwdDataFrameCursorFactory factory = new IntervalBwdDataFrameCursorFactory(engine, "x", 0, intervals, timestampIndex);
    ...
    private void testIntervals(int partitionBy, long increment, int rowCount, CharSequence expected, long expectedCount) throws Exception {
        TestUtils.assertMemoryLeak(() -> {
            ...
            try (TableReader reader = new TableReader(configuration, "x")) {
                final TableReaderRecord record = new TableReaderRecord();
                IntervalBwdDataFrameCursor cursor = new IntervalBwdDataFrameCursor(IntervalBwdDataFrameCursorTest.intervals, reader.getMetadata().getTimestampIndex());
                cursor.of(reader);
                record.of(reader);
