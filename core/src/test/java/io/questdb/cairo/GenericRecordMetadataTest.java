...
public class GenericRecordMetadataTest {
    ...
    public void testBaseInterfaceDefaults() {
        ...
        metadata.add(new TableColumnMetadata("cde", 2, ColumnType.SYMBOL, true, 1024, true, null));
    ...
    public void testDuplicateColumn() {
        ...
        try {
            metadata.add(new TableColumnMetadata("abc", 1, ColumnType.FLOAT));
            Assert.fail();
        } catch (CairoException e) {
            TestUtils.assertContains(e.getFlyweightMessage(), "Duplicate column 'abc'");
        }
    ...
    public void testDuplicateColumn2() {
        ...
        try {
            metadata.add(new TableColumnMetadata("ABC", 3, ColumnType.FLOAT));
            Assert.fail();
        } catch (CairoException e) {
            TestUtils.assertContains(e.getFlyweightMessage(), "Duplicate column 'abc'");
        }
    ...
    public void testReuse() {
        ...
        metadata.clear();
        metadata.add(new TableColumnMetadata("x", 4, ColumnType.SYMBOL, false, 0, false, null));
        metadata.add(new TableColumnMetadata("z", 5, ColumnType.DATE));
        sink.clear();
        metadata.toJson(sink);
