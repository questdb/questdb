...
public class WalWriterTest extends AbstractCairoTest {
    ...
    public void testAddColumnRollsUncommittedRowsToNewSegment() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (WalWriter walWriter1 = engine.getWalWriter(AllowAllCairoSecurityContext.INSTANCE, tableToken)) {
                try (WalWriter walWriter2 = engine.getWalWriter(AllowAllCairoSecurityContext.INSTANCE, tableToken)) {
            ...
            try (TableModel model = defaultModel(tableToken.getTableName())) {
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName1, 0, 1)) {
                ...
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName2, 0, 1)) {
                    assertEquals(2, reader.getColumnCount());
                ...
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName1, 1, 1)) {
                ...
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName2, 1, 1)) {
                    assertEquals(3, reader.getColumnCount());
                    assertEquals(walName2, reader.getWalName());
                    assertEquals(tableToken.getTableName(), reader.getTableName());
                    assertEquals(1, reader.size());
    ...
    public void testAddingColumnClosesSegment() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (TableModel model = defaultModel(tableToken.getTableName())) {
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 0, 1)) {
                    assertEquals(2, reader.getColumnCount());
                ...
                model.col("c", ColumnType.INT);
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 1, 1)) {
                    assertEquals(3, reader.getColumnCount());
                    assertEquals(walName, reader.getWalName());
                    assertEquals(tableToken.getTableName(), reader.getTableName());
                    assertEquals(1, reader.size());
                ...
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 2, 1)) {
    ...
    public void testAddingColumnOverlapping() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (TableModel model = defaultModel(tableToken.getTableName())) {
                model.col("c", ColumnType.INT);
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName1, 0, 0)) {
                    assertEquals(3, reader.getColumnCount());
                    assertEquals(walName1, reader.getWalName());
                    assertEquals(tableToken.getTableName(), reader.getTableName());
                ...
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName2, 0, 0)) {
                    assertEquals(4, reader.getColumnCount());
                    assertEquals(walName2, reader.getWalName());
                    assertEquals(tableToken.getTableName(), reader.getTableName());
                    assertEquals(0, reader.size());
    ...
    public void testAddingDuplicateColumn() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (TableModel model = defaultModel(tableName)) {
                ...
                model.col("c", ColumnType.INT);
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 1, 2)) {
                    assertEquals(3, reader.getColumnCount());
                    assertEquals(walName, reader.getWalName());
                    assertEquals(tableName, reader.getTableName());
                ...
                try {
                    engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 2, 1);
    ...
    public void testCancelRowDoesNotStartsNewSegment() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (TableModel model = defaultModel(tableName)) {
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 0, 3)) {
                    assertEquals(2, reader.getColumnCount());
                    assertEquals(walName, reader.getWalName());
                    assertEquals(tableName, reader.getTableName());
    ...
    public void testCommit() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (TableModel model = defaultModel(tableName)) {
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 0, 44)) {
                    assertEquals(2, reader.getColumnCount());
                    assertEquals(walName, reader.getWalName());
                    assertEquals(tableName, reader.getTableName());
                ...
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 1, 7)) {
                    assertEquals(2, reader.getColumnCount());
                    assertEquals(walName, reader.getWalName());
                    assertEquals(tableName, reader.getTableName());
            ...
            try (TableModel model = defaultModel(tableName)) {
                for (Map.Entry<Integer, AtomicInteger> counterEntry : counters.entrySet()) {
                    ...
                    for (int i = 0; i < count; i++) {
                        try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 2 * i, numOfRows)) {
                        ...
                        try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 2 * i + 1, 0)) {
                            assertEquals(2, reader.getRealColumnCount());
                            assertEquals(walName, reader.getWalName());
                            assertEquals(tableName, reader.getTableName());
                            assertEquals(0, reader.size());
            ...
            try (TableModel model = new TableModel(configuration, tableName, PartitionBy.HOUR)
                ...
                for (Map.Entry<Integer, AtomicInteger> counterEntry : counters.entrySet()) {
                    ...
                    for (int segmentId = 0; segmentId < count * numOfSegments; segmentId++) {
                        try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, segmentId, maxRowCount)) {
                            assertEquals(3, reader.getColumnCount());
                            assertEquals(walName, reader.getWalName());
                            assertEquals(tableName, reader.getTableName());
    ...
    public void testReadAndWriteAllTypes() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try {
                ...
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 0, rowsToInsertTotal)) {
    ...
    public void testRemovingColumnClosesSegment() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (TableModel model = defaultModel(tableName)) {
                ...
                try {
                    engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 1, 0);
    ...
    public void testRemovingNonExistentColumn() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (TableModel model = defaultModel(tableName)) {
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 0, 2)) {
                    assertEquals(2, reader.getColumnCount());
                    assertEquals(walName, reader.getWalName());
                    assertEquals(tableName, reader.getTableName());
                    assertEquals(2, reader.size());
    ...
    public void testSameWalAfterEngineCleared() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (TableModel model = defaultModel(tableName)) {
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, wal1Name, 0, 44)) {
                    assertEquals(2, reader.getColumnCount());
                ...
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, wal1Name, 1, 7)) {
                    assertEquals(2, reader.getColumnCount());
                    assertEquals(wal1Name, reader.getWalName());
                    assertEquals(tableName, reader.getTableName());
                    assertEquals(7, reader.size());
                ...
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, wal2Name, 0, 34)) {
                    assertEquals(2, reader.getColumnCount());
                ...
                try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, wal2Name, 1, 7)) {
                    assertEquals(2, reader.getColumnCount());
                    assertEquals(wal2Name, reader.getWalName());
                    assertEquals(tableName, reader.getTableName());
                    assertEquals(7, reader.size());
    ...
    public void testSymbolWal() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (TableWriter tableWriter = engine.getWriter(AllowAllCairoSecurityContext.INSTANCE, tableToken, "symbolTest")) {
                for (int i = 0; i < 5; i++) {
            ...
            try (WalReader reader = engine.getWalReader(AllowAllCairoSecurityContext.INSTANCE, tableToken, walName, 0, 10L)) {
                assertEquals(4, reader.getColumnCount());
    ...
    public void testWalEvenReaderConcurrentReadWrite() throws Exception {
        ...
        assertMemoryLeak(ff, () -> {
            ...
            WalWriter walWriter = engine.getWalWriter(AllowAllCairoSecurityContext.INSTANCE, tableToken);
            TableWriter.Row row = walWriter.newRow(0);
            row.putInt(0, 1);
            row.append();
    ...
    public void testWalWritersUnknownTable() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (WalWriter ignored = engine.getWalWriter(AllowAllCairoSecurityContext.INSTANCE, tableToken)) {
                Assert.fail();
            } catch (CairoException e) {
