...
public class BitmapIndexTest extends AbstractCairoTest {
    ...
    public static void create(CairoConfiguration configuration, Path path, CharSequence name, int valueBlockCapacity) {
        ...
        try {
            ...
            try (MAMemoryImpl mem = new MAMemoryImpl(ff, BitmapIndexUtils.keyFileName(path, name), ff.getPageSize())) {
    ...
    private MAMemoryImpl openKey() {
        try (Path path = new Path()) {
            return new MAMemoryImpl(configuration.getFilesFacade(), path.of(configuration.getRoot()).concat("x").put(".k").$(), configuration.getFilesFacade().getPageSize());
        }
    }
