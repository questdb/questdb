...
public class AlterTableAttachPartitionFromSoftLinkTest extends AlterTableAttachPartitionBase {
        ...
        assertMemoryLeak(FilesFacadeImpl.INSTANCE, () -> {
            ...
            attachPartitionFromSoftLink(tableName, partitionName, 2, "2022-10-17T00:00:17.279900Z", "2022-10-18T23:59:59.000000Z", "S3", txn -> {
                try {
                    ...
                    executeOperation("UPDATE " + tableName + " SET l = 13 WHERE ts = '" + partitionName + "T23:59:42.220100Z'", CompiledQuery.UPDATE);
                    txn++;
                    ...
                    executeOperation("UPDATE " + tableName + " SET l = 13 WHERE ts = '2022-10-17T00:00:34.559800Z'", CompiledQuery.UPDATE);
    ...
    public void testAlterTableAttachPartitionFromSoftLinkThenDetachIt() throws Exception {
        ...
        assertMemoryLeak(FilesFacadeImpl.INSTANCE, () -> {
            final String tableName = testName.getMethodName();
            final String partitionName = "2022-10-17";
            attachPartitionFromSoftLink(tableName, partitionName, 2, "2022-10-17T00:00:17.279900Z", "2022-10-18T23:59:59.000000Z", "SNOW", txn -> {
                try {
                    ...
                    other.of(configuration.getRoot()).concat(tableName).concat(partitionName).put(configuration.getAttachPartitionSuffix()).$();
                    Assert.assertFalse(ff.exists(other));
                    // verify no copy was produced to hot space
                    path.of(configuration.getRoot()).concat(tableName).concat(partitionName).put(TableUtils.DETACHED_DIR_MARKER).$();
    ...
    public void testAlterTableAttachPartitionFromSoftLinkThenDropIt() throws Exception {
        ...
        assertMemoryLeak(FilesFacadeImpl.INSTANCE, () -> {
            final String tableName = testName.getMethodName();
            final String partitionName = "2022-10-17";
            attachPartitionFromSoftLink(tableName, partitionName, 2, "2022-10-17T00:00:17.279900Z", "2022-10-18T23:59:59.000000Z", "IGLOO", txn -> {
                try {
                    ...
                    ff.walk(other, (file, type) -> fileCount.incrementAndGet());
                    Assert.assertTrue(fileCount.get() > 0);
                    path.of(configuration.getRoot()).concat(tableName).concat(partitionName).put(".2").$();
                    Assert.assertFalse(ff.exists(path));
                } catch (SqlException ex) {
                    Assert.fail(ex.getMessage());
                }
    ...
    public void testAlterTableAttachPartitionFromSoftLinkThenDropItWhileThereIsAReader() throws Exception {
        ...
        assertMemoryLeak(FilesFacadeImpl.INSTANCE, () -> {
            ...
            attachPartitionFromSoftLink(tableName, partitionName, 2, "2022-11-04T00:00:17.279900Z", "2022-11-05T23:59:59.000000Z", "FINLAND", txn -> {
                try {
                    try (TableReader ignore = engine.getReader(AllowAllCairoSecurityContext.INSTANCE, tableName)) {
                        ...
                        path.of(configuration.getRoot()) // <-- soft link path
                                .concat(tableName).concat(partitionName).put(".2").$();
                        Assert.assertTrue(Files.exists(path));
                    }
    ...
    public void testAlterTableAttachPartitionFromSoftLinkThenUpdate() throws Exception {
        ...
        assertMemoryLeak(FilesFacadeImpl.INSTANCE, () -> {
            final String tableName = testName.getMethodName();
            final String partitionName = "2022-10-17";
            attachPartitionFromSoftLink(tableName, partitionName, 2, "2022-10-17T00:00:17.279900Z", "2022-10-18T23:59:59.000000Z", "LEGEND", txn -> {
                try {
                    assertSql("SELECT min(ts), max(ts), count() FROM " + tableName, "min\tmax\tcount\n" + "2022-10-17T00:00:17.279900Z\t2022-10-18T23:59:59.000000Z\t10000\n");
                    ...
                    executeOperation("UPDATE " + tableName + " SET l = 13 WHERE ts = '" + partitionName + "T00:00:17.279900Z'", CompiledQuery.UPDATE);
                    assertSql("SELECT min(ts), max(ts), count() FROM " + tableName, "min\tmax\tcount\n" + "2022-10-17T00:00:17.279900Z\t2022-10-18T23:59:59.000000Z\t10000\n");
    ...
    private void attachPartitionFromSoftLink(String tableName, String partitionName, int partitionCount, String expectedMinTimestamp, String expectedMaxTimestamp, String otherLocation, Consumer<Integer> test) throws Exception {
        Assume.assumeTrue(Os.type != Os.WINDOWS);
        assertMemoryLeak(FilesFacadeImpl.INSTANCE, () -> {

            int txn = 0;
            try (TableModel src = new TableModel(configuration, tableName, PartitionBy.DAY)) {
                createPopulateTable(1, src.col("l", ColumnType.LONG).col("i", ColumnType.INT).col("s", ColumnType.SYMBOL).indexed(true, 32).timestamp("ts"), 10000, partitionName, partitionCount);
            }
            txn++;
    ...
    private void copyToDifferentLocationAndMakeAttachableViaSoftLink(String tableName, CharSequence partitionName, String otherLocation) throws IOException {
        ...
        copyPartitionAndMetadata( // this creates s3Buckets
                configuration.getRoot(), tableName, detachedPartitionName, s3Buckets, tableName, detachedPartitionName, null);
        Files.rmdir(path.of(configuration.getRoot()).concat(tableName).concat(detachedPartitionName).$());
        Assert.assertFalse(ff.exists(path));
        ...
        other.of(s3Buckets) // <-- the copy of the now lost .detached folder
                .concat(tableName).concat(detachedPartitionName).$();
        path.of(configuration.getRoot()) // <-- soft link path
                .concat(tableName).concat(partitionName).put(configuration.getAttachPartitionSuffix()).$();
