        ...
        assertMemoryLeak(() -> {
            ...
            try (TableWriter writer = engine.getWriter(AllowAllCairoSecurityContext.INSTANCE, "t1", "test")) {
                for(int i = 0; i < 10; i++) {
                    ...
                    row.putGeoHash(0, GeoHashes.fromString0("qeustdb", 0, 1));
                    row.putGeoHash(1, GeoHashes.fromString0("qeustdb", 0, 2));
                    row.putGeoHash(2, GeoHashes.fromString0("qeustdb", 0, 4));
                    row.putGeoHash(3, GeoHashes.fromString0("qeustdb123456", 0, 8));
                    row.putGeoHash(4, i);
                    row.append();
                }
                writer.commit();
