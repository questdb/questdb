...
public class LastDoubleGroupByFunctionFactoryTest extends AbstractGriffinTest {
    ...
    @Test
    public void testLastSequentialInput() throws SqlException {
        ...
        try (RecordCursorFactory factory = compiler.compile("select last(d) from tab", sqlExecutionContext).getRecordCursorFactory()) {
            try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                Record record = cursor.getRecord();
                Assert.assertEquals(1, cursor.size());
                Assert.assertTrue(cursor.hasNext());
                Assert.assertEquals(99.0, record.getDouble(0), 0.0001);
            }
        }
    }
}
