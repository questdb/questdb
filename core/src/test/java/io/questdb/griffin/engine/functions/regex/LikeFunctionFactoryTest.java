...
public class LikeFunctionFactoryTest extends AbstractGriffinTest {
    ...
    public void testLikeEscapeUnderscore1() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = compiler.compile("select * from myTable where comment like '%30\\%%'", sqlExecutionContext).getRecordCursorFactory()) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), false, sink);
    ...
    public void testLikeEscapeUnderscore2() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (RecordCursorFactory factory = compiler.compile("select * from myTable where comment like '%\\_sr'", sqlExecutionContext).getRecordCursorFactory()) {
                try (RecordCursor cursor = factory.getCursor(sqlExecutionContext)) {
                    sink.clear();
                    printer.print(cursor, factory.getMetadata(), false, sink);
                    Assert.assertEquals("", sink.toString().replace("\n", ""));
